...
| Lem 68 | `ReachedFromEmpty`, `RegistrationProtocol`, `RegistrationProvenance`, `SupportWellFounded`, `SupportSolution`, `SupportWellFoundedResult`, `supportWellFoundedTheorem` | **statement under repair/review**: the corrected premise ties child insertion to a tagged nonempty parent iterator stage and an admitted component rank; legal post-remove name reissue is allowed; well-foundedness and uniqueness remain unproved |
...
| Lem 72 | `ActionSubsequence`, `EpisodeDeletedActor`, `RegisteredNamesDuring`, `NoDependentClosingEpisode`, `NoRegisteredEpisode`, `DeletionResult`, `deletionTheorem` | **statement under repair/review, unproved**: selected deletion is lifecycle-only, so O-Retire/O-Remove survive; R-owned actions, open-R exclusion, relevant-time dependencies, provenance, all-trace totality, recovery, and outside-R controls remain explicit |
| Thm 73 | `NameBijection`, `RegistrationCorrespondenceByRenaming`, `SameOrchestrationModuloGenerated`, `SystemEquivalentByRenaming`, `CanonicalRegistrationCorrespondence`, `CanonicalSchedule`, `ConfluenceResult`, `confluenceTheorem` | **statement under repair/review, finite specialization**: canonical traces carry the same protocol and tree correspondence; Lemma-56 renaming fixes external names and transports generated names, parents, views, tables, and full controls; constructive proof remains open |
