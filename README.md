...
- `DGamma.CP4DeletionGenerationChecks`,
  ...
  the replayer for O-Insert/O-Retire/O-Remove. The five lifecycle tags remain the
  local replay blocker before endpoint and public Lemma-72 assembly. The
...
| Lem 72 | `GenerationActionSubsequence`, `RegistrationGeneration`, `RegisteredGenerationsDuring`, `EpisodeGenerationDeletedActor`, `WithdrawnGenerationResult`, `ControlEquivalentOutsideGenerations`, `DeletionResult`, `ActualEffectFrame`, `InactiveLeafDeletionPlan`, `checkedLifecycleAfterInactivePlan`, `buildCurrentRegisteredDeletionPlan`, `CurrentRegisteredInactiveFibers`, `reachedCurrentRegisteredInactive`, `CurrentRegisteredChildless`, `reachedCurrentRegisteredChildless`, `reachedDisciplinedBoundaryGivesDeletionPlan`, `CurrentRegisteredInactiveLeaves`, `currentRegisteredLeavesGivePlan`, `GenerationEnvironmentBounded`, `generationTraceScanPreservesUnique`, `currentGenerationOutsideImpliesActorOutsidePlan`, `checkedLifecycleAfterCurrentRegisteredPlan`, `deletionBeforeFromRegisteredDuring`, `decEpisodeGenerationDeletedActor`, `filterGenerationActions`, `splitLocatedNoRegisteredSegments`, `DeletionTraceSkeleton`, `selectedEpisodeRetainedReplayGivesReadiness`, `registeredGenerationRetainedReplayGivesReadiness`, `NoEpisodeReplayBoundary`, `DeletedRegisteredEpisodeBoundaryStep`, `deletedRegisteredOrchestrationHeadPreservesEpisodeBoundary`, `retainedSuffixHeadAfterCurrentPlan`, `retainedSuffixHeadAtBoundary`, `assembleDeletionResult`, `deletionTheorem` | **CP4 Findings #8 repair / partial; re-review required**: filtering, no-episode evidence, outside controls, and endpoint withdrawal use exact `(raw name, birth ordinal)` generations, so later legal reissues survive. A checked quiet-root countermodel proves the old raw filter wrong and the new filter right. Actual-effect frames cover all ten tags, every surviving lifecycle rule remains checked-applicable after indexed Inactive-leaf deletion, scanner ordinal bounds prove the complete pre-episode segment is retained verbatim, the total filter constructs the dependent subsequence whenever each kept replay action fires, and global no-episode/totality evidence is constructively split at exact generation boundaries. Trace/result assembly is proved conditional on filter success and the three exact endpoint invariants. Exact no-episode evidence proves current R generations remain Inactive; disciplined child-registration provenance plus well-formed parent closure now proves they are childless, including safe raw-name reissue, so the full exact-generation boundary plan is constructive. The selected-episode/suffix structural readiness inductions are proved from a saturated retained-head interface; the all-action suffix retained-head frame is proved at an exact current-R plan boundary; all retained and deleted one-head cases preserve the complete boundary with exact ambient/ordered-binding comparison, and `noEpisodeSuffixReplayFold` assembles the entire suffix with its final boundary; the selected-segment quotient now has its checked L-Begin base, full skipped-selected installed dispatcher, foreign Definition-60 effect transposition, exact checked-L-Unload effect join, and the approved ordered relational selected-to-suffix interface; retained foreign lifecycle control replay is now exhaustively dispatched from the occurrence-local provider and repaired outcome evidence; public no-episode evidence derives the empty current-R plan, and deleted R O-Insert/O-Retire/O-Remove heads now preserve the selected boundary while the survivor remains fixed; the parallel relational suffix induction now threads generation scanning, filter readiness, live-name uniqueness, and final ordered boundaries, with deleted heads discharged constructively and retained heads isolated behind the explicit per-action operational-congruence interface; that interface is now inhabited for all eight actions, including every repaired L-Advance failure/success branch; selected opening, selected-retire, retained foreign orchestration/lifecycle, selected-close, and generic relational endpoint evidence are proved. the simultaneous selected-segment fold is now checked through opening, every installed interior head, and selected L-Unload, producing `PostCloseSelectedBoundary`; the crossing-activation lifecycle provider exclusion is now proved from committed-provider persistence and `NoDependentClosingEpisode`, and `selectedClosedEpisodeFoldFromPremises` constructs the full fold without an extra anchor premise. Post-close full-control upgrades are checked for selected absence and surviving `Inactive Nothing`; standard effect congruence is checked for every retained action, and the complete retained foreign-orchestration post-close step threads selected-static control plus current-R invariants. Selected O-Retire and every retained foreign lifecycle prefix case now check with the full post-close invariant; only structural dispatch/discharge and the occurrence-to-`CurrentRegisteredWithdrawable` join remain; the accumulator algebra and complete selected installed-trace temporal induction (all control branches, boundary exclusions, and successful L-Advance pushes) are proved. Finding #10 makes exact effect equality retain complete ordered tables; Finding #12 completes that observable-state discipline for function-valued lifecycle accumulators, with a proof-distinct concrete control witness. All recovery, endpoint, renamed-control, and vestigial consumers were revalidated. |
