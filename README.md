...
| Lem 72 | `ActionSubsequence`, `EpisodeDeletedActor`, `RegisteredNamesDuring`, `WithdrawnNameResult`, `DeletionResult`, `ActualEffectFrame`, `deletionTheorem` | **partial**: selected deletion is lifecycle-only; R-owned actions and relevant-time guards remain explicit; registered endpoints cover both vestigial/absent and already-absent/absent cases. Actual-forward effect soundness is proved exhaustively for all ten evaluator tags, including every L-Advance alternative and accumulator-driven L-Unload. Independence-driven control applicability and deleted-trace assembly remain open. |
