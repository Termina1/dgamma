...
| Thm 73 | `RegistrationGenerationBijection`, `RegistrationTraceCorrespondence`, `RegistrationCorrespondenceByGeneration`, `CurrentEndpointRenaming`, `CanonicalRegistrationCorrespondence`, `ConfluenceResult`, `confluenceTheorem` | **statement under round-7 review, finite specialization**: cross-trace child births and their live-parent births are matched bijectively by `(raw name, birth ordinal)`, while external/current live roots are compared separately and fixed. The concrete child-1-vs-child-2 traces with the same later live root 1 construct the complete same-input premise and pass the full theorem-premise application guard. Canonical deletion remains per-generation; standalone endpoint historical metadata is validated only under `CanonicalSchedule`. Constructive deletion/sorting and general endpoint assembly remain unproved |
