...
| Lem 68 | `ReachedFromEmpty`, `RegistrationDiscipline`, `SupportWellFounded`, `SupportSolution`, `SupportWellFoundedResult`, `supportWellFoundedTheorem` | **statement under repair/review**: Erratum #3 requires an explicit trace premise—child insertion during the parent's Reloading activation, no name rebirth, and child retirement before parent recovery; well-foundedness and uniqueness are unproved |
...
| Lem 70 | `SupportMatchesActive`, `supportAtQuiescenceTheorem`, `supportMatchesActiveEmpty` | **statement under repair/review**: now requires `ReachedFromEmpty`, `RegistrationDiscipline`, and semantic Definition-69 totality; only the empty-registry base is proved |
...
| Lem 72 | `ActionSubsequence`, `RegisteredNamesDuring`, `NoDependentClosingEpisode`, `NoRegisteredEpisode`, `DeletionResult`, `deletionTheorem` | **statement under repair/review, unproved**: bidirectional filtering forces deletion; open R episodes, relevant-time dependency edges, retirement provenance, all-trace totality, effect recovery, and outside-R controls are explicit |
| Thm 73 | `SupportPath`, `LinearizesSupport`, `LocatedOpenEpisodeBlock`, `RootInputsBeforeLifecycle`, `SameExternalOrchestration`, `CanonicalInputPlacement`, `CanonicalEndpointRelation`, `CanonicalSchedule`, `FiberControlRelated`, `confluenceTheorem` | **statement under repair/review, finite specialization**: the previously verified combined order, unique contiguous blocks, and full Equation-53 controls remain; all-root placement and nonempty withdrawn endpoints are now explicit; only zero-withdrawal endpoint assembly is proved |
