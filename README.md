...
| Thm 73 | `LocatedActionOccurrence`, `RegistrationGeneration`, `LocatedGeneratedRegistration`, `NameBijection`, `RegistrationCorrespondenceByRenaming`, `CanonicalRegistrationCorrespondence`, `SystemEquivalentByRenaming`, `ConfluenceResult`, `confluenceTheorem` | **statement under review, finite specialization**: occurrence correspondence remains ordinal-bijective; canonical deletion accounts for `(raw name, birth ordinal)` generations separately from current raw endpoint omissions; root/child freshness and placement are per located birth. Thus a withdrawn child generation may coexist with a later live root generation of the same raw name; the nine-action role-changing regression is checked. Constructive sorting remains unproved |
