...
| Lem 68 | `ReachedFromEmpty`, `SupportWellFounded`, `SupportSolution`, `SupportWellFoundedResult`, `supportWellFoundedTheorem` | **stated** with the essential checked reached-from-empty premise; mixed parent/precedence well-foundedness and uniqueness remain proof debt |
| Def 69 | `ProgramFinishes`, `ComponentTotalOnProvision`, `ComponentsTotalOnProvision` | semantic component-level totality over every successful full program execution; `fiberTotalOnProvision`/`allFibersTotalOnProvision` are separately documented weaker current-Active runtime checks |
| Lem 70 | `SupportMatchesActive`, `supportAtQuiescenceTheorem`, `supportMatchesActiveEmpty` | **partial/stated**: paper statement now requires `ReachedFromEmpty` and semantic Definition-69 totality; only the empty-registry base is proved |
...
| Lem 72 | `ActionSubsequence`, `RegisteredNamesDuring`, `NoDependentClosingEpisode`, `NoRegisteredEpisode`, `DeletionResult`, `deletionTheorem` | **faithfully stated, unproved**: selected closing episode, exact registered set R, segment-specific deletion, effect recovery, outside-R control agreement, and vestigial/absent names are explicit |
| Thm 73 | `SupportPath`, `LinearizesSupport`, `LocatedOpenEpisodeBlock`, `CanonicalInputPlacement`, `CanonicalSchedule`, `FiberControlRelated`, `confluenceTheorem` | **faithfully stated finite/no-nested-registration specialization, unproved**: unique Equation-62 order, exactly one ordered contiguous open block per supported fiber, lifecycle coverage, input placement, and full Equation-53 control relation; orchestration equivalence and the final endpoint diagram are proved |
