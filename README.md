...
| Thm 61 | `AccumulatorHandle`, `actualAccumulatorAt`, `accumulatorEffectMap`, `ForeignReplay`, `AccumulatorFactorization`, `AccumulatorModel`, `beginAccumulatorModel`, `foreignStepPreservesAccumulatorModel`, `selectedRetirePreservesAccumulatorModel`, `successfulAdvancePushesAccumulatorFactorization`, `recoveryExactnessTheorem` | **partial/stated**: L-Begin base, exact runtime-handle model, foreign control persistence, selected O-Retire, and the only accumulator-changing successful L-Advance factorization are proved; foreign effect/replay commutation and remaining selected control branches are open, so the public alias remains uninhabited |
