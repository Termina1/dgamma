...
- `DGamma.CP3`: finite-host Lemmas 68–72/Theorem 73 interfaces, including
  activation-stamped, parent-local surviving-registration trees and exact
  historical external-root coupling.
...
| Thm 73 | `RegistrationActivation`, `SurvivingRegistration`, `DeletedClosingRegistration`, `RegistrationGenerationBijection`, `RegistrationEventMatch`, `RegistrationTraceCorrespondence`, `ExternalRootBirthCorrespondence`, `RegistrationCorrespondenceByGeneration`, `CurrentEndpointRenaming`, `CanonicalRegistrationCorrespondence`, `ConfluenceResult`, `confluenceTheorem` | **statement submitted for round-9 review, finite specialization**: only births surviving their parent activation episode enter the compared registration tree; discarded births require explicit later `L-Unload parent` evidence. Child positions reset at each `L-Begin` activation and remain parent-local, preserving within-activation iterator order without imposing cross-parent chronology. Historical external-root births remain coupled to exact matched O-Inserts and current endpoint names use a separate bijection. Public-premise checks cover fresh-choice, cross-parent interleaving, and a checked delay/divert/delete/reopen pair whose deleted left birth is excluded and whose surviving left/right births both have position zero. A complete-full-relation candidate rejects permutation of two removed historical roots. Constructive deletion/sorting and general endpoint assembly remain unproved |
