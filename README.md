...
| Lem 70 | `SupportMatchesActive`, `RegistrationDiscipline`, `supportAtQuiescenceTheorem`, `supportMatchesActiveEmpty`, `reachedActiveFibersProvideAll` | **statement repaired / partial; re-review required**: the alias now consumes repaired trace-indexed Definition-69 evidence while otherwise retaining the accepted premises. Actual Active-table totality is constructively derived from the reached trace; the remaining Active/support fixed-point assembly is in progress |
