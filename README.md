...
| Thm 66 | `ProgressResult`, `progressTheorem`, `continuationsBoundedBy`, `transitionPreservesContinuationsBoundedBy`, `UnboundedProgressTheorem`, `unboundedProgressAliasCounterexample`, `progressAliasCounterexample`, `progressCounterContinuationRejected`, `repairedContinuationPremisePositive`, `searchedLifecycleMove`, `maximalQuietFromNoDeadlock`, `progressEndFromNoDeadlock`, `progressEndFromSearch` | **approved CP4 statement repair / partial; re-review required**: the alias now exposes the paper-implicit bound on every initial `Reloading` continuation. The old shape admits a checked five-step same-target countermodel at `K=0`; the repaired premise rejects it, has a positive checked nonempty-trace witness, and is preserved by all ten evaluator rules. Search soundness, maximality, and the empty-suffix base are proved; unloading-chain no-deadlock and the general numeric bound remain open |
