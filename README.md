...
| Lem 72 | `ActionSubsequence`, `EpisodeDeletedActor`, `RegisteredNamesDuring`, `WithdrawnNameResult`, `DeletionResult`, `ActualEffectFrame`, `InactiveLeafDeletionPlan`, `checkedLifecycleAfterInactivePlan`, `deletionTheorem` | **partial**: selected deletion is lifecycle-only; R-owned actions and relevant-time guards remain explicit. Actual-forward effect soundness is proved exhaustively for all ten evaluator tags. The suffix control side is also proved: every surviving lifecycle rule remains checked-applicable after an indexed deletion of distinct Inactive leaves, including target, committed-capability, and reliance guards. Deriving that plan from the episode premises, handling the selected episode boundary, and final `ActionSubsequence`/endpoint assembly remain open. |
