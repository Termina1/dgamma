...
| Thm 66 | `ProgressResult`, `progressTheorem`, `searchedLifecycleMove`, `maximalQuietFromNoDeadlock`, `progressEndFromNoDeadlock`, `progressEndFromSearch` | **partial/stated**: exact theorem is stated; search soundness, maximality consequence, and the empty-suffix quantitative base are proved; unloading-chain no-deadlock and the precedence-count bound remain open |
| Def 67–69 | `supportSet`, `isSupported`, `fiberTotalOnProvision`, `allFibersTotalOnProvision`, `noFailedFibers` | executable bounded least-support computation and success/totality predicates |
| Lem 70 | `SupportMatchesActive`, `supportAtQuiescenceTheorem`, `supportMatchesActiveEmpty` | **partial/stated**: exact finite statement plus proved empty-registry base; well-founded support induction remains open |
| Lem 71 | `activationEffectTransposition` | **partial**: generated-monoid effect diamond proved; control applicability frames remain open |
| Lem 72 | `DeletionResult`, `deletionKeepsAll`, `deletionResultsCompose` | **partial**: identity and sequential composition proved; constructive one-episode deletion remains open |
| Thm 73 | `SameOrchestration`, `CanonicalSchedule`, `confluenceTheorem`, `sameOrchestrationReflexive`, `sameOrchestrationSymmetric`, `sameOrchestrationTransitive`, `canonicalEndpointsEquivalent`, `confluenceFromCanonicalSchedules` | **partial/stated**: exact theorem is stated; orchestration equivalence and final canonical-endpoint diagram are proved; constructive canonical sorting remains open |
