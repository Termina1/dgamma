#!/usr/bin/env python3
"""Read-only source/log/commit/immutable-plan authentication; no compiler launched."""
import datetime,hashlib,importlib.util,json,pathlib,re,subprocess,sys
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=pathlib.Path('/tmp/dgamma-r200');ART=ROOT/'research-tests';START='60869648'
...
assert sha((ART/'run-r200-check.py').read_bytes())==policy['runnerSHA256']==launch['runnerSHA256']
assert sha(subprocess.check_output(['git','show',launch['head']+':research-tests/run-r200-check.py'],cwd=ROOT))==policy['runnerSHA256']
assert sha((ART/'run-r200-final-validation.py').read_bytes())==launch['driverSHA256']
...
for i,r in enumerate(records):
    ...
    assert r['start']<('2026-09-09T16:49:00' if re.fullmatch(r'V\d+',r['unit']) else '2026-09-09T16:34:00')
...
source_receipts=[r for r in receipts if r['event']=='GUARDED COMMIT'];proof_receipts=[r for r in source_receipts if r.get('kind')!='authorized-comment-only']
assert len(source_receipts)==45 and len(proof_receipts)==44
...
auth=json.loads((OUT/'comment-authorization.json').read_text())
assert auth==json.loads((ART/'O6-R200-COMMENT-CORRECTION.json').read_text())
for receipt in receipts:
    ...
    if receipt['event']=='GUARDED COMMIT':
        ...
        if receipt.get('kind')=='authorized-comment-only':
            assert r['unit']=='D1-COMMENT' and path==auth['path'] and before.returncode==0
            assert sha(before.stdout)==auth['beforeSHA256'] and sha(after)==auth['afterSHA256']
            assert before.stdout.replace(auth['beforeComment'].encode(),auth['afterComment'].encode())==after
            assert c.declarations(before.stdout)==c.declarations(after)
        else:
...
assert [r['unit'] for r in proofs]==['B'+str(i)+'-1' for i in range(1,25)]+['A'+str(i)+'-1' for i in range(1,21)]
assert all(r['passed'] for r in proofs)
assert all(any(x['invocation']==r['unit'] for x in proof_receipts) for r in proofs)
assert not any(re.fullmatch(r'C\d+-\d+',r['unit']) for r in records), 'C ineligible'
assert [r['unit'] for r in records if not re.fullmatch(r'(?:[AB]\d+-\d+|V\d+)',r['unit'])]==['S0-1','D1-COMMENT']
assert not git('diff',START,'--','research/DGamma/CP5ProviderHeadObservedSpike.idr'), 'Exhausted R197 D5 changed'
plan_bytes=(OUT/'final-validation-plan.json').read_bytes();plan=json.loads(plan_bytes);scope=json.loads((ART/'O6-R200-VALIDATION-SCOPE.json').read_text())
assert sha(plan_bytes)==(OUT/'final-validation-plan.sha256').read_text().strip()==scope['planSHA256']==launch['planSHA256']
assert plan_bytes==(ART/'O6-R200-FINAL-VALIDATION-PLAN.json').read_bytes()
inventory=json.loads((ART/'O6-R199-ROOT-CONTRACT-COSTS.json').read_text())['entries'];assert len(inventory)==266
...
old=json.loads((ART/'O6-R199-COMPILER-LEDGER.json').read_text())['records'];inherited={r['path'] for r in old if r['passed'] and r['path']!='package'}
assert len(inherited)==157 and all((ROOT/p).is_file() for p in inherited)
assert not scope['excludedInheritedApplicablePaths']
assert {x['path'] for x in plan}==inherited|set(changed)|{'package'} and len(plan)==166
...
if '--require-final-complete' in sys.argv:
    assert len(completed)==166 and len(records)==212 and all(r['passed'] for r in records)
...
D_commits=[git('show','-s','--format=%s',r['resultingCommitHash']).strip() for r in receipts if re.match(r'R200 D[1-4]:',git('show','-s','--format=%s',r['resultingCommitHash']))]
assert len(D_commits)<=4 and len({x.split(':',1)[0] for x in D_commits})==len(D_commits)
report=dict(status='PASS',timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),head=git('rev-parse','HEAD').strip(),invocations=len(records),expectedPASS=sum(r['passed'] for r in records),rejected=[r['unit'] for r in records if not r['passed']],sourceReceipts=45,proofSourceReceipts=44,commentReceipts=1,artifactReceipts=len(receipts)-45,finalCompleted=len(completed),finalTotal=166,finalPlanSHA256=sha(plan_bytes),inheritedApplicableSources=157,newSources=8,changedIdrisSources=9,sourceSnapshotsAuthenticated=True,sourceReceiptsAuthenticated=True,allSourceCommitsReceipted=True,oneDeclarationPerProofCommit=True,authorizedCommentExact=True,allPriorCommittedNonDocCodeRetained=True,DCommits=D_commits,allChangedSourcesChecked=True,allPlannedTargetImportsTopological=True,compilerOverlap=False,resourceStops=[r['unit'] for r in records if r['resourceStopped']],mutations=[r['unit'] for r in records if r['targetMutationDetected']],noLockOperations=True,ownerPolicySHA256=sha(policy_bytes),crossLaneOverlapTimestampsUTC={r['unit']:r['crossLaneOverlapTimestampsUTC'] for r in records if r['crossLaneOverlapTimestampsUTC']},microUnitAttempts={r['unit'].rsplit('-',1)[0]:1 for r in proofs},AUnits=20,AInvocations=20,ARetained=20,BUnits=24,BInvocations=24,BRetained=24,exhaustedAndReverted=[],CProofAttempts=0,DCommentChecks=1,noStagedFiles=not git('diff','--cached','--name-only').strip(),qualification='Read-only machine authentication, NOT independent human proof review. Seeded direct targets, not cold. RSS sample0 means no live capture, not zero peak. Global selection/all-name bridge, physical canonical Insert pairing/whole modulo synchronization and convergence remain OPEN.')
