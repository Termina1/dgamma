#!/usr/bin/env python3
"""E3 validation-only coordinator: committed plan, unchanged targets, one check
...
OUT=Path('/tmp/dgamma-l2r11')
PLAN=ROOT/'research-tests/O6-L2R11-FINAL-VALIDATION-PLAN.json'
...
for task in plan['validations']:
 ...
 subprocess.run(['python3','-I',str(ROOT/'research-tests/run-l2r11-launch.py'),task['unit'],task['path']],cwd=ROOT,check=True)
