#!/usr/bin/env python3
"""Copied from L2R12 for L2R13: detached lane2-only seeded single check;
...
Usage: python3 -I run-l2r13-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]].
"""
...
assert path.startswith('research-tests/O6-L2R13-Sources/'), 'Lane-owned targets or exact unchanged bootstrap only'
...
if re.fullmatch(r'[ABCD]\d+-\d+',unit):
 ...
 assert int(unit.split('-')[0][1:]) <= {'A':14,'B':14,'C':10}[unit[0]]
...
heavy=False;locked=False;lock_events=[]  # L2R13: light-only; NEVER inspect or mutate shared lock/window.
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R13 task BUILD SEED; target only, dependency rebuilds acknowledged')
...
for i,(extra,data) in enumerate(bundle):
 ...
 bundle_records.append(dict(path=str(extra.relative_to(ROOT)),sourceSHA256=hashlib.sha256(data).hexdigest(),sourceFile=sourcefile,targetMtimeTouch=dict(path=str(extra),oldMtimeNs=old,newMtimeNs=extra.stat().st_mtime_ns,authority='No companion bundle is authorized in L2R13')))
...
# L2R13 persists ONLY overlap timestamps from its first invocation.
