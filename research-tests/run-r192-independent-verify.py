...
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=pathlib.Path('/tmp/dgamma-r192')
...
assert (OUT/'final-validation-plan.json').read_bytes()==(ROOT/'research-tests/O6-R192-FINAL-VALIDATION-PLAN.json').read_bytes()
assert (OUT/'final-source-recheck-plan.json').read_bytes()==(ROOT/'research-tests/O6-R192-FINAL-SOURCE-RECHECK-PLAN.json').read_bytes()
