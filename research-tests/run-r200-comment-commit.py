#!/usr/bin/env python3
"""Commit the one owner-authorized comment-only source correction after fresh PASS.
Usage: python3 -I research-tests/run-r200-comment-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-r200')/(unit+'.json')).read_text())
assert record['passed'] and record['fresh'] and not record['interrupted']
assert record['exit'] == 0 and not record['expectedDiagnostic']
assert json.loads(pathlib.Path('/tmp/dgamma-r200/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
assert unit == 'D1-COMMENT'
assert record['path'] == 'research/DGamma/CP5O20DeletionDisappearanceSpike.idr'
auth = json.loads(pathlib.Path('/tmp/dgamma-r200/comment-authorization.json').read_text())
assert old.returncode == 0
assert hashlib.sha256(old.stdout).hexdigest() == auth['beforeSHA256']
assert hashlib.sha256((ROOT/record['path']).read_bytes()).hexdigest() == auth['afterSHA256']
assert old.stdout.replace(auth['beforeComment'].encode(), auth['afterComment'].encode()) == (ROOT/record['path']).read_bytes()
assert declarations(old.stdout) == declarations((ROOT/record['path']).read_bytes())
...
assert not subprocess.check_output(['git','diff','--cached','--name-only'],cwd=ROOT,text=True).strip()
receipt = dict(event='GUARDED COMMIT', kind='authorized-comment-only', unit='D1', attempt='1', invocation=unit, sourceHash=record['sourceSHA256'], resultingCommitHash=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(), timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(), guardChecksPassed=['fresh PASS','exit 0','not interrupted','no expected diagnostic','exact source SHA256','exact owner-authorized comment replacement; all non-doc bytes identical','no intervening compiler invocation','no pre-staged files','no own-worktree compiler (lane2 excluded)','git diff --check','git commit success','no post-staged files'])
with pathlib.Path('/tmp/dgamma-r200/commit-receipts.jsonl').open('a') as ledger:
