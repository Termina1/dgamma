...
Usage: python3 -I research-tests/run-l2r1-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-l2r1')/(unit+'.json')).read_text())
assert record['passed'] and record['fresh'] and not record['interrupted']
assert record['exit'] == 0 and not record['expectedDiagnostic']
assert json.loads(pathlib.Path('/tmp/dgamma-l2r1/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
assert record['path'].startswith(('research/DGamma/CP5L2R1', 'research-tests/DGamma/L2R1')), 'Lane-owned source only'
