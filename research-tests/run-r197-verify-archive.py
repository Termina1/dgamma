...
ROOT=pathlib.Path(__file__).resolve().parents[1];ART=ROOT/'research-tests';OUT=pathlib.Path('/tmp/dgamma-r197')
sys.dont_write_bytecode=True
spec=importlib.util.spec_from_file_location('contract',ART/'r197_evidence_contract.py')
c=importlib.util.module_from_spec(spec);spec.loader.exec_module(c)
sha=lambda b:hashlib.sha256(b).hexdigest()
archive=ART/'O6-R197-COMPILER-EVIDENCE.tar.gz'
metadata=json.loads((ART/'O6-R197-EVIDENCE-VERIFICATION.json').read_text())
...
with tarfile.open(archive,'r:gz') as tar:
    ...
    assert all(n.startswith('dgamma-r197/') and '..' not in n.split('/') for n in names)
    def data(path):
        member=tar.extractfile('dgamma-r197/'+path);assert member is not None
        return member.read()
    anchor=json.loads(data('archive-anchor.json'))
    assert anchor['anchorCommit']==metadata['anchorCommit']
    assert set(names)=={'dgamma-r197/'+p for p in anchor['filesSHA256']}|{'dgamma-r197/archive-anchor.json'}
    ...
    source_receipts=[r for r in receipts if r['event']=='GUARDED COMMIT'];assert len(source_receipts)==33
    ...
    assert len(plan)==144 and json.loads(data('final-validation-result.json'))['status']=='PASS'
    assert data('final-validation-plan.json')==(ART/'O6-R197-FINAL-VALIDATION-PLAN.json').read_bytes()
    for item in plan:
        r=next(x for x in records if x['unit']==item['unit'])
        assert r['passed'] and r['sourceSHA256']==item['sourceHash']
report=dict(status='PASS',anchorCommit=metadata['anchorCommit'],archiveSHA256=metadata['archiveSHA256'],allArchiveFilesVerified=len(members),nativeRecordsVerified=189,sourceSnapshotsVerified=189,rawLogsVerified=189,sourceReceiptsArchived=33,artifactReceiptsArchived=len(receipts)-33,expectedPASS=178,rejectedSnapshotsRetained=11,finalChecksVerified=144,qualification='Read-only post-creation verification; not claimed inside its own archive or as parent/reviewer acceptance. Anchor deliberately excludes future publication/gate receipts.')
