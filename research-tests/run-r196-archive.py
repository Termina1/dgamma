...
plan=json.loads((ART/'O6-R196-DEPENDENT-RECHECK-PLAN.json').read_text())
...
archive_info=dict(anchorCommit=anchor,preparedUTC=now,qualification='Archive anchor precedes publication commit; own/future artifact receipts are not claimed inside. Original source snapshots and logs retained, including proposal before/after and owner ruling.',files=[str(p.relative_to(OUT)) for p in OUT.rglob('*') if p.is_file() and 'pycache' not in p.parts])
...
with tarfile.open(archive,'w:gz') as tar:
    for p in sorted(OUT.rglob('*')):
        if p.is_file() and 'pycache' not in p.parts:tar.add(p,arcname='dgamma-r196/'+str(p.relative_to(OUT)))
verification=dict(anchorCommit=anchor,archiveSHA256=sha(archive.read_bytes()),archiveBytes=archive.stat().st_size,invocations=len(records),inventoryRechecked=measured,inventoryTotal=244,unknownCostsRemaining=cost['unknownCostCount'],BCompleted=sum(r['passed'] and r['unit'].startswith('B') for r in records),BTotal=120,resourceStops=[r['unit'] for r in records if r.get('resourceStopped')],currentSourceHashes={p:sha((ROOT/p).read_bytes()) for p in {r['path'] for r in records if r['path']!='package'}},receiptQualification=archive_info['qualification'])
