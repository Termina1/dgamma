#!/usr/bin/env python3
"""Read-only R195 source/cache/census gate; never compiles or deletes seeds."""
import datetime
import hashlib
import json
import pathlib
...
# R195 has no type rehome permission; the prior R192 A11 revision is preserved.
...
# No LocalDiamond visibility change in R195; seeded TTC retained without deletion.
report = dict(timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat(),head=git('rev-parse','HEAD').strip(),start=START,
    ...
    CrossTraceDiffVsStart='empty',RenamingCompositionDiffVsStart='empty (R192 A11 manifest preserved)',
...
out = pathlib.Path(sys.argv[1] if len(sys.argv)>1 else '/tmp/dgamma-r195/frozen.json')
