...
spec=importlib.util.spec_from_file_location('l2r10_verifier',Path(__file__).with_name('run-l2r10-independent-verify.py'))
v=importlib.util.module_from_spec(spec);spec.loader.exec_module(v)

class EvidenceTests(unittest.TestCase):
 def setUp(self):
  self.path='research-tests/O6-L2R10-Sources/DGamma/L2R10Test.idr'
  self.source=b'module DGamma.L2R10Test\n%default total\npublic export\ndummy : Nat\ndummy = 0\n'
  self.log='1/1: Building DGamma.L2R10Test ('+str(v.ROOT/self.path)+')\n'
 ...
 def test_wrong_target_build(self):
  log=self.log.replace('L2R10Test','Wrong');r=copy.deepcopy(self.r);r.update(transcript=log,buildingLines=log.splitlines())
  with self.assertRaises(AssertionError):v.record_valid(r,self.source,log)
 ...
 def test_only_c3_declared_heavy(self):self.rejects(declaredHeavy=True)
 ...
 def test_path_traversal(self):self.assertFalse(v.path_allowed('research-tests/O6-L2R10-Sources/../../src/DGamma/X.idr'))
 def test_absolute_path(self):self.assertFalse(v.path_allowed(str(v.ROOT/self.path)))
 def test_exact_predecessor_comment_target(self):self.assertTrue(v.path_allowed(v.REPAIR))
