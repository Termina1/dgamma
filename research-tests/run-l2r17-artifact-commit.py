#!/usr/bin/env python3
"""Copied from L2R15: guarded L2R17 NON-IDRIS artifact publication following
an exact source PASS. No predecessor, protected document or source edits.
Usage: python3 -I research-tests/run-l2r17-artifact-commit.py UNIT MESSAGE PATH...
"""
import datetime,hashlib,json,pathlib,re,subprocess,sys
ROOT=pathlib.Path('/Users/vyacheslavshebanov/Work/dgamma-lane2');OUT=pathlib.Path('/tmp/dgamma-l2r17')
...
assert all(p.startswith(('research-tests/O6-L2R17-','research-tests/run-l2r17-')) and not p.endswith('.idr') and (ROOT/p).is_file() for p in paths)
