#!/usr/bin/env python3
"""Copied from L2R11 for L2R12: detached lane2-only seeded single check;
...
Usage: python3 -I run-l2r12-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]].
"""
...
OUT = pathlib.Path('/tmp/dgamma-l2r12')
...
assert path.startswith('research-tests/O6-L2R12-Sources/') or (unit == 'V0' and path == 'research-tests/O6-L2R11-Sources/DGamma/L2R11PhaseDecode.idr'), 'Lane-owned targets or exact unchanged bootstrap only'
...
if re.fullmatch(r'[ABCD]\d+-\d+',unit):
 ...
 assert int(unit.split('-')[0][1:]) <= {'A':14,'B':14,'C':14,'D':8}[unit[0]]
...
heavy=False;locked=False;lock_events=[]  # L2R12: light-only; NEVER inspect or mutate shared lock/window.
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R12 task BUILD SEED; target only, dependency rebuilds acknowledged')
