...
Usage: python3 -I research-tests/run-r198-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-r198')/(unit+'.json')).read_text())
assert record['passed'] and record['fresh'] and not record['interrupted']
assert record['exit'] == 0 and not record['expectedDiagnostic']
assert json.loads(pathlib.Path('/tmp/dgamma-r198/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
with pathlib.Path('/tmp/dgamma-r198/commit-receipts.jsonl').open('a') as ledger:
