...
previous117 accounting and the untouched SupportSolution TTC. No compiler/locks.
"""
import sys, pathlib, json, re, collections, subprocess, io, tarfile
sys.dont_write_bytecode=True
sys.path.insert(0,str(pathlib.Path(__file__).resolve().parent))
from r207_common import ROOT, OUT, START, sha, git, frozen, compiler_scopes, write_json, utc
from r207_evidence_contract import validate_record

report=json.loads((ROOT/'research-tests/O6-R207-DISPOSITION.json').read_text())
rs=[json.loads(x) for x in (OUT/'ledger.jsonl').read_text().splitlines()]
assert len({r['unit'] for r in rs})==len(rs)
assert json.loads((ROOT/'research-tests/O6-R207-COMPILER-LEDGER.json').read_text())['records']==rs
...
for r in rs:
    ...
    m=re.fullmatch(r'(U0|E|VREST|A|B)(\d+)-(\d+)',unit)
    if m:
        assert 1<=int(m[2])<=dict(U0=10,E=26,VREST=1,A=16,B=16)[m[1]] and 1<=int(m[3])<=3
        budgets[m[1]+m[2]].append(r)
    cutoff='2026-09-10T08:18:00' if unit.startswith(('V','P')) and not unit.startswith('VREST') else '2026-09-10T08:03:00'
...
assert not any(p for p in OUT.glob('*.pid') if re.fullmatch(r'[A-Za-z0-9_-]+\.pid', p.name) and not p.with_suffix('.json').exists())
...
base={m['path']:m for m in json.loads((ROOT/'research-tests/O6-R206-DISPOSITION.json').read_text())['modules']}
...
for p,m in mods.items():
    ...
    elif status.startswith('R206 authenticated seed'):
...
blocked117={p for p,b in base.items() if b['status']=='blocked by untrusted/retired import'}
assert len(blocked117)==117
counts=collections.Counter(mods[p]['status'] for p in blocked117)
assert dict(counts)==report['R206Blocked117']['statusCounts']
...
assert (ROOT/new).read_bytes()==subprocess.check_output(['git','show',START+':'+new],cwd=ROOT)

# Exact obligations and import-only migrations are checked independently of
# the editing scripts. TYPE-only records do not count as proof inhabitants.
needs=json.loads((ROOT/'research-tests/O6-R207-CROSSTRACE-NEEDS.json').read_text())
assert len(needs['statements'])==36 and needs['recordedBeforeUnitE']
for spec in needs['statements']:
    baseline=subprocess.check_output(['git','show',START+':'+spec['path']],cwd=ROOT,text=True)
    assert baseline.count(spec['baselineStatement'])==1,(spec['path'],spec['name'])
original=next(s['baselineStatement'] for s in needs['statements'] if s['name']=='o19ActualOperationalBlockSwap')
operational=(ROOT/'research/DGamma/CP5O19OperationalAssemblySpike.idr').read_text()
assert 'O19OperationalUnconditionalObligation =\n'+original.split(' :\n',1)[1] in operational
resolver=json.loads((ROOT/'research-tests/O6-R207-BLOCK-RESOLVER-NEEDS.json').read_text())
baseline=subprocess.check_output(['git','show',START+':'+resolver['path']],cwd=ROOT,text=True)
assert sha(baseline.encode())==resolver['sourceBeforeSHA256'] and len(resolver['statements'])==5
for spec in resolver['statements']:
    assert baseline.count(spec['preMigrationStatement'])==1
    assert spec['expandedStatement']==spec['preMigrationStatement'].replace('ActorLifecycleOnly actor body','ActorLifecycleOnly nameEq actor body').replace('ActorLifecycleOnly left body','ActorLifecycleOnly nameEq left body')
for p,constructor in [
    ('research/DGamma/CP5O19WholeBlockSpike.idr','MkO19WholeBlockUnconditionalObligation'),
    ('research/DGamma/CP5O19ReachedBlocksSpike.idr','MkO19ReachedUnconditionalObligation'),
    ('research/DGamma/CP5O20BlockResolverFrameSpike.idr','MkO20ExpandedBlockResolverObligation')]:
    assert (ROOT/p).read_text().count(constructor)==1,p
imports=json.loads((ROOT/'research-tests/O6-R207-IMPORT-AUDIT.json').read_text())
for entry in imports['entries']:
    p=entry['path'];baseline=subprocess.check_output(['git','show',START+':'+p],cwd=ROOT,text=True)
    assert sha(baseline.encode())==entry['sourceBeforeSHA256']
    if entry['status'].startswith('PASS'):
        assert not entry['crossTraceDeclaredNamesUsed']
        replacement='' if 'CanonicalWordCoverage' in p else 'import DGamma.CP5O19SurfaceSpike\n'
        assert (ROOT/p).read_text()==baseline.replace('import DGamma.CP5ConfluenceCrossTraceSpike\n',replacement)
        assert sha((ROOT/p).read_bytes())==entry['sourceAfterSHA256']
    else: assert (ROOT/p).read_text()==baseline,p
...
assert sum(c['unit']=='ARTIFACT' for c in commits)<=4, 'D-phase artifact cap'
...
archive=ROOT/'research-tests/O6-R207-EVIDENCE.tar.gz'
manifest=json.loads((ROOT/'research-tests/O6-R207-ARCHIVE-MANIFEST.json').read_text())
...
result=dict(timestampUTC=utc(),head=last,kind='compiler-free independent evidence cross-check; not an external reviewer',passed=True,
    ...
    proofBudgetMax=3,microUnits={k:len(v) for k,v in budgets.items()},unitCaps=dict(U0=10,E=26,VREST=1,A=16,B=16,C=6,D=4),invalidationClosure=len(closure),
    invalidatedUnknown=0,previous117StatusCounts=dict(counts),productionByteIdentity=True,frozenUnchanged=True,
    census=frozen()['census'],supportSolutionTTC=support,guardedCommitReceipts=len(commits),sourceCommitReceipts=sum(c['unit'] not in ['ARTIFACT','RETIRE'] for c in commits),noStagedFiles=True,noOwnedCompiler=True,
    ...
    reviewGate='External read-only reviewer and supervisor gate required; this script is NOT that review')
if '--read-only' not in sys.argv:
    write_json(ROOT/'research-tests/O6-R207-INDEPENDENT-VERIFICATION.json',result)
