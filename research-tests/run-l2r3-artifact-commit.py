...
Only L2R3 artifacts, plus TWO exact authorized L2R2 runner docstring
...
Usage: python3 -I research-tests/run-l2r3-artifact-commit.py UNIT MESSAGE PATH...
"""
...
BASE = '1893851e'
REPAIRS = {'research-tests/run-l2r2-commit.py': (b'Commit exactly one source only after an authenticated fresh PASS.', b'Commit one declaration after fresh PASS; authorized C4 two-file bundles are the exception.'), 'research-tests/run-l2r2-check.py': (b' data. Performs the approved target-only mtime touch.', b' data. Performs target-only mtime touch; authorized C4 two-file bundles are the exception.')}
...
repair_checks = []
for path in paths:
 assert path in REPAIRS or (path.startswith(('research-tests/O6-L2R3-', 'research-tests/run-l2r3-')) and not path.endswith('.idr'))
