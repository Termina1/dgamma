#!/usr/bin/env python3
"""Persist R180 exact per-attempt evidence and verify serialized intervals."""
import hashlib
import json
import pathlib
import re
...
OUT = pathlib.Path('/tmp/dgamma-r180')
...
for r in records:
    ...
    if r['path'] != 'package':
        ...
        if r['path'] not in accepted_names:
            baseline = subprocess.run(['git','show','7fa4185:'+r['path']],cwd=ROOT,stdout=subprocess.PIPE,stderr=subprocess.DEVNULL)
            ...
            if base_names:
                r['baselineDeclarationSource'] = '7fa4185:'+r['path']
    ...
    history = subprocess.check_output(['git','log','--reverse','--format=%H','7fa4185..HEAD','--',r['path']],cwd=ROOT,text=True).splitlines()
...
(ROOT/'research-tests/O6-R180-COMPILER-LEDGER.json').write_text(json.dumps(records,indent=2)+'\n')
with tarfile.open(ROOT/'research-tests/O6-R180-COMPILER-EVIDENCE.tar.gz','w:gz') as archive:
    for r in records:
