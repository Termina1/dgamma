#!/usr/bin/env python3
"""One serialized, seeded, fresh R202 check. Launch detached with python3 -I.
No TTC is deleted. JSON/log/source snapshots live under /tmp/dgamma-r202.
Usage: run-r202-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]]; PATH=package for build.
"""
import datetime
import hashlib
import json
...
OUT = pathlib.Path('/tmp/dgamma-r202')
...
# Conservative shift start15:06Z; timeout19:06Z; new proof18:26Z; validation18:41Z.
...
cutoff = datetime.datetime(2026,9,9,20,37,0,tzinfo=datetime.timezone.utc) if planned_validation else datetime.datetime(2026,9,9,20,22,0,tzinfo=datetime.timezone.utc)
assert datetime.datetime.now(datetime.timezone.utc) < cutoff, 'R202 attempt20:22:00 / frozen-validation20:37:00 start guard'
...
if path == 'package':
    ...
    if any(word in path for word in ['ActorLifecycleOnlyExtended','CP5AvailabilityAware']):
        assert planned_validation and snapshot == subprocess.check_output(['git','show','bc791641:'+path],cwd=ROOT)
    target.touch()
...
# R202 starts under the already-effective owner no-lock rule.
...
for frozen in ['CP5ConfluenceLocalDiamondSpike','CP5ConfluenceDeletionChainSpike','CP5O19SurfaceSpike','CP5ConfluenceCrossTraceSpike','CP5ConfluenceCanonicalSortSpike','CP5ConfluenceRenamingCompositionSpike']:
    ...
    assert (ROOT/fp).read_bytes()==subprocess.check_output(['git','show','bc791641:'+fp],cwd=ROOT), 'Frozen source changed: '+fp
...
if path == 'research/DGamma/CP5ConfluenceLocalDiamondSpike.idr':
    ...
    assert snapshot == subprocess.check_output(['git','show','bc791641:'+path],cwd=ROOT), 'Changed LocalDiamond requires prior gate'
