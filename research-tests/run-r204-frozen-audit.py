#!/usr/bin/env python3
"""Read-only R204 exact whole-frozen-source/body/census/seed audit; no compiler.
...
START='9b532666'
OUT=pathlib.Path('/tmp/dgamma-r204')
...
old_frozen=json.loads((ROOT/'research-tests/O6-R200-FROZEN-AUDIT.json').read_text())
...
assert changed, "R204 retained research modules required"
for p in changed:
    assert '%default total' in (ROOT/p).read_text()
    prior=subprocess.run(['git','show',START+':'+p],cwd=ROOT,capture_output=True)
    assert prior.returncode!=0 or p=='research-tests/DGamma/R203NativeDisappearancePositive.idr', 'Only new proof sources or exact authorized R203 P1 fixture repair'
    if prior.returncode==0:
        assert sha(prior.stdout)=='b05d2323fbec18160bc212a7ffa3270f0e28e7dc68cfaaa24b1c181f8e410068'
        assert sha((ROOT/p).read_bytes())=='6946cbd30558b3013a418e0d3cb7ff85ef3698e3e7cd6555545079df6b596618'
...
prohibited={p:re.findall(p,code,re.M) for p in [r'\blet\b',r'\bwith\b',r'\bbelieve_me\b',r'\bassert_total\b',r'\bassert_smaller\b',r'^\s*partial\b',r'\?\w+',r'\bdeletionTheoremProof\b',r'\bpostulate\b',r'\b(?:sortClosingFreeTraceSpike|selectOperationalCanonicalPermutationSpike|canonicalSchedulesConvergeSpike|replayedCanonicalToOriginalEndpointSpike)\b']}
...
record['declaredNewSourceNames']={}
for p in changed:
    prior=subprocess.run(['git','show',START+':'+p],cwd=ROOT,capture_output=True)
    record['declaredNewSourceNames'][p]=sorted(declarations((ROOT/p).read_text())-declarations(prior.stdout.decode() if prior.returncode==0 else ''))
...
assert record['retainedNewDeclarations']==sum(r['event']=='GUARDED COMMIT' and r.get('invocation')!='U0a-2' for r in receipts)==29
record['rejectedDevelopmentInvocations']=[r['unit'] for r in [json.loads(s) for s in (OUT/'ledger.jsonl').read_text().splitlines()] if not r['passed']]
record['policyLabel']='R204 from first invocation; no metadata correction'
record['ordinarySyntaxQualification']='No new let aliases or with. R203 A13 structurally recursive erased where rightHistory remains byte-identical. P1 inline the annotations preserve the entire old statement; no runtime/proof escape hatch.'
record['CEligibility']='INELIGIBLE: A/B partial, no macro body attempted'
record['A15Reverted']=json.loads((OUT/'A15-revert.json').read_text())
assert record['A15Reverted']['exactHeadBytes']
assert sha((ROOT/record['A15Reverted']['path']).read_bytes())==record['A15Reverted']['revertedSHA256']
