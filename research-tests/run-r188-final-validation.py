#!/usr/bin/env python3
"""Detached serial final validations from the immutable R188 plan.
...
compilers. R188 proof stop and all-start cutoff are enforced by the check runner.
"""
import datetime
import json
import pathlib
...
for item in plan:
    ...
    args = ['python3', '-I', str(ROOT/'research-tests/run-r188-check.py'), item['unit'], item['path']]
    ...
    if result.returncode or not record['passed'] or not record['fresh']:
        ...
        sys.exit(1)
(OUT/'final-validation-complete.json').write_text(json.dumps(dict(completedUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(), invocations=[p['unit'] for p in plan], serial=True), indent=2)+'\n')
