...
Usage: python3 -I research-tests/run-l2r13-launch.py UNIT TARGET
"""
...
with (OUT/(unit+'.wrapper.log')).open('w') as log:
    child=subprocess.Popen([sys.executable,'-I',str(ROOT/'research-tests/run-l2r13-check.py'),unit,target],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
