#!/usr/bin/env python3
"""Adapted from run-l2r7-fixture-recipe.py: pure one-origin contiguity recipes.
...
def step(schedule,index):
 ...
 return f'(Fired {{before = contiguityState {p[index]}}} {{afterState = contiguityState {p[index+1]}}} %search %search ({action}) {tag} (contiguityEdge{j} contiguityNativeExecution))'
