L2R7 FINAL GATE at 4469421e ACCEPTED as an explicitly PARTIAL bounded milestone; stand-down authorized; independent reviewer launched.

# L2R8 shift audit

Exact clean start 4469421e, cp5-thm73-lane-a8a10, lane2 worktree only. L2R7 audit/origins read first, then Tier-2 draft. Main tree and heavy lock/window untouched.

## A scope ruling

Supervisor AUTHORIZED a NEW proof-carrying isElem release scan, observing the actual library result by its equation at the call site. General releaseScanAgrees with unchanged scanReleaseOrdinals is NAMED OPEN, not assumed. Every downstream theorem using the new scan must disclose this. Both concrete scans must be compared on C12/barrier. A8 is not re-entered.

## A bounded stop

A1–A9 retained; A10 observedReleaseFixtures exhausted3/3 and fully reverted to A9 bytes. No fourth attempt/restatement. A1 passed3/3 at quantity0 (missing direct isYes import, then erased proof in runtime absurd). A9 concrete-index type was proactively stopped at sampled8,939,200KiB; abstract native-trace fields passed2/3 at607,104KiB. A10 attempts1/2 cannot reduce erased proof-carrying scan to [3] in a consumer; attempt3 own literal native equations hit checkedApplyAction reduction first. Supervisor executable-ordinal-scan hint arrived after A10-3 launch. New runtime ordinal scan and proof linkage are OPEN; A9 is a contract only, no data-level C12/barrier agreement produced. General releaseScanAgrees is NAMED OPEN, not assumed. Phases are not produced.
...
## E2 — actual final checks and publication evidence

**Immutable plan published at9c72d7ae: V10–V21 completed12/12 fresh own-target PASS**, leaf-before-dependent, serialized detached driver. Exactly ONE own Building line per check; all hashes match the plan; no source edits followed. Final-validation maximum sampled RSS **1,330,688KiB = 1.269042969GiB**. No own compiler remains.

Invocation ledger **62 total =48 bounded proof attempts + V0/V1 +12 final validations;51 PASS /11 FAIL**. All37 retained declarations have their own exact-source fresh PASS and guarded publication receipt. The11 failed attempts comprise7 compiler rejections and4 interrupted checks (A9-1/B2-1 proactive stops; B3-1/B3-2 light-memory stops). Maximum all-shift sampled RSS **18,927,216KiB = 18.050399780GiB**, below19GiB; this is250ms sampling, NOT a continuous peak guarantee. The light18GiB stop is stricter than the48GiB ceiling. No source mutation or heavy-lock event; no main tree, lock/window, build deletion, mass touch or foreground target check.

**16/16 compiler-free guard AST tests PASS**, rerun after final checks. **Independent prepublication reconstruction PASS at9c72d7ae**: all62 individual records/logs/source snapshots; exact one-declaration/unchanged-prior-body source deltas; chronological receipts; A10 full revert and B3 deferred full revert; attempt/cap/source-closure rules; plan-before-check timing and exact source hashes; all old draft Idris fragments/manifest fields/source hashes and30 renamings; frozen paths unchanged; no staged files or own compiler. This is script-based independent evidence reconstruction, **NOT independent human proof review**.
...
**Types/data only:** explicit contiguity states and17-edge contract (no native fixture); authentic core/general core-restoration TYPE; revised placed-prefix phase invariant TYPE; C12/barrier release-agreement fixture TYPE. No type is counted as its missing producer.

**Open/deferred:** releaseScanAgrees and the unrestricted executable ordinal/proof split; C12/barrier release agreement (A10 stopped3/3); phaseScanOk/produceForcedRootPhases; coreContiguityRestored fixture (B3 deferred2/3) and general proof; positive-root forcing/crossing linkage; native squares including Insert/Insert; produceAdmittedDistanceMove/normalizePhaseDistance; native D8 reproduction FROM general producers; stable anchors/other-root distance/phase transport; origin map/filter→entry/native-birth identity and maximality converse; front/control exclusion/general placement; produceAttachedNormalForm; all-premises-produced zero-gap for both originals and attachedC/controlFixture; cross-bundle authenticated prefix/generation history.

**Next only under a new supervisor gate:** review this partial boundary; design the runtime ordinal/proof observation split without restating exhausted A10; split the native fixture's scalar packets only under B3's retained-budget ruling; finish phase/move/outer-invariant transport before general normalization; then finish origin/front/control/placed-NF integration. The draft remains NOT SIGNABLE as a complete attached grammar + placement cure. Stand-down after E3/final gate; parent-owned independent reviewer remains REQUIRED.
