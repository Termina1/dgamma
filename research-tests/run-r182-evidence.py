#!/usr/bin/env python3
"""Persist R182 exact per-attempt evidence and verify serialized intervals."""
import hashlib
import json
import pathlib
import re
...
OUT = pathlib.Path('/tmp/dgamma-r182')
...
for r in records:
    ...
    if r['path'] != 'package':
        ...
        if r['path'] not in accepted_names:
            baseline = subprocess.run(['git','show','159ef07:'+r['path']],cwd=ROOT,stdout=subprocess.PIPE,stderr=subprocess.DEVNULL)
            ...
            if base_names:
                r['baselineDeclarationSource'] = '159ef07:'+r['path']
...
with tarfile.open(ROOT/'research-tests/O6-R182-COMPILER-EVIDENCE.tar.gz','w:gz') as archive:
    for r in records:
