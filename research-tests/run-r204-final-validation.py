...
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=pathlib.Path('/tmp/dgamma-r204')
...
assert data==(ROOT/'research-tests/O6-R204-FINAL-VALIDATION-PLAN.json').read_bytes()
scope=json.loads((ROOT/'research-tests/O6-R204-VALIDATION-SCOPE.json').read_text())
assert len(plan)==scope['checksIncludingPackage'] and len({x['path'] for x in plan})==len(plan)
assert scope['inheritedSources']==181 and not scope['excludedInheritedApplicablePaths']
...
assert sha((ROOT/'research-tests/run-r204-check.py').read_bytes())==policy['runnerSHA256']
...
assert not any(re.fullmatch(r'V\d+',r['unit']) for r in records), 'No silent replay; authorized V0-CACHE is not a plan target'
completed=[]
for item in plan[len(completed):]:
    assert sha((ROOT/'research-tests/run-r204-check.py').read_bytes())==launch['runnerSHA256'], 'Runner mutated'
    ...
    command=[sys.executable,'-I',str(ROOT/'research-tests/run-r204-check.py'),item['unit'],item['path']]
