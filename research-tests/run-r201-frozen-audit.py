#!/usr/bin/env python3
"""Read-only R201 exact whole-frozen-source/body/census/seed audit; no compiler."""
...
assert changed, "R201 retained research modules required"
for p in changed:
    assert '%default total' in (ROOT/p).read_text()
    prior=subprocess.run(['git','show',START+':'+p],cwd=ROOT,capture_output=True)
    assert prior.returncode!=0, 'R201 changes only new sources; inherited source edits not authorized'
...
assert record['retainedNewDeclarations']==sum(r['event']=='GUARDED COMMIT' and r.get('kind')!='authorized-comment-only' for r in receipts)==45
record['fixtureExpectationCorrection']=json.loads((OUT/'A19-amendment.json').read_text())
