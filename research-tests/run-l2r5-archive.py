...
Usage: python3 -I research-tests/run-l2r5-archive.py L2R5 769d332d FINAL_HEAD
...
assert sys.argv[4:] == []
publication = ''
assert shift=='L2R5' and start=='769d332d'
...
for r in records:
    ...
    assert r['path'].startswith('research-tests/O6-L2R5-Sources/') or (r['unit']=='V0' and r['path']=='research-tests/O6-L2R4-Sources/DGamma/L2R4AnchorMeasure.idr'), 'Lane-owned or exact bootstrap target only'
    target = r['path']
...
(ROOT/'research-tests/O6-L2R5-MICRO-UNIT-LEDGER.json').write_text(json.dumps(dict(base=start,sourceBoundary=end,caps={'A':20,'B':16,'C':10,'D':6},attemptedSlots={'A':20,'B':5,'C':10},retainedDeclarations=33,unusedSlots={'B':list(range(6,17))},unusedAuthority='Supervisor-ratified provider UNIT STOP after L2R4 B7 and L2R5 B5; no deletion-square relabeling',records=micro),indent=2)+'\n')
