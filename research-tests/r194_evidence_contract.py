...
def validate_attempts(records, snapshots):
    ...
    for unit, runs in attempts.items():
        ...
        if any(r['passed'] for r in runs[:-1]):
            assert unit == 'A8' and len(runs) == 2 and all(r['passed'] for r in runs), unit
            assert source_code(snapshots['A8-1']) == source_code(snapshots['A8-2']), 'A8 exception is comments only'
            assert snapshots['A8-1'] != snapshots['A8-2'], 'A8 documented docstring correction'
    for lane, cap in [('A', 30), ('B', 16)]:
