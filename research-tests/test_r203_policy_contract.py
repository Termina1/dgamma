...
spec=importlib.util.spec_from_file_location('contract',ROOT/'research-tests/r203_evidence_contract.py')
c=importlib.util.module_from_spec(spec);spec.loader.exec_module(c)
class PolicyTests(unittest.TestCase):
    def setUp(self):
        self.p=json.dumps(dict(authorization='SUPERVISOR RULE CHANGE: CROSS-LANE HEAVY LOCK ABOLISHED',runnerSHA256='new',shift='R203',oneCompilerPerLane=True)).encode()
