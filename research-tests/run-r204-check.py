#!/usr/bin/env python3
"""One serialized, seeded, fresh R204 check. Launch detached with python3 -I.
No TTC is deleted. JSON/log/source snapshots live under /tmp/dgamma-r204.
Usage: run-r204-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]]; PATH=package for build.
...
# R204 start20:39Z; timeout00:39Z; no new proofs23:59Z; validation by00:14Z.
...
cutoff = datetime.datetime(2026,9,10,0,14,0,tzinfo=datetime.timezone.utc) if planned_validation else datetime.datetime(2026,9,9,23,59,0,tzinfo=datetime.timezone.utc)
assert datetime.datetime.now(datetime.timezone.utc) < cutoff, 'R204 attempt23:59:00 / frozen-validation00:14:00 start guard'
...
if path == 'package':
    ...
    if any(word in path for word in ['ActorLifecycleOnlyExtended','CP5AvailabilityAware']):
        assert planned_validation and snapshot == subprocess.check_output(['git','show','9b532666:'+path],cwd=ROOT)
    target.touch()
...
# R204 starts under the already-effective owner no-lock rule.
...
for frozen in ['CP5ConfluenceLocalDiamondSpike','CP5ConfluenceDeletionChainSpike','CP5O19SurfaceSpike','CP5ConfluenceCrossTraceSpike','CP5ConfluenceCanonicalSortSpike','CP5ConfluenceRenamingCompositionSpike']:
    ...
    assert (ROOT/fp).read_bytes()==subprocess.check_output(['git','show','9b532666:'+fp],cwd=ROOT), 'Frozen source changed: '+fp
...
if path == 'research/DGamma/CP5ConfluenceLocalDiamondSpike.idr':
    ...
    assert snapshot == subprocess.check_output(['git','show','9b532666:'+path],cwd=ROOT), 'Changed LocalDiamond requires prior gate'
