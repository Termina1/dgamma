"""Pure R201 source/log/record and immutable validation-plan contracts."""
import hashlib, re
...
def validate_execution_policy(record, policy_bytes):
    """All R201 records begin under the already-effective no-lock owner rule."""
    import json
    policy=json.loads(policy_bytes)
    assert policy['authorization']=='SUPERVISOR RULE CHANGE: CROSS-LANE HEAVY LOCK ABOLISHED'
    assert policy['shift']=='R201' and policy['oneCompilerPerLane'] is True
