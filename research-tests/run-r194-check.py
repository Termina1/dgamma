#!/usr/bin/env python3
"""One serialized, seeded, fresh R194 check. Launch detached with python3 -I.
No TTC is deleted. JSON/log/source snapshots live under /tmp/dgamma-r194.
Usage: run-r194-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]]; PATH=package for build.
"""
import atexit
import datetime
import shutil
...
# R194: no proof/new attempt in final40min; hash-frozen validation may
# start until final25min. Start23:53:53Z, timeout03:53:53Z.
...
cutoff = datetime.datetime(2026,9,9,3,28,53,tzinfo=datetime.timezone.utc) if planned_validation else datetime.datetime(2026,9,9,3,13,53,tzinfo=datetime.timezone.utc)
assert datetime.datetime.now(datetime.timezone.utc) < cutoff, 'R194 attempt03:13:53 / frozen-validation03:28:53 start guard'
...
heavy_paths = set(json.loads((ROOT/'research-tests/O6-R194-HEAVY-PATHS.json').read_text()))
...
if path == 'research/DGamma/CP5ConfluenceLocalDiamondSpike.idr':
    ...
    assert snapshot == subprocess.check_output(['git','show','b81362d8:'+path],cwd=ROOT), 'Changed LocalDiamond requires prior gate'
    rss_limit_kib = 52*1024*1024
lock = pathlib.Path('/tmp/dgamma-heavy.lock')
lock_owner = dict(lane='R194-main',pid=os.getpid(),timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),unit=unit,path=path)
lock_acquired = False
lock_events = []
def release_lock():
