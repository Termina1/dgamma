#!/usr/bin/env python3
"""Copied from L2R7; extra Building lines now reject; 18 GiB light stop (stricter than 48 GiB): detached lane2-only seeded single check.
...
Usage: python3 -I run-l2r8-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]].
"""
...
assert path.startswith('research-tests/O6-L2R8-Sources/') or (unit == 'V0' and path == 'research-tests/O6-L2R7-Sources/DGamma/L2R7ReleaseDecode.idr'), 'Lane-owned targets or exact unchanged bootstrap target only'
...
heavy=False;locked=False;lock_events=[]  # L2R8: light-only; NEVER inspect or mutate shared lock/window.
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R8 task BUILD SEED; target only, dependency rebuilds acknowledged')
