#!/usr/bin/env python3
"""Archive exact R194 records, snapshots, monitors and receipts without Idris.
Usage: python3 -I research-tests/run-r194-archive.py END_COMMIT
...
archive = ROOT / 'research-tests/O6-R194-COMPILER-EVIDENCE.tar.gz'
...
ledger = dict(shift='R194', generatedUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
    ...
    records=normalized)
(ROOT / 'research-tests/O6-R194-COMPILER-LEDGER.json').write_text(json.dumps(ledger, indent=2) + '\n')
