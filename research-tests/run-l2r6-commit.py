...
record = json.loads((pathlib.Path('/tmp/dgamma-l2r6')/(unit+'.json')).read_text())
...
assert json.loads(pathlib.Path('/tmp/dgamma-l2r6/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
with pathlib.Path('/tmp/dgamma-l2r6/commit-receipts.jsonl').open('a') as ledger:
