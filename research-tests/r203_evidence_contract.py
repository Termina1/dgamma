"""Pure R203 source/log/record and immutable validation-plan contracts."""
import hashlib, re
...
def validate_execution_policy(record, policy_bytes):
    """R203 starts with the correct label; no inherited-label relabeling exists."""
    import json
    policy=json.loads(policy_bytes)
    assert policy['authorization']=='SUPERVISOR RULE CHANGE: CROSS-LANE HEAVY LOCK ABOLISHED'
    assert policy['shift']=='R203' and policy['oneCompilerPerLane'] is True
...
def validate_superseded_pass(first, replacement):
    """Only the exact B16 explicit-index cleanup may leave a PASS unreceipted."""
    old=first.decode()
    fixed=old.replace('generationEq source _ premises\n','generationEq _ _ premises\n').replace('generationEq source target sourcePremises\n','generationEq _ _ sourcePremises\n')
    assert first!=replacement and fixed.encode()==replacement
