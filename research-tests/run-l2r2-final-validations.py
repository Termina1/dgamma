...
assert (OUT/'final-validation-plan.json').read_bytes() == (ROOT/'research-tests/O6-L2R2-FINAL-VALIDATION-PLAN.json').read_bytes()
for item in plan:
...
for item in plan:
    with (OUT/(item['unit']+'.wrapper.log')).open('w') as log:
        outcome = subprocess.run([sys.executable, '-I', str(ROOT/'research-tests/run-l2r2-check.py'),
