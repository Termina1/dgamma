...
for i,r in enumerate(records):
    ...
    assert r['heavyLock'] and r['heavyLock'][0]['event']=='continuous-window-owned'
    ...
    cutoff='2026-09-09T07:19:35' if re.fullmatch(r'B\d+',r['unit']) else '2026-09-09T07:04:35'
...
if '--require-B-complete' in sys.argv:assert len(completed)==120 and len(source_receipts)>=4
report=dict(timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),head=git('rev-parse','HEAD').strip(),status='PASS',invocations=len(records),expectedPASS=sum(r['passed'] for r in records),sourceReceipts=len(source_receipts),artifactReceipts=len(receipts)-len(source_receipts),BCompleted=len(completed),BTotal=120,resourceStops=[r['unit'] for r in records if r.get('resourceStopped')],mutations=[r['unit'] for r in records if r['targetMutationDetected']],compilerOverlap=False,sourceReceiptsAuthenticated=True,sourceSnapshotsAuthenticated=True,sampledRSSQualification='One-second own-worktree samples, not OS high-water; zero means no live sample captured',microUnitAttempts={k:len(v) for k,v in attempts.items()},noStagedFiles=not git('diff','--cached','--name-only').strip())
