#!/usr/bin/env python3
"""Compiler-free, separately executable L2R4 evidence/receipt/scope verifier.
...
worktree access occurs. Writes only /tmp/dgamma-l2r4/independent-verification.json.
"""
...
OUT = pathlib.Path('/tmp/dgamma-l2r4')
BASE = '61d262b2'
OWNED = 'research-tests/O6-L2R4-Sources/'
def git(*args):
...
for r in records:
 ...
 assert r['path'].startswith(OWNED)
 ...
 assert not r.get('bundleSources'), 'No L2R4 bundle authority'
...
planGroups = [('final-validation-plan.json','research-tests/O6-L2R4-FINAL-VALIDATION-PLAN.json')]
if (ROOT/'research-tests/O6-L2R4-ADDENDUM-VALIDATION-PLAN.json').exists():
 planGroups.append(('addendum-validation-plan.json','research-tests/O6-L2R4-ADDENDUM-VALIDATION-PLAN.json'))
plans = []
seen_paths = set()
for localPlan, planpath in planGroups:
 ...
 plans += group
# No predecessor source or runner repair is authorized in L2R4.
repairs = {}
paths = git('diff', '--name-only', BASE).decode().splitlines()
allowed = ('research-tests/O6-L2R4-', 'research-tests/run-l2r4-')
...
assert sum(map(len, newdecls.values())) == len(sourcecommits) == 47
for letter, cap, retained in [('A',22,22), ('B',18,14), ('C',12,12)]:
 attempts = {}
 for r in records:
 ...
 for group in attempts.values():
  ...
  if letter == 'B' and group[0]['unit'].startswith('B7-'):
   assert len(group) == 3 and not any(r['passed'] for r in group)
   assert not (ROOT/group[-1]['path']).exists(), 'Stopped B7 source was not fully reverted'
  else:
   assert len(group) <= 3 and group[-1]['passed'] and sum(r['passed'] for r in group) == 1
publication = '-ADDENDUM' if len(planGroups)>1 else ''
manifest = json.loads((ROOT/('research-tests/O6-L2R4'+publication+'-COMPILER-LEDGER.json')).read_text())
archive = ROOT/('research-tests/O6-L2R4'+publication+'-COMPILER-EVIDENCE.tar.gz')
...
if publication:
 oldManifest = json.loads((ROOT/'research-tests/O6-L2R4-COMPILER-LEDGER.json').read_text())
 oldArchive = ROOT/'research-tests/O6-L2R4-COMPILER-EVIDENCE.tar.gz'
