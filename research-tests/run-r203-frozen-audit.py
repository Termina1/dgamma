#!/usr/bin/env python3
"""Read-only R203 exact whole-frozen-source/body/census/seed audit; no compiler."""
...
assert changed, "R203 retained research modules required"
for p in changed:
    assert '%default total' in (ROOT/p).read_text()
    prior=subprocess.run(['git','show',START+':'+p],cwd=ROOT,capture_output=True)
    assert prior.returncode!=0, 'R203 changes only new sources; inherited source edits not authorized'
...
prohibited={p:re.findall(p,code,re.M) for p in [r'\bbelieve_me\b',r'\bassert_total\b',r'\bassert_smaller\b',r'^\s*partial\b',r'\?\w+',r'\bdeletionTheoremProof\b',r'\bpostulate\b',r'\b(?:sortClosingFreeTraceSpike|selectOperationalCanonicalPermutationSpike|canonicalSchedulesConvergeSpike|replayedCanonicalToOriginalEndpointSpike)\b']}
...
assert record['retainedNewDeclarations']==sum(r['event']=='GUARDED COMMIT' and r.get('kind')!='authorized-comment-only' for r in receipts)==32
...
record['policyLabel']='R203 from first invocation; no metadata correction'
record['ordinarySyntaxQualification']='An explicit erased let binds the concrete A5 fixture filter/proof; a local erased structural rightHistory helper is in A13. Neither is an escape hatch or a new capital premise. Inherited R202 stylistic keyword bans are not proof-safety checks.'
