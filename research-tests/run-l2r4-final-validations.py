...
localPlan, committedPlan = ('addendum-validation-plan.json','research-tests/O6-L2R4-ADDENDUM-VALIDATION-PLAN.json') if sys.argv[1:] else ('final-validation-plan.json','research-tests/O6-L2R4-FINAL-VALIDATION-PLAN.json')
...
for item in plan:
    with (OUT/(item['unit']+'.wrapper.log')).open('w') as log:
        outcome = subprocess.run([sys.executable, '-I', str(ROOT/'research-tests/run-l2r4-check.py'),
