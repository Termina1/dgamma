#!/usr/bin/env python3
"""Detached R208 exact-source check; one owned compiler, no shared locks.
Usage: python3 -I research-tests/run-r208-check.py INVOCATION PATH
...
is_validation = unit.startswith('V') or path=='package'
assert utc() < ('2026-09-10T09:30:00' if is_validation else '2026-09-10T08:13:00'), 'R208 proof/validation cutoff'
...
if path!='package':
    ...
    if re.fullmatch(r'(?:S|T|P|W)\d+-\d+',unit):
        ...
        cap_unit=re.fullmatch(r'(S|T|P|W)(\d+)-\d+',unit)
        assert 1<=int(cap_unit[2])<=dict(S=10,T=8,P=26,W=10)[cap_unit[1]], 'Micro-unit cap'
...
(OUT/(unit+'.common.py')).write_bytes((ROOT/'research-tests/r208_common.py').read_bytes())
