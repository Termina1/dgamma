#!/usr/bin/env python3
"""Read-only R200 exact whole-frozen-source/body/census/seed audit; no compiler."""
...
assert changed, "R200 retained research modules required"
for p in changed:
    ...
    if prior.returncode==0:
        ...
        assert p==auth['path'] and sha(prior.stdout)==auth['beforeSHA256'] and sha((ROOT/p).read_bytes())==auth['afterSHA256']
        assert prior.stdout.replace(auth['beforeComment'].encode(),auth['afterComment'].encode())==(ROOT/p).read_bytes(), 'Only authorized comment change'
...
assert record['retainedNewDeclarations']==sum(r['event']=='GUARDED COMMIT' and r.get('kind')!='authorized-comment-only' for r in receipts)==44
record['commentCorrection']=json.loads((OUT/'comment-authorization.json').read_text())
