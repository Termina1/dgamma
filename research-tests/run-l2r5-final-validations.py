...
Only the final L2R5 plan is authorized; no addendum/source cap extension.
...
assert sys.argv[1:] == []
localPlan, committedPlan = 'final-validation-plan.json','research-tests/O6-L2R5-FINAL-VALIDATION-PLAN.json'
...
for item in plan:
    with (OUT/(item['unit']+'.wrapper.log')).open('w') as log:
        outcome = subprocess.run([sys.executable, '-I', str(ROOT/'research-tests/run-l2r5-check.py'),
