...
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=pathlib.Path('/tmp/dgamma-r202');ART=ROOT/'research-tests'
sha=lambda b:hashlib.sha256(b).hexdigest()
archive=ART/'O6-R202-COMPILER-EVIDENCE.tar.gz';assert not archive.exists(), 'Append-only publication; gate replacement'
...
plan=json.loads((OUT/'final-validation-plan.json').read_text());scope=json.loads((ART/'O6-R202-VALIDATION-SCOPE.json').read_text())
...
for path in new:
    ...
    entries.append(dict(path=path,module='DGamma.'+pathlib.Path(path).stem,estimatedSeconds=None,estimatedSampleRSSKiB=None,estimateBasis='New R202 target',heavyLock=False,expectedDiagnostic=None,symbol=None))
measured=0
for item in entries:
    ...
    item['r202Checks']=[dict(unit=r['unit'],seconds=r['seconds'],maxSampleRSSKiB=r['maxSampleRSSKiB'],rssLimitKiB=r['rssLimitKiB'],expectedDiagnostic=r['expectedDiagnostic'],startUTC=r['start'],endUTC=r['end'],executionPolicySHA256=r['executionPolicySHA256']) for r in own]
    if own:
        measured+=1;item['r202Status']='Current-source direct checked expected outcome';item['estimatedSeconds']=max(r['seconds'] for r in own);item['estimatedSampleRSSKiB']=max(r['maxSampleRSSKiB'] for r in own);item['estimateBasis']='R202 current-source direct one-second samples; not OS high-water; zero means no live sample'
    else:item['r202Status']='NOT rechecked in R202; inherited excluded stale/unclassified/legacy TTC is not fresh PASS'
    item['r202PeakMeasurementQuality']='one-second sample, NOT OS high-water' if own and item['estimatedSampleRSSKiB'] else 'NO R202 LIVE PEAK MEASUREMENT'
    item['ttcPresentAtR202Publication']=(ROOT/'build/ttc/2025081600'/pathlib.Path(item['module'].replace('.','/')+'.ttc')).exists()
...
cost.update(status='R202 complete measured refresh; seeded, NOT cold',preparedUTC=now,sourceFreezeHead=scope['sourceFreezeHead'],publicationAnchorCommit=anchor,inheritedR201InventoryCount=280,currentCount=285,r202MeasuredInventoryEntries=174,r202DirectSourceTargets=176,r202AuxiliaryInheritedTargetsOutsideInventory=auxiliary,r202UnvalidatedInventoryEntries=111,r202NewPaths=new,qualification='280 inherited inventory+5 new=285;174 directly checked inventory entries+2 unchanged inherited main baseline variants outside inventory=176 source targets. ALL171 inherited applicable paths+5 new+seeded package checked.111 excluded inventory paths remain NOT rechecked. Earlier round fields remain historical. No lane2 worktree or cold certification.')
(ART/'O6-R202-ROOT-CONTRACT-COSTS.json').write_text(json.dumps(cost,indent=2)+'\n')
...
(ART/'O6-R202-COMPILER-LEDGER.json').write_text(json.dumps(dict(anchorCommit=anchor,preparedUTC=now,executionPolicy=policy,records=[{k:r[k] for k in keys} for r in records]),indent=2)+'\n')
rows=['# R202 per-module direct measurements','','One-second own-worktree samples, NOT OS high-water. Zero means no captured live sample.207/211 expected outcomes; B2-1, A8-1, A13-1 and A16-1 remain rejected snapshots. Seven final fixtures are exact expected negatives, not positive typechecks.','','| Invocation | Target | Seconds | Sample KiB | Limit KiB | Outcome |','|---|---|---:|---:|---:|---|']
for r in records:
...
rows+=['','Metadata-only policy-label history: `O6-R202-POLICY-LABEL-CORRECTION.json`, with old policy bytes retained. Every source receipt is inside the raw archive; artifact publication and final-gate receipts deliberately follow its anchor.']
(ART/'O6-R202-MICRO-UNITS.md').write_text('\n'.join(rows)+'\n')
...
with tarfile.open(archive,'w:gz') as tar:
    for p in files+[OUT/'archive-anchor.json']:tar.add(p,arcname='dgamma-r202/'+str(p.relative_to(OUT)))
