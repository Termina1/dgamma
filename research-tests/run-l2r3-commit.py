#!/usr/bin/env python3
"""Commit one declaration after fresh PASS; authorized C4 two-file bundles are the exception.
Usage: python3 -I research-tests/run-l2r3-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-l2r3')/(unit+'.json')).read_text())
...
assert json.loads(pathlib.Path('/tmp/dgamma-l2r3/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
with pathlib.Path('/tmp/dgamma-l2r3/commit-receipts.jsonl').open('a') as ledger:
