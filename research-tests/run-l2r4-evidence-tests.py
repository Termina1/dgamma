...
class EvidenceTests(unittest.TestCase):
 def setUp(self):
  self.declarations = helper('research-tests/run-l2r4-check.py', 'declarations', {'re': re})
  self.building = helper('research-tests/run-l2r4-independent-verify.py', 'building', {'re': re, 'pathlib': pathlib, 'ROOT': ROOT})
 ...
 def test_own_building_line_required(self):
  path = 'research-tests/O6-L2R4-Sources/DGamma/L2R4RootPhase.idr'
  self.assertTrue(self.building({'transcript': '1/1: Building DGamma.L2R4RootPhase ('+str(ROOT/path)+')\n'}, path))
 ...
 def test_dependency_or_wrong_worktree_not_freshness(self):
  path = 'research-tests/O6-L2R4-Sources/DGamma/L2R4RootPhase.idr'
  self.assertFalse(self.building({'transcript': '1/1: Building DGamma.L2R4SmallStates ('+str(ROOT/'research-tests/O6-L2R4-Sources/DGamma/L2R4SmallStates.idr')+')\n'}, path))
  self.assertFalse(self.building({'transcript': '1/1: Building DGamma.L2R4RootPhase (/Users/vyacheslavshebanov/Work/dgamma/'+path+')\n'}, path))
 def test_classifier_does_not_own_main_or_wrapper(self):
  ...
  rows = '\n'.join([
   '101 1 500 '+engine+' --source-dir '+str(ROOT/'src')+' --check '+str(ROOT/'research-tests/O6-L2R4-Sources/DGamma/L2R4RootPhase.idr'),
  ...
  scan = helper('research-tests/run-l2r4-check.py', 'compiler_processes', {'pathlib': pathlib, 'subprocess': fake, 'ROOT': ROOT})
 ...
 def assert_expression(self, text, scope):
  tree = ast.parse((ROOT/'research-tests/run-l2r4-check.py').read_text())
 ...
 def test_target_guard_rejects_missing_empty_and_outside(self):
  with tempfile.TemporaryDirectory(prefix='dgamma-l2r4-test-') as folder:
 ...
 def test_actual_caps_no_self_extension(self):
  for unit, expected in [('A22-1',True),('A23-1',False),('B18-1',True),('B19-1',False),('C12-1',True),('C13-1',False)]:
 ...
 def test_all_runner_files_parse_without_execution(self):
  for path in sorted((ROOT/'research-tests').glob('run-l2r4-*.py')):
