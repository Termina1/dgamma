#!/usr/bin/env python3
"""Copied from L2R9 for L2R10; extra Building lines now reject; 18 GiB light stop (stricter than 48 GiB): detached lane2-only seeded single check.
...
Usage: python3 -I run-l2r10-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]].
"""
...
if re.fullmatch(r'[ABCD]\d+-\d+',unit):
 ...
 assert int(unit.split('-')[0][1:]) <= {'A':14,'B':16,'C':10,'D':10}[unit[0]]
...
heavy=False;locked=False;lock_events=[]  # L2R10: light-only; NEVER inspect or mutate shared lock/window.
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R10 task BUILD SEED; target only, dependency rebuilds acknowledged')
