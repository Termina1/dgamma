#!/usr/bin/env python3
"""One serialized, seeded, fresh R198 check. Launch detached with python3 -I.
No TTC is deleted. JSON/log/source snapshots live under /tmp/dgamma-r198.
Usage: run-r198-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]]; PATH=package for build.
"""
import atexit
import datetime
import shutil
...
OUT = pathlib.Path('/tmp/dgamma-r198')
...
# Conservative shift start08:49Z; timeout12:49Z; new proof12:09Z; validation12:24Z.
...
cutoff = datetime.datetime(2026,9,9,12,24,0,tzinfo=datetime.timezone.utc) if planned_validation else datetime.datetime(2026,9,9,12,9,0,tzinfo=datetime.timezone.utc)
assert datetime.datetime.now(datetime.timezone.utc) < cutoff, 'R198 attempt12:09:00 / frozen-validation12:24:00 start guard'
...
if path == 'package':
    ...
    if any(word in path for word in ['ActorLifecycleOnlyExtended','CP5AvailabilityAware']):
        assert planned_validation and snapshot == subprocess.check_output(['git','show','2c9c2e19:'+path],cwd=ROOT)
    target.touch()
...
for frozen in ['CP5ConfluenceLocalDiamondSpike','CP5ConfluenceDeletionChainSpike','CP5O19SurfaceSpike','CP5ConfluenceCrossTraceSpike','CP5ConfluenceCanonicalSortSpike','CP5ConfluenceRenamingCompositionSpike']:
    ...
    assert (ROOT/fp).read_bytes()==subprocess.check_output(['git','show','2c9c2e19:'+fp],cwd=ROOT), 'Frozen source changed: '+fp
...
if path == 'research/DGamma/CP5ConfluenceLocalDiamondSpike.idr':
    ...
    assert snapshot == subprocess.check_output(['git','show','2c9c2e19:'+path],cwd=ROOT), 'Changed LocalDiamond requires prior gate'
...
lock_owner = dict(lane='R198-main',pid=os.getpid(),timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),unit=unit,path=path)
lock_acquired = False
lock_events = []
def release_lock():
