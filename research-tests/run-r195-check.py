#!/usr/bin/env python3
"""One serialized, seeded, fresh R195 check. Launch detached with python3 -I.
No TTC is deleted. JSON/log/source snapshots live under /tmp/dgamma-r195.
Usage: run-r195-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]]; PATH=package for build.
"""
import atexit
import datetime
import shutil
...
# R195: no proof/new attempt in final40min; hash-frozen validation may
# start until final25min. Start02:02:28Z, timeout06:02:28Z.
...
cutoff = datetime.datetime(2026,9,9,5,37,28,tzinfo=datetime.timezone.utc) if planned_validation else datetime.datetime(2026,9,9,5,22,28,tzinfo=datetime.timezone.utc)
assert datetime.datetime.now(datetime.timezone.utc) < cutoff, 'R195 attempt05:22:28 / frozen-validation05:37:28 start guard'
...
if path == 'package':
    ...
    if any(word in path for word in ['ActorLifecycleOnlyExtended','CP5AvailabilityAware']):
        assert planned_validation and snapshot == subprocess.check_output(['git','show','981e6137:'+path],cwd=ROOT)
    target.touch()
...
if path == 'research/DGamma/CP5ConfluenceLocalDiamondSpike.idr':
    ...
    assert snapshot == subprocess.check_output(['git','show','981e6137:'+path],cwd=ROOT), 'Changed LocalDiamond requires prior gate'
    rss_limit_kib = 52*1024*1024
lock = pathlib.Path('/tmp/dgamma-heavy.lock')
lock_owner = dict(lane='R195-main',pid=os.getpid(),timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),unit=unit,path=path)
lock_acquired = False
lock_events = []
def release_lock():
...
record = dict(unit=unit,path=path,command=command,start=started,
              ...
              targetMutationDetected=mutation_detected, rssSamples=samples, maxSampleRSSKiB=maximum,rssLimitKiB=rss_limit_kib,validationContinuationSHA256=continuation_sha,
