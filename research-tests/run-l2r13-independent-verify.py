#!/usr/bin/env python3
"""Independent, compiler-free L2R13 evidence validator. Imports NO check/commit
...
PREFIX='research-tests/O6-L2R13-'
RAW=Path('/tmp/dgamma-l2r13')
BASE='b9130fbb'
DOCUMENTARY_PATHS={'research-tests/O6-L2R12-DECLARATION-ORIGINS.md','research-tests/O6-L2R12-MICRO-UNIT-LEDGER.json','research-tests/run-l2r12-publish-evidence.py'}
CAPS={'A':14,'B':14,'C':10}
...
def visibility_validation_valid(prior, validation, before, after, authority):
 assert prior['unit']=='B10-2' and validation['unit']=='V0'
 assert authority['validation']=='V0' and authority['pendingAttempt']=='B14-2'
 assert prior['path']==validation['path']==authority['path']==PREFIX+'Sources/DGamma/L2R13TerminalMove.idr'
...
def documentary_repair_valid(path,before,after,authority,phase):
 assert path in DOCUMENTARY_PATHS and phase in {1,2}
 assert sha(before)==authority['beforeSHA256'] and sha(after)==authority['afterSHA256']
 old='checked type; inhabitance at exact declared scope'
 negative='checked TYPE declaration; NOT inhabited'
 neutral='checked TYPE declaration; inhabitance reported separately'
 if phase==1:
  assert old.encode() in before and before.replace(old.encode(),negative.encode())==after
 elif path.endswith('DECLARATION-ORIGINS.md'):
  rows=before.decode().splitlines(keepends=True)
  expected=''.join(row.replace(negative,neutral) if negative in row and 'ForcedRootPhaseFromObservedAgreement' not in row else row for row in rows)
  assert expected.encode()==after and before!=after
 elif path.endswith('MICRO-UNIT-LEDGER.json'):
  expected=json.loads(before);changed=0
  for row in expected['sourceUnits']:
   if row['kind']=='type' and row['name']!='ForcedRootPhaseFromObservedAgreement':
    assert row['status']==negative;row['status']=neutral;changed+=1
  assert changed==5 and json.loads(after)==expected
 else:
  oldline="status='checked TYPE declaration; NOT inhabited' if kind=='type' else ('proved at exact documented scope' if kind=='proof' else 'checked total executable definition')"
  newline="status=('checked TYPE declaration; NOT inhabited' if name=='ForcedRootPhaseFromObservedAgreement' else 'checked TYPE declaration; inhabitance reported separately') if kind=='type' else ('proved at exact documented scope' if kind=='proof' else 'checked total executable definition')"
  assert before.count(oldline.encode())==1 and before.replace(oldline.encode(),newline.encode())==after
 return True
...
def verify(use_archive=True):
 ...
 for r in records:
  ...
  if match:
   ...
   if unit=='V0':
    ...
    visibility_validation_valid(byid['B10-2'],r,stored['B10-2.source'],data,authority)
    assert retained[r['path']]==stored['B10-2.source']
 ...
 for receipt in receipts:
  ...
  for repair in receipt.get('authorizedCommentRepairs',[]):
   phase=1 if 'replacements' not in repair else 2
   authority=json.loads(stored['DOCUMENTARY-RULING.json' if phase==1 else 'DOCUMENTARY-SECOND-RULING.json'])[repair['path']]
   assert repair['beforeSHA256']==authority['beforeSHA256'] and repair['afterSHA256']==authority['afterSHA256']
   documentary_repair_valid(repair['path'],git('show',commit+'^:'+repair['path']),git('show',commit+':'+repair['path']),authority,phase)
 ...
 for path in DOCUMENTARY_PATHS:
  first=json.loads(stored['DOCUMENTARY-RULING.json'])[path];second=json.loads(stored['DOCUMENTARY-SECOND-RULING.json'])[path]
  assert sha(git('show',BASE+':'+path))==first['beforeSHA256']
  assert first['afterSHA256']==second['beforeSHA256'] and sha((ROOT/path).read_bytes())==second['afterSHA256']
 ...
 assert all(p.startswith((PREFIX,'research-tests/run-l2r13-')) or p in DOCUMENTARY_PATHS for p in changed+untracked)
 ...
 for row,receipt in zip(micro['sourceUnits'],successful):
  ...
  if row['kind']=='type':assert row['status']=='checked TYPE declaration; inhabitance reported separately'
 ...
 assert tests['passed'] and tests['count']==51
 assert tests['verifierSHA256']==sha(Path(__file__).read_bytes())
 assert tests['testFileSHA256']==sha((ROOT/'research-tests/run-l2r13-evidence-tests.py').read_bytes())
 assert tests['logSHA256']==sha(stored[tests['logFile']]) and b'Ran 51 tests' in stored[tests['logFile']] and stored[tests['logFile']].rstrip().endswith(b'OK')
