#!/usr/bin/env python3
"""Detached continuous R196 rebuild lock owner; no compiler or worktree writes."""
...
owner=dict(lane='R196-main',pid=os.getpid(),timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),unit='A1-through-B-package',path='/Users/vyacheslavshebanov/Work/dgamma/')
...
def event(kind,**kwargs):
    ...
    with (OUT/'window-events.jsonl').open('a') as f: f.write(json.dumps(entry)+'\n')
...
window_data=dict(lane='R196-main',pid=os.getpid(),start=owner['timestampUTC'],expectedEnd='2026-09-09T07:19:35Z',note='LocalDiamond contract rebuild; lane 2 must not run heavy checks')
window.write_text(json.dumps(window_data,indent=2)+'\n')
(OUT/'window-owner.json').write_text(owner_text)
event('acquired',owner=owner,window=window_data)
(OUT/'window-ready').write_text(str(os.getpid()))
while not stopping and not (OUT/'window-stop').exists():
