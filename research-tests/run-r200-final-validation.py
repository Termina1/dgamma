...
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=pathlib.Path('/tmp/dgamma-r200')
...
assert data==(ROOT/'research-tests/O6-R200-FINAL-VALIDATION-PLAN.json').read_bytes()
scope=json.loads((ROOT/'research-tests/O6-R200-VALIDATION-SCOPE.json').read_text())
assert len(plan)==scope['checksIncludingPackage'] and len({x['path'] for x in plan})==len(plan)
assert scope['inheritedSources']==157 and not scope['excludedInheritedApplicablePaths']
...
assert sha((ROOT/'research-tests/run-r200-check.py').read_bytes())==policy['runnerSHA256']
...
for item in plan[len(completed):]:
    assert sha((ROOT/'research-tests/run-r200-check.py').read_bytes())==launch['runnerSHA256'], 'Runner mutated'
    ...
    command=[sys.executable,'-I',str(ROOT/'research-tests/run-r200-check.py'),item['unit'],item['path']]
