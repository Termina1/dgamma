#!/usr/bin/env python3
"""Compiler-free append-only freeze of all 143 inherited main sources + changes + package."""
...
old=json.loads((ART/'O6-R197-COMPILER-LEDGER.json').read_text())['records'];items={}
for r in old:
    if r['passed'] and r['path']!='package':items[r['path']]=dict(path=r['path'],expectedDiagnostic=r['expectedDiagnostic'],symbol=r['symbol'],origin='inherited R197 import-closed main target')
assert len(items)==143, '136 pre-R197 + 7 R197 sources'
excluded=[]
...
for path in changed:items[path]=dict(path=path,expectedDiagnostic=None,symbol=None,origin='R198 changed/new source')
...
data=(json.dumps(plan,indent=2)+'\n').encode();(OUT/'final-validation-plan.json').write_bytes(data);(OUT/'final-validation-plan.sha256').write_text(sha(data)+'\n');(ART/'O6-R198-FINAL-VALIDATION-PLAN.json').write_bytes(data)
report=dict(status='FROZEN; execution pending',preparedUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),sourceFreezeHead=git('rev-parse','HEAD').strip(),baseline=START,planSHA256=sha(data),inheritedSources=len(inherited),newChangedSources=len(changed),sourceTargets=len(items),checksIncludingPackage=len(plan),inheritedPaths=inherited,newPaths=changed,importClosedWithinInheritedInvalidationInventory=True,invalidationInventoryCount=252,allPlannedImportsTopologicallySorted=True,excludedInheritedApplicablePaths=excluded,seededDependencyQualification='Unchanged prerequisites outside the inherited252-source invalidation inventory/source plan are source-pinned reused seeds, not fresh-PASS claims.',qualification='ALL143 inherited R197 main sources (136 +7), except only explicitly absent paths, plus every R198 source and seeded package. Expected negatives retain exact diagnostic AND symbol. No lane2 worktree or cold build.')
(ART/'O6-R198-VALIDATION-SCOPE.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({k:v for k,v in report.items() if k not in ['inheritedPaths','newPaths']},indent=2))
