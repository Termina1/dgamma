#!/usr/bin/env python3
"""Read-only R202 exact whole-frozen-source/body/census/seed audit; no compiler."""
...
assert changed, "R202 retained research modules required"
for p in changed:
    assert '%default total' in (ROOT/p).read_text()
    prior=subprocess.run(['git','show',START+':'+p],cwd=ROOT,capture_output=True)
    assert prior.returncode!=0, 'R202 changes only new sources; inherited source edits not authorized'
...
assert record['retainedNewDeclarations']==sum(r['event']=='GUARDED COMMIT' and r.get('kind')!='authorized-comment-only' for r in receipts)==30
...
record['policyLabelCorrection']=json.loads((OUT/'policy-label-correction.json').read_text())
