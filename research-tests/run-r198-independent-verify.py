...
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=pathlib.Path('/tmp/dgamma-r198');ART=ROOT/'research-tests';START='2c9c2e19'
sys.dont_write_bytecode=True
spec=importlib.util.spec_from_file_location('contract',ART/'r198_evidence_contract.py');c=importlib.util.module_from_spec(spec);spec.loader.exec_module(c)
...
for i,r in enumerate(records):
    ...
    assert r['heavyLock'] and r['heavyLock'][0]['event']=='acquired' and r['heavyLock'][0]['lane']=='R198-main'
    assert r['rssLimitKiB']==(52 if r['path'].endswith('CP5ConfluenceLocalDiamondSpike.idr') else 48)*1024*1024
    assert r['start']<('2026-09-09T12:24:00' if re.fullmatch(r'V\d+',r['unit']) else '2026-09-09T12:09:00')
...
assert len([u for u in attempts if u.startswith('A')])==26 and len([u for u in attempts if u.startswith('B')])==16
assert sum(len(v) for u,v in attempts.items() if u.startswith('A'))==33
assert sum(len(v) for u,v in attempts.items() if u.startswith('B'))==16
for unit,runs in attempts.items():
    assert [int(r['unit'].rsplit('-',1)[1]) for r in runs]==list(range(1,len(runs)+1)) and len(runs)<=3
    if unit=='A3':assert len(runs)==3 and not any(r['passed'] for r in runs)
    elif unit=='A20':
        assert len(runs)==2 and all(r['passed'] for r in runs)
        assert not any(x['invocation']=='A20-1' for x in receipts), 'Guard-rejected whitespace check was not committed'
        assert (OUT/'A20-whitespace-recheck.json').is_file()
        assert (OUT/'A20-1.source').read_bytes().rstrip()==(OUT/'A20-2.source').read_bytes().rstrip(), 'Only EOF whitespace repair authorized'
        assert any(x['invocation']=='A20-2' for x in source_receipts)
    else:
...
revert=json.loads((OUT/'A3-stop-revert.json').read_text())
assert revert['attempts']==['A3-1','A3-2','A3-3'] and revert['status'].startswith('STOP3/3')
assert sha((ROOT/'research-tests/DGamma/R198RepeatedTagStageProbe.idr').read_bytes())==revert['restoredSHA256']
assert not git('diff',START,'--','research/DGamma/CP5ProviderHeadObservedSpike.idr'), 'R197 D5 remains exhausted/frozen'
plan_bytes=(OUT/'final-validation-plan.json').read_bytes();plan=json.loads(plan_bytes);scope=json.loads((ART/'O6-R198-VALIDATION-SCOPE.json').read_text())
assert sha(plan_bytes)==(OUT/'final-validation-plan.sha256').read_text().strip()==scope['planSHA256']
assert plan_bytes==(ART/'O6-R198-FINAL-VALIDATION-PLAN.json').read_bytes()
inventory=json.loads((ART/'O6-R197-ROOT-CONTRACT-COSTS.json').read_text())['entries'];assert len(inventory)==252
...
old=json.loads((ART/'O6-R197-COMPILER-LEDGER.json').read_text())['records'];inherited={r['path'] for r in old if r['passed'] and r['path']!='package'}
assert len(inherited)==143 and all((ROOT/p).is_file() for p in inherited)
assert not scope['excludedInheritedApplicablePaths']
assert {x['path'] for x in plan}==inherited|set(changed)|{'package'} and len(plan)==151
completed=[]
for item in plan:
...
if '--require-final-complete' in sys.argv:
    assert len(completed)==151 and len(records)==202 and sum(r['passed'] for r in records)==195
...
D_docs=[git('show','-s','--format=%s',r['resultingCommitHash']).strip() for r in receipts if r['event']=='GUARDED ARTIFACT COMMIT' and re.match(r'R198 D[1-4]:',git('show','-s','--format=%s',r['resultingCommitHash']))]
assert len(D_docs)<=4 and len({x.split(':',1)[0] for x in D_docs})==len(D_docs)
report=dict(status='PASS',timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),head=git('rev-parse','HEAD').strip(),invocations=len(records),expectedPASS=sum(r['passed'] for r in records),rejected=[r['unit'] for r in records if not r['passed']],sourceReceipts=len(source_receipts),artifactReceipts=len(receipts)-len(source_receipts),finalCompleted=len(completed),finalTotal=151,finalPlanSHA256=sha(plan_bytes),inheritedApplicableSources=143,newSources=7,sourceSnapshotsAuthenticated=True,sourceReceiptsAuthenticated=True,allSourceCommitsReceipted=True,oneDeclarationPerRetainedCommit=True,allPriorCommittedCodeLinesRetained=True,DDocumentationCommits=D_docs,allChangedSourcesChecked=True,allPlannedTargetImportsTopological=True,compilerOverlap=False,resourceStops=[r['unit'] for r in records if r['resourceStopped']],mutations=[r['unit'] for r in records if r['targetMutationDetected']],heavyLockAcquisitionAndReleaseAuthenticated=True,microUnitAttempts={u:len(v) for u,v in attempts.items()},AUnits=26,AInvocations=33,ARetained=25,BUnits=16,BInvocations=16,BRetained=16,exhaustedAndReverted=['A3'],inheritedExhaustedUnchanged=['R197 D5'],CProofAttempts=0,DProofAttempts=0,noStagedFiles=not git('diff','--cached','--name-only').strip(),qualification='Read-only machine authentication, NOT independent human proof review. Seeded direct targets, not cold. RSS sample0 means no live capture, not zero peak. A20 includes a real compiler PASS rejected by whitespace commit guard, followed only by a fresh whitespace recheck. No synchronization/all-name producer or convergence closure is claimed.')
