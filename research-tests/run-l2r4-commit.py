...
Usage: python3 -I research-tests/run-l2r4-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-l2r4')/(unit+'.json')).read_text())
...
assert json.loads(pathlib.Path('/tmp/dgamma-l2r4/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
assert record['path'].startswith(('research-tests/O6-L2R4-Sources/',)), 'Lane-owned source only'
source = ROOT/record['path']
commitPaths=[record['path']]
assert not record.get('bundleSources'), 'No L2R4 two-file bundle gate exists; predecessor C4 exception is not reusable'
...
with pathlib.Path('/tmp/dgamma-l2r4/commit-receipts.jsonl').open('a') as ledger:
