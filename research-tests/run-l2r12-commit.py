...
Usage: python3 -I research-tests/run-l2r12-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-l2r12')/(unit+'.json')).read_text())
...
assert record['path'].startswith(('research-tests/O6-L2R12-Sources/',)), 'Lane-owned source only'
