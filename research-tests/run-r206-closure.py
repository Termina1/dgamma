...
# A previous dependent PASS precedes its parent's latest proof change? Invalidate it.
latest={r['path']:r for r in records()}
for p in closure:
    if p in nodes and p not in roots and p in latest:
        if any(root in latest and latest[p]['end']<latest[root]['end'] for root in roots): status[p]='invalidated dependent'
...
for index,p in enumerate(ordered):
    ...
    if p in roots and status[p]=='passed': outcome='fresh root already checked'
    elif bad: outcome='blocked';status[p]='blocked'
    else:
        ...
        with log.open('w') as f:
            subprocess.run(['python3','-I',str(ROOT/'research-tests/run-r206-check.py'),unit,p],cwd=ROOT,stdout=f,stderr=subprocess.STDOUT)
        rp=OUT/(unit+'.json')
