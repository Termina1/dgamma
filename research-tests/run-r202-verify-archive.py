...
ROOT=pathlib.Path(__file__).resolve().parents[1];ART=ROOT/'research-tests';OUT=pathlib.Path('/tmp/dgamma-r202')
...
archive=ART/'O6-R202-COMPILER-EVIDENCE.tar.gz';metadata=json.loads((ART/'O6-R202-EVIDENCE-VERIFICATION.json').read_text())
...
with tarfile.open(archive,'r:gz') as tar:
    ...
    assert all(n.startswith('dgamma-r202/') and '..' not in n.split('/') for n in names)
    def data(path):
        member=tar.extractfile('dgamma-r202/'+path);assert member is not None
        return member.read()
    ...
    assert set(names)=={'dgamma-r202/'+p for p in anchor['filesSHA256']}|{'dgamma-r202/archive-anchor.json'}
    ...
    assert data('final-validation-plan.json')==(ART/'O6-R202-FINAL-VALIDATION-PLAN.json').read_bytes()
    for item in plan:
    ...
    assert data('execution-policy-inherited-label.json')==(ART/'O6-R202-EXECUTION-POLICY-INHERITED-LABEL.json').read_bytes()
