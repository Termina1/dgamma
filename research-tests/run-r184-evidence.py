#!/usr/bin/env python3
"""Persist R184 exact per-attempt evidence and verify serialized intervals."""
import datetime
import hashlib
import json
import pathlib
...
OUT = pathlib.Path('/tmp/dgamma-r184')
...
for r in records:
    ...
    if r['path'] != 'package':
        ...
        if r['path'] not in accepted_names:
            baseline = subprocess.run(['git','show','9ecf780:'+r['path']],cwd=ROOT,stdout=subprocess.PIPE,stderr=subprocess.DEVNULL)
            ...
            if base_names:
                r['baselineDeclarationSource'] = '9ecf780:'+r['path']
    ...
    # baseline, not to a nonexistent change commit inside 9ecf780..HEAD.
    baseline_commit = subprocess.check_output(['git','rev-parse','9ecf780'],cwd=ROOT,text=True).strip()
    tracked_path = 'dgamma.ipkg' if r['path'] == 'package' else r['path']
    history = [baseline_commit] + subprocess.check_output(['git','log','--reverse','--format=%H','9ecf780..HEAD','--',tracked_path],cwd=ROOT,text=True).splitlines()
...
proof_commits = []
commits = subprocess.check_output(['git','log','--reverse','--format=%H','9ecf780..HEAD','--',
...
assert sum(k.startswith('B') for k in attempts)<=12
assert sum(k.startswith('C') for k in attempts)<=8
assert not (ROOT/'research/DGamma/CP5O19InsertOpeningSpike.idr').exists(), 'exhausted A2 retained'
assert 'o19TargetFromResolution' not in (ROOT/'research/DGamma/CP5O19InsertObservationSpike.idr').read_text(), 'exhausted A6 retained'
actual_domain = (ROOT/'research/DGamma/CP5O19ActualCommutedDomainSpike.idr').read_text()
assert 'o19PairEarlyEffectRunObserved' not in actual_domain, 'exhausted R183 A25 retained'
(ROOT/'research-tests/O6-R184-COMPILER-LEDGER.json').write_text(json.dumps(records,indent=2)+'\n')
with tarfile.open(ROOT/'research-tests/O6-R184-COMPILER-EVIDENCE.tar.gz','w:gz') as archive:
    for r in records:
        for suffix in ['.source','.log','.json','.runner']:
            path = OUT/(r['unit']+suffix)
            if path.exists():
                archive.add(path,arcname=path.name)
    for name in ['run-regressions.py','regressions.runner','regression-results.json','A-stop-frozen.json','A-final-frozen.json','final-frozen.json']:
        path=OUT/name
        if path.exists():
...
summary = dict(checks=len(records),ordinaryPasses=sum(r['passed'] and not r['expectedDiagnostic'] for r in records),
    ...
    rejectedSourcesNotCommitted=True,exhaustedR183A25ConsumerAbsent=True,exhaustedA2AndA6Absent=True,
    retainedNewDeclarations=sum(len(r.get('newTopLevelDeclarations',[])) for r in records if r['passed'] and not r['expectedDiagnostic']),
    ledgerSHA256=hashlib.sha256((ROOT/'research-tests/O6-R184-COMPILER-LEDGER.json').read_bytes()).hexdigest(),
    archiveSHA256=hashlib.sha256((ROOT/'research-tests/O6-R184-COMPILER-EVIDENCE.tar.gz').read_bytes()).hexdigest(),
