...
Usage: python3 -I research-tests/run-l2r4-archive.py L2R4 61d262b2 FINAL_HEAD [ADDENDUM]
...
assert shift=='L2R4' and start=='61d262b2'
...
for r in records:
    ...
    assert r['path'].startswith('research-tests/O6-L2R4-Sources/'), 'Lane-owned targets only'
    target = r['path']
...
for unit, attempts in groups.items():
    ...
    if successes:
        ...
        assert unit=='B7' and len(attempts)==3 and not (ROOT/last['path']).exists()
        names=['retireProviderHead (REVERTED, NOT retained)']
...
(ROOT/'research-tests/O6-L2R4-MICRO-UNIT-LEDGER.json').write_text(json.dumps(dict(base=start,sourceBoundary=end,caps={'A':22,'B':18,'C':12},attemptedSlots={'A':22,'B':14,'C':12},retainedDeclarations=47,unusedSlots={'B':[15,16,17,18]},unusedAuthority='Supervisor-ratified lifecycle structural park; no relabeling delete square as Retire square',records=micro),indent=2)+'\n')
