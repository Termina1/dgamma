...
Usage: python3 -I research-tests/run-r193-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-r193')/(unit+'.json')).read_text())
assert record['passed'] and record['fresh'] and not record['interrupted']
assert record['exit'] == 0 and not record['expectedDiagnostic']
assert json.loads(pathlib.Path('/tmp/dgamma-r193/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
assert not subprocess.check_output(['git','diff','--cached','--name-only'],cwd=ROOT,text=True).strip()
receipt = dict(event='GUARDED COMMIT', unit=unit.rsplit('-',1)[0], attempt=unit.rsplit('-',1)[1], invocation=unit, sourceHash=record['sourceSHA256'], resultingCommitHash=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(), timestampUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(), guardChecksPassed=['fresh PASS','exit 0','not interrupted','no expected diagnostic','exact source SHA256','one new declaration','no intervening compiler invocation','no pre-staged files','no own-worktree compiler (lane2 excluded)','git diff --check','git commit success','no post-staged files'])
with pathlib.Path('/tmp/dgamma-r193/commit-receipts.jsonl').open('a') as ledger:
