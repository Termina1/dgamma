...
def verify(archive=False):
 ...
 for group,rs in groups.items():
  assert [int(r['unit'].rsplit('-',1)[1]) for r in rs]==list(range(1,len(rs)+1))
 ...
 assert {r['invocation'] for r in successful}=={r['unit'] for rs in groups.values() for r in rs if r['passed']}
