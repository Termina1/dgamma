#!/usr/bin/env python3
"""Read-only R182 source/cache/census gate; never compiles or deletes seeds."""
import datetime
import hashlib
import json
import pathlib
...
cross = text(PATHS['CrossTrace'])
reverted_cross = cross.replace('import DGamma.CP5RankedEarlyApplicabilitySpike\n', '')
...
for part, name in [('CanonicalSort','sortClosingFreeTraceSpike'),('CrossTrace','operationalAdjacentBlockSwapSpike'),('CrossTrace','selectOperationalCanonicalPermutationSpike'),('CrossTrace','canonicalSchedulesConvergeSpike'),('RenamingComposition','replayedCanonicalToOriginalEndpointSpike')]:
    ...
    if name == 'operationalAdjacentBlockSwapSpike':
        assert new_decl.replace('(applicableSafety :', '(safety :').replace('sourcePremises applicableSafety', 'sourcePremises safety') == old_decl
    else:
...
out = pathlib.Path(sys.argv[1] if len(sys.argv)>1 else '/tmp/dgamma-r182/frozen.json')
