...
0 extendAdmittedMoveByRoot : {name, key, world, error : Type} -> {value : key -> Type} ->
  ...
  (0 forced : ForcedOnTrace nameEq keyEq (appendAvailability oldTrail (AvailabilityStep oldFinal (Fired {before = oldFinal} {afterState = oldSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag oldRoot) NoTransitions (AvailabilityEnd oldSuccessor))) (S (length (prefixWord move)))) ->
  (target, untouched : Nat) -> (0 bounded : LTE target (length (prefixWord move))) ->
  (0 oldFrame : totalDistance nameEq keyEq (appendAvailability oldTrail (AvailabilityStep oldFinal (Fired {before = oldFinal} {afterState = oldSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag oldRoot) NoTransitions (AvailabilityEnd oldSuccessor))) = minus (S (length (prefixWord move))) target + untouched) ->
  (0 newFrame : totalDistance nameEq keyEq (appendAvailability newTrail (AvailabilityStep newFinal (Fired {before = newFinal} {afterState = newSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag newRoot) NoTransitions (AvailabilityEnd newSuccessor))) = minus (length (prefixWord move)) target + untouched) ->
  AdmittedDistanceMove name key world error value nameEq keyEq (appendAvailability oldTrail (AvailabilityStep oldFinal (Fired {before = oldFinal} {afterState = oldSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag oldRoot) NoTransitions (AvailabilityEnd oldSuccessor))) (appendAvailability newTrail (AvailabilityStep newFinal (Fired {before = newFinal} {afterState = newSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag newRoot) NoTransitions (AvailabilityEnd newSuccessor)))
extendAdmittedMoveByRoot {name} {key} {world} {error} {value} {oldFinal} {newFinal} {oldTrace} {newTrace}
  ...
  MkAdmittedDistanceMove (movedRoot move) (movedComponent move) (prefixWord move) (crossedAction move)
    ...
    (trans (nativeWordAppend oldTrail (AvailabilityStep oldFinal (Fired {before = oldFinal} {afterState = oldSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag oldRoot) NoTransitions (AvailabilityEnd oldSuccessor)))
    ...
    (trans (nativeWordAppend newTrail (AvailabilityStep newFinal (Fired {before = newFinal} {afterState = newSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag newRoot) NoTransitions (AvailabilityEnd newSuccessor)))
    ...
    (totalDistance nameEq keyEq (appendAvailability oldTrail (AvailabilityStep oldFinal (Fired {before = oldFinal} {afterState = oldSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag oldRoot) NoTransitions (AvailabilityEnd oldSuccessor)))) (totalDistance nameEq keyEq (appendAvailability newTrail (AvailabilityStep newFinal (Fired {before = newFinal} {afterState = newSuccessor} nameEq keyEq (OInsert following Root component) OInsertTag newRoot) NoTransitions (AvailabilityEnd newSuccessor)))) Refl Refl
