...
assert data==(ROOT/'research-tests/O6-R197-FINAL-VALIDATION-PLAN.json').read_bytes()
assert len(plan)==144 and len({x['path'] for x in plan})==144
assert not (OUT/'final-validation-launched.json').exists(), 'Append-only launch; gate any continuation'
launch=dict(startUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),planSHA256=sha(data),runnerSHA256=sha((ROOT/'research-tests/run-r197-check.py').read_bytes()),count=len(plan))
(OUT/'final-validation-launched.json').write_text(json.dumps(launch,indent=2)+'\n')
completed=[]
for item in plan:
    assert sha((ROOT/'research-tests/run-r197-check.py').read_bytes())==launch['runnerSHA256'], 'Runner mutated'
    ...
    command=[sys.executable,'-I',str(ROOT/'research-tests/run-r197-check.py'),item['unit'],item['path']]
...
report=dict(status='PASS',completed=completed,total=len(plan),sourceTargets=143,packageBuild='seeded PASS; not cold',endUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),planSHA256=launch['planSHA256'])
(OUT/'final-validation-result.json').write_text(json.dumps(report,indent=2)+'\n')
