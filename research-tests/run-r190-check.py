#!/usr/bin/env python3
"""One serialized, seeded, fresh R190 check. Launch detached with python3 -I.
No TTC is deleted. JSON/log/source snapshots live under /tmp/dgamma-r190.
Usage: run-r190-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]]; PATH=package for build.
"""
import datetime
import hashlib
import json
...
assert datetime.datetime.now(datetime.timezone.utc) < datetime.datetime(2026,9,8,16,41,47,tzinfo=datetime.timezone.utc), 'R190 new-attempt time guard'
if re.fullmatch(r'[A-E]\d+-\d+', unit):
    assert datetime.datetime.now(datetime.timezone.utc) < datetime.datetime(2026,9,8,16,41,47,tzinfo=datetime.timezone.utc), 'R190 proof-attempt stop; reserve final validation'
