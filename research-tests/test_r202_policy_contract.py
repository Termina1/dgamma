...
spec=importlib.util.spec_from_file_location('contract',ROOT/'research-tests/r202_evidence_contract.py')
c=importlib.util.module_from_spec(spec);spec.loader.exec_module(c)
class PolicyTests(unittest.TestCase):
    def setUp(self):
        self.p=json.dumps(dict(authorization='SUPERVISOR RULE CHANGE: CROSS-LANE HEAVY LOCK ABOLISHED',runnerSHA256='new',shift='R202',oneCompilerPerLane=True)).encode()
    ...
    def test_non_timestamp_overlap_rejected(self):
        ...
        with self.assertRaises(AssertionError):c.validate_execution_policy(self.r,self.p)
    def history(self):
        old=json.loads(self.p);old['shift']='R201';old=json.dumps(old).encode()
        correction=dict(onlyChangedKey='shift',before='R201',after='R202',beforeSHA256=hashlib.sha256(old).hexdigest(),afterSHA256=hashlib.sha256(self.p).hexdigest(),priorInvocations=['V12'],timestampUTC='2026-09-09T12:36:00+00:00')
        return old,correction
    def test_unapproved_old_label_rejected(self):
        old,correction=self.history();self.r['executionPolicySHA256']=hashlib.sha256(old).hexdigest()
        with self.assertRaises(AssertionError):c.validate_execution_policy(self.r,old)
    def test_authenticated_metadata_only_history(self):
        old,correction=self.history();self.r['executionPolicySHA256']=hashlib.sha256(old).hexdigest()
        c.authenticate_policy_record(self.r,self.p,old,correction)
    def test_changed_enforcement_history_rejected(self):
        old,correction=self.history();mut=json.loads(old);mut['oneCompilerPerLane']=False;old=json.dumps(mut).encode()
        correction['beforeSHA256']=hashlib.sha256(old).hexdigest();self.r['executionPolicySHA256']=hashlib.sha256(old).hexdigest()
        with self.assertRaises(AssertionError):c.authenticate_policy_record(self.r,self.p,old,correction)
