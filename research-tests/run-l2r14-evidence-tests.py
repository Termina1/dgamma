#!/usr/bin/env python3
"""Copied from L2R13; compiler-free focused tests of the INDEPENDENT evidence validator, not
...
spec=importlib.util.spec_from_file_location('l2r14_verifier',Path(__file__).with_name('run-l2r14-independent-verify.py'))
v=importlib.util.module_from_spec(spec);spec.loader.exec_module(v)

class EvidenceTests(unittest.TestCase):
 def setUp(self):
  self.path='research-tests/O6-L2R14-Sources/DGamma/L2R14Test.idr'
  self.source=b'module DGamma.L2R14Test\n%default total\npublic export\ndummy : Nat\ndummy = 0\n'
  self.log='1/1: Building DGamma.L2R14Test ('+str(v.ROOT/self.path)+')\n'
 ...
 def test_wrong_target_build(self):
  log=self.log.replace('L2R14Test','Wrong');r=copy.deepcopy(self.r);r.update(transcript=log,buildingLines=log.splitlines())
  with self.assertRaises(AssertionError):v.record_valid(r,self.source,log)
 ...
 def test_path_traversal(self):self.assertFalse(v.path_allowed('research-tests/O6-L2R14-Sources/../../src/DGamma/X.idr'))
 ...
 def visibility_fixture(self):
  before=self.source+b'export\n0 packetWholeTransportFromActionShapes : Nat\npacketWholeTransportFromActionShapes = 0\n'
  old=b'export\n0 packetWholeTransportFromActionShapes :'
  new=b'||| Public because consumers reduce its computed record fields.\npublic export\n0 packetWholeTransportFromActionShapes :'
  after=before.replace(old,new);path=v.PREFIX+'Sources/DGamma/L2R14LocalSquareProduction.idr'
  prior=dict(unit='C8-1',path=path,passed=True,end=self.r['start'],sourceSHA256=v.sha(before))
  validation=dict(unit='V0',path=path,passed=True,start=self.r['end'],sourceSHA256=v.sha(after))
  authority=dict(validation='V0',pendingAttempt='C10-1',path=path,beforeSHA256=v.sha(before),afterSHA256=v.sha(after),old=old.decode(),new=new.decode())
 ...
 def test_visibility_rejects_body_change(self):
  a,b,c,d,e=self.visibility_fixture();d=d.replace(b'Shapes = 0',b'Shapes = 1');b['sourceSHA256']=e['afterSHA256']=v.sha(d)
 ...
 def test_visibility_rejects_quantity_change(self):
  a,b,c,d,e=self.visibility_fixture();d=d.replace(b'0 packetWhole',b'1 packetWhole');b['sourceSHA256']=e['afterSHA256']=v.sha(d)
  with self.assertRaises(AssertionError):v.visibility_validation_valid(a,b,c,d,e)
 def test_visibility_rejects_type_change(self):
  a,b,c,d,e=self.visibility_fixture();d=d.replace(b'Shapes : Nat',b'Shapes : Bool');b['sourceSHA256']=e['afterSHA256']=v.sha(d)
  with self.assertRaises(AssertionError):v.visibility_validation_valid(a,b,c,d,e)
