...
Accept lane-owned L2R14 non-Idris artifacts ONLY. No predecessor documentary or source repair is authorized.
Usage: python3 -I research-tests/run-l2r14-artifact-commit.py UNIT MESSAGE PATH...
"""
...
repair_checks = []
for path in paths:
 assert path in REPAIRS or path in DOCUMENTARY or (path.startswith(('research-tests/O6-L2R14-', 'research-tests/run-l2r14-')) and not path.endswith('.idr'))
 ...
 if path in DOCUMENTARY:
  ...
  expected=before
  for replacement in repair.get('replacements',[{k:repair[k] for k in ['old','new']} ] if 'old' in repair else []):
   assert expected.count(replacement['old'].encode())==1
   expected=expected.replace(replacement['old'].encode(),replacement['new'].encode())
