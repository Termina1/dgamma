#!/usr/bin/env python3
"""One serialized, seeded, fresh R201 check. Launch detached with python3 -I.
No TTC is deleted. JSON/log/source snapshots live under /tmp/dgamma-r201.
Usage: run-r201-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]]; PATH=package for build.
"""
import datetime
import hashlib
import json
...
OUT = pathlib.Path('/tmp/dgamma-r201')
...
cutoff = datetime.datetime(2026,9,9,18,41,0,tzinfo=datetime.timezone.utc) if planned_validation else datetime.datetime(2026,9,9,18,26,0,tzinfo=datetime.timezone.utc)
assert datetime.datetime.now(datetime.timezone.utc) < cutoff, 'R201 attempt18:26:00 / frozen-validation18:41:00 start guard'
...
if path == 'package':
    ...
    if any(word in path for word in ['ActorLifecycleOnlyExtended','CP5AvailabilityAware']):
        assert planned_validation and snapshot == subprocess.check_output(['git','show','13020369:'+path],cwd=ROOT)
    target.touch()
...
# R201 starts under the already-effective owner no-lock rule.
...
for frozen in ['CP5ConfluenceLocalDiamondSpike','CP5ConfluenceDeletionChainSpike','CP5O19SurfaceSpike','CP5ConfluenceCrossTraceSpike','CP5ConfluenceCanonicalSortSpike','CP5ConfluenceRenamingCompositionSpike']:
    ...
    assert (ROOT/fp).read_bytes()==subprocess.check_output(['git','show','13020369:'+fp],cwd=ROOT), 'Frozen source changed: '+fp
...
if path == 'research/DGamma/CP5ConfluenceLocalDiamondSpike.idr':
    ...
    assert snapshot == subprocess.check_output(['git','show','13020369:'+path],cwd=ROOT), 'Changed LocalDiamond requires prior gate'
