...
Usage: python3 -I research-tests/run-l2r9-commit.py UNIT MESSAGE
...
record = json.loads((pathlib.Path('/tmp/dgamma-l2r9')/(unit+'.json')).read_text())
...
assert json.loads(pathlib.Path('/tmp/dgamma-l2r9/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
...
assert record['path'].startswith(('research-tests/O6-L2R9-Sources/',)), 'Lane-owned source only'
...
with pathlib.Path('/tmp/dgamma-l2r9/commit-receipts.jsonl').open('a') as ledger:
