#!/usr/bin/env python3
"""Independent, compiler-free L2R10 evidence validator. Imports NO check/commit
...
PREFIX='research-tests/O6-L2R10-'
RAW=Path('/tmp/dgamma-l2r10')
BASE='a4c9f506'
REPAIR='research-tests/O6-L2R9-Sources/DGamma/L2R9ProviderHead.idr'
CAPS={'A':14,'B':16,'C':10,'D':10}
...
def path_allowed(path):
 return '..' not in Path(path).parts and not Path(path).is_absolute() and (path.startswith(PREFIX+'Sources/') or path in {REPAIR,'research-tests/O6-L2R9-Sources/DGamma/L2R9OrdinalScan.idr'})
...
def record_valid(r,data,log):
 ...
 if r.get('declaredHeavy'):assert r['unit']=='C3-3'
 assert all(p['classification']!='lane2' and str(ROOT)+'/' not in p['command'] for p in r['separateCompilerObservations'])
 guard=48 if r.get('declaredHeavy') else 18
...
def verify(use_archive=True):
 ...
 closed=json.loads(stored['source-closed.json']);assert closed['retainedDeclarations']==42 and closed['newAttemptsForbidden']
 ...
 assert {k:sum(r['unit'].startswith(k) for r in successful) for k in CAPS}=={'A':14,'B':16,'C':2,'D':10}
 for path,data in retained.items():assert (ROOT/path).read_bytes()==data
 repair=json.loads((ROOT/(PREFIX+'COMMENT-REPAIR.json')).read_text());assert repair['path']==REPAIR
 old=git('show',BASE+':'+REPAIR);new=(ROOT/REPAIR).read_bytes()
 assert sha(old)==repair['beforeSHA256'] and sha(new)==repair['afterSHA256']
 assert old.count(repair['old'].encode())==1 and old.replace(repair['old'].encode(),repair['new'].encode())==new
 assert code(old)==code(new)
 changed=git('diff','--name-only',BASE,'HEAD').decode().splitlines()
 assert all(p==REPAIR or p.startswith((PREFIX,'research-tests/run-l2r10-')) for p in changed)
 ...
 assert plan_time<=datetime.datetime.fromisoformat(byid['V5']['start']).timestamp()
 assert len(plan['validations'])==12
 ...
 final=json.loads(stored['final-validation-result.json']);assert final['passed'] and len(final['validations'])==12
 heavy=byid['C3-3'];assert heavy.get('declaredHeavy') and heavy['interrupted'] and not heavy['passed'] and heavy['buildingCount']==0
 authority=json.loads((ROOT/(PREFIX+'HEAVY-EXCEPTION.json')).read_text())
 assert sha((ROOT/(PREFIX+'HEAVY-EXCEPTION.json')).read_bytes())==heavy['heavyAuthoritySHA256']
 assert heavy['sourceSHA256']==authority['sourceSHA256'] and stored['C3-2.source']==stored['C3-3.source']
 assert heavy['concurrentHeavyOverlapTimestamps'] and heavy['maxSampleRSSKiB']>48*1024*1024
 assert all(heavy['start']<=t<=heavy['end'] for t in heavy['concurrentHeavyOverlapTimestamps'])
 assert len(groups['C3'])==3 and not any(x['passed'] for x in groups['C3'])
 ...
 return dict(passed=True,checkedHead=git('rev-parse','HEAD').decode().strip(),records=len(records),proofAttempts=sum(len(v) for v in groups.values()),retainedDeclarations=42,failedAttempts=[r['unit'] for r in records if not r['passed']],guardedSourceCommits=len(successful),guardedReceipts=len(receipts),finalValidations=12,archiveFiles=len(stored),archiveSHA256=manifest['archiveSHA256'],noStagedFiles=True,ownCompilerRunning=False,humanMathematicalReview='parent-owned, not performed by this script',scope='PARTIAL; no full single/R191 replay, phases, contiguity instance, admitted moves or normalizer')
