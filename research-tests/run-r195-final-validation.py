#!/usr/bin/env python3
"""Detached serial immutable R195 plan. Stop on failure; never retry silently.
Every compiler is launched by python3 -I run-r195-check.py with seeded caches,
...
assert plan_bytes == (ROOT / 'research-tests/O6-R195-FINAL-VALIDATION-PLAN.json').read_bytes()
...
for item in plan:
    ...
    args = ['python3', '-I', str(ROOT / 'research-tests/run-r195-check.py'), item['unit'], item['path']]
