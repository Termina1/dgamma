#!/usr/bin/env python3
"""Copied from L2R13; supervisor-authorized L2R14 C8 visibility-only commit after exact V0 PASS.
No type/body/multiplicity changes; exact hash/text ruling checked before stage.
Usage: python3 -I research-tests/run-l2r14-visibility-commit.py V0 MESSAGE
"""
import datetime
import hashlib
import json
...
record = json.loads((pathlib.Path('/tmp/dgamma-l2r14')/(unit+'.json')).read_text())
...
assert json.loads(pathlib.Path('/tmp/dgamma-l2r14/ledger.jsonl').read_text().splitlines()[-1])['unit'] == unit, 'No intervening compiler invocation'
source_attempt = unit
assert unit == 'V0'
authority=json.loads(pathlib.Path('/tmp/dgamma-l2r14/VISIBILITY-RULING.json').read_text())
...
assert record['path'].startswith(('research-tests/O6-L2R14-Sources/',)), 'Lane-owned source only'
