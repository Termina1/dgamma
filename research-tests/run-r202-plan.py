#!/usr/bin/env python3
"""Compiler-free append-only freeze of all 171 inherited main sources + changes + package."""
...
old=json.loads((ART/'O6-R201-COMPILER-LEDGER.json').read_text())['records'];items={}
for r in old:
...
assert len(items)==171, '165 pre-R201 + 6 R201 sources'
excluded=[]
...
for path in changed:items[path]=dict(path=path,expectedDiagnostic=None,symbol=None,origin='R202 changed/new source')
