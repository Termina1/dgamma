...
Usage: python3 -I run-l2r4-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]].
"""
...
assert path.startswith('research-tests/O6-L2R4-Sources/'), 'Lane-owned targets only'
...
if re.fullmatch(r'[ABC]\d+-\d+',unit):
 assert int(unit.split('-')[0][1:]) <= {'A':22,'B':18,'C':12}[unit[0]]
...
heavy=os.environ.get('L2R4_HEAVY')=='1' or any(k in path for k in ['CanonicalSort','R8FullPipeline','ReachedBlocks','AllFour','LocalDiamond','L2R4AvailabilityCollision'])
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R4 task BUILD SEED; target only, dependency rebuilds acknowledged')
