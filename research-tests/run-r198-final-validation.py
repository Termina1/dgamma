...
ROOT=pathlib.Path(__file__).resolve().parents[1];OUT=pathlib.Path('/tmp/dgamma-r198')
...
assert data==(ROOT/'research-tests/O6-R198-FINAL-VALIDATION-PLAN.json').read_bytes()
scope=json.loads((ROOT/'research-tests/O6-R198-VALIDATION-SCOPE.json').read_text())
assert len(plan)==scope['checksIncludingPackage'] and len({x['path'] for x in plan})==len(plan)
assert scope['inheritedSources']==143 and not scope['excludedInheritedApplicablePaths']
...
launch=dict(startUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),head=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),planSHA256=sha(data),runnerSHA256=sha((ROOT/'research-tests/run-r198-check.py').read_bytes()),count=len(plan))
(OUT/'final-validation-launched.json').write_text(json.dumps(launch,indent=2)+'\n')
completed=[]
for item in plan:
    assert sha((ROOT/'research-tests/run-r198-check.py').read_bytes())==launch['runnerSHA256'], 'Runner mutated'
    ...
    command=[sys.executable,'-I',str(ROOT/'research-tests/run-r198-check.py'),item['unit'],item['path']]
