#!/usr/bin/env python3
"""Compiler-free, separately executable L2R6 evidence/receipt/scope verifier.
...
worktree access occurs. Writes only /tmp/dgamma-l2r6/independent-verification.json.
"""
...
for r in records:
 ...
 assert not r.get('bundleSources'), 'No L2R6 bundle authority'
...
planGroups = [('final-validation-plan.json','research-tests/O6-L2R6-FINAL-VALIDATION-PLAN.json')]
plans = []
seen_paths = set()
for localPlan, planpath in planGroups:
 ...
 plans += group
# No predecessor source or runner repair is authorized in L2R6.
repairs = {}
paths = git('diff', '--name-only', BASE).decode().splitlines()
allowed = ('research-tests/O6-L2R6-', 'research-tests/run-l2r6-')
...
publication = ''
manifest = json.loads((ROOT/('research-tests/O6-L2R6'+publication+'-COMPILER-LEDGER.json')).read_text())
archive = ROOT/('research-tests/O6-L2R6'+publication+'-COMPILER-EVIDENCE.tar.gz')
...
rehome=json.loads((ROOT/'research-tests/O6-L2R6-CP3-REHOME-MANIFEST.json').read_text())
...
draft=(ROOT/'research-tests/O6-L2R6-CP3-DIFF-DRAFT.md').read_text()
