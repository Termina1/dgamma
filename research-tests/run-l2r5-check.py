...
Usage: python3 -I run-l2r5-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]].
"""
...
assert path.startswith('research-tests/O6-L2R5-Sources/') or (unit == 'V0' and path == 'research-tests/O6-L2R4-Sources/DGamma/L2R4AnchorMeasure.idr'), 'Lane-owned targets or exact unchanged bootstrap target only'
...
if re.fullmatch(r'[ABC]\d+-\d+',unit):
 assert int(unit.split('-')[0][1:]) <= {'A':20,'B':16,'C':10}[unit[0]]
...
heavy=os.environ.get('L2R5_HEAVY')=='1' or any(k in path for k in ['CanonicalSort','R8FullPipeline','ReachedBlocks','AllFour','LocalDiamond','L2R5AvailabilityCollision'])
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R5 task BUILD SEED; target only, dependency rebuilds acknowledged')
