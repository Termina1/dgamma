...
assert path.startswith('research-tests/O6-L2R6-Sources/') or (unit == 'V0' and path == 'research-tests/O6-L2R5-Sources/DGamma/L2R5RootCatalog.idr'), 'Lane-owned targets or exact unchanged bootstrap target only'
...
if re.fullmatch(r'[ABCD]\d+-\d+',unit):
 assert int(unit.split('-')[0][1:]) <= {'A':14,'B':14,'C':10,'D':12}[unit[0]]
...
heavy=False;locked=False;lock_events=[]  # L2R6: light-only; NEVER inspect or mutate shared lock/window.
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R6 task BUILD SEED; target only, dependency rebuilds acknowledged')
