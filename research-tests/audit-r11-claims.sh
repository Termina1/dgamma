...
if len(tracked) != 57:
    raise SystemExit(f'expected 57 tracked tests, found {len(tracked)}')
...
if len(entries) != 29:
    raise SystemExit(f'baseline manifest has {len(entries)} holes, expected 29')
baseline_holes = [entry['hole'] for entry in entries]
if len(set(baseline_holes)) != 29:
...
expected = {
    ...
    'CP5ConfluenceLocalDiamondSpike.idr': 4,
