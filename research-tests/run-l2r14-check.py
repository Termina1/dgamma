#!/usr/bin/env python3
"""Copied from L2R13 for L2R14: detached lane2-only seeded single check;
...
Usage: python3 -I run-l2r14-check.py UNIT PATH [DIAGNOSTIC [SYMBOL]].
"""
...
OUT = pathlib.Path('/tmp/dgamma-l2r14')
...
assert path.startswith('research-tests/O6-L2R14-Sources/'), 'Lane-owned targets or exact unchanged bootstrap only'
...
if re.fullmatch(r'[ABCD]\d+-\d+',unit):
 ...
 assert int(unit.split('-')[0][1:]) <= {'A':14,'B':16,'C':10}[unit[0]]
...
heavy=False;locked=False;lock_events=[]  # L2R14: light-only; NEVER inspect or mutate shared lock/window.
...
touch_record=dict(path=str(target),oldMtimeNs=old_mtime,newMtimeNs=target.stat().st_mtime_ns,authority='L2R14 task BUILD SEED; target only, dependency rebuilds acknowledged')
...
for i,(extra,data) in enumerate(bundle):
 ...
 bundle_records.append(dict(path=str(extra.relative_to(ROOT)),sourceSHA256=hashlib.sha256(data).hexdigest(),sourceFile=sourcefile,targetMtimeTouch=dict(path=str(extra),oldMtimeNs=old,newMtimeNs=extra.stat().st_mtime_ns,authority='No companion bundle is authorized in L2R14')))
...
# L2R14 persists ONLY overlap timestamps from its first invocation.
