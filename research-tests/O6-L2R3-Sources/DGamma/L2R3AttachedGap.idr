...
||| CP3:1873 / L2R2ConditionalGap:42 counterpart for ATTACHED blocks. The
||| actual gap's offset is linked to the earlier block's entire physical body.
||| Count transport is retained for dependent endpoints. Coverage/normalization
||| and bundle nonoverlap remain inputs; none is generated by this theorem.
export
0 attachedZeroGapInNormalForm :
  {name, key, world, error : Type} -> {value : key -> Type} ->
  ...
  (later : LocatedOpenEpisodeBlockAttached name key world error value nameEq keyEq laterName global) ->
  (ordered : BlockBeforeAttached name key world error value nameEq keyEq global earlierName laterName earlier later) ->
...
attachedZeroGapInNormalForm earlier later ordered gap offset physical offsetPhysical covered normal separated =
  trans (sym physical) (attachedGapCount gap offset covered normal separated)
