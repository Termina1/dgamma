...
||| Unpack only the actual accounting witness, preserving its birth rather
||| than reconstructing a trace or a registration occurrence independently.
export
0 o20DeletionRetainedBirthFromAccount :
  {name, key, world, error : Type} -> {value : key -> Type} ->
  {protocol : RegistrationProtocol key value world error} ->
  {nameEq : DecEq name} -> {keyEq : DecEq key} ->
  {initial, finalState : SystemState name key value world error} ->
...
o20DeletionRetainedBirthFromAccount trace premises candidate step generation classified (birth ** exact) =
