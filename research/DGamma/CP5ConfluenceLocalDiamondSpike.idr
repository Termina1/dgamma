...
||| Runtime-owning refinement of LocatedConsTargetStage. The original whole
||| stage remains the index, while producer-owned equations relate each runtime
||| projection to the exact StageFromAdvance payload retained by the package.
data LocatedConsTargetStageRuntime :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  {targetFirst, targetMiddle, targetFinal :
    SystemState name key value world error} ->
  (targetHead : Transition targetFirst targetMiddle) ->
  ...
  IteratorStage name key world error value actor
    (MoreTransitions targetHead targetTail) -> Type where
  MkLocatedConsTargetStageRuntime :
    (actor : name) ->
    ...
    (0 forwardRuntimeExact : (state : EffectState name key value world) ->
      traceGeneratorMap (IteratorForwardGenerator targetStage) state =
        traceGeneratorMap (IteratorForwardGenerator
          (StageFromAdvance nameEq keyEq actor tag equation occurs fiber found
            remaining accumulator view lifecycle step rest suffix)) state) ->
    (0 yieldedRuntimeExact : (origin, state : EffectState name key value world) ->
...
||| Universally quantify the occurrence together with the exact reconstructed
||| stage. Unlike the failed local helper from revision 63, `occurs` is an
||| argument of this eliminator rather than a fixed value captured outside it.
0 jointForwardTargetRegion :
  (targetStage : IteratorStage name key world error value actor
    (MoreTransitions targetHead targetTail)) ->
  (before, afterState : SystemState name key value world error) ->
  ...
  JointLocatedConsTargetGenerator name key world error value targetHead
    targetTail actor (IteratorForwardGenerator targetStage)
jointForwardTargetRegion targetStage before afterState nameEq keyEq tag equation
  OccursHere occurrenceView fiber found remaining accumulator view lifecycle step
  rest suffix forwardExact outcomeExact =
    JointConsForwardHere targetStage
      (StageFromAdvance nameEq keyEq actor tag equation OccursHere fiber found
        remaining accumulator view lifecycle step rest suffix)
      (\state => trans Refl (sym (forwardExact state)))
      (\state => trans (outcomeExact state) Refl)
jointForwardTargetRegion targetStage before afterState nameEq keyEq tag equation
  (DGamma.Metatheory.OccursLater later) occurrenceView fiber found remaining
  accumulator view lifecycle step rest suffix forwardExact outcomeExact =
    JointConsForwardLater targetStage
      (StageFromAdvance nameEq keyEq actor tag equation later fiber found
        remaining accumulator view lifecycle step rest suffix)
      (\state => trans Refl (sym (forwardExact state)))
      (\state => trans (outcomeExact state) Refl)

0 jointYieldedTargetRegion :
  (targetStage : IteratorStage name key world error value actor
    (MoreTransitions targetHead targetTail)) ->
  (origin : EffectState name key value world) ->
  (before, afterState : SystemState name key value world error) ->
  ...
  (rest : List (StepEffect key value world error
    ...
    (componentProvisions (fiberComponent fiber)))) ->
  (suffix : ReachableSuffix remaining (step :: rest)) ->
  (0 yieldedExact : (observedOrigin, state :
      EffectState name key value world) ->
    traceGeneratorMap (IteratorYieldedGenerator targetStage observedOrigin) state =
      traceGeneratorMap (IteratorYieldedGenerator
        (StageFromAdvance nameEq keyEq actor tag equation occurs fiber found
          remaining accumulator view lifecycle step rest suffix)
        observedOrigin) state) ->
  (0 outcomeExact : (state : EffectState name key value world) ->
    iteratorStageOutcome targetStage state =
      iteratorStageOutcome
        (StageFromAdvance nameEq keyEq actor tag equation occurs fiber found
  ...
  JointLocatedConsTargetGenerator name key world error value targetHead
    targetTail actor (IteratorYieldedGenerator targetStage origin)
jointYieldedTargetRegion targetStage origin before afterState nameEq keyEq tag
  equation OccursHere occurrenceView fiber found remaining accumulator view
  lifecycle step rest suffix yieldedExact outcomeExact =
    JointConsYieldedHere targetStage origin
      (StageFromAdvance nameEq keyEq actor tag equation OccursHere fiber found
        remaining accumulator view lifecycle step rest suffix)
      (\state => trans Refl (sym (yieldedExact origin state)))
      (\state => trans (outcomeExact state) Refl)
jointYieldedTargetRegion targetStage origin before afterState nameEq keyEq tag
  equation (DGamma.Metatheory.OccursLater later) occurrenceView fiber found
  remaining accumulator view lifecycle step rest suffix yieldedExact
  outcomeExact =
    JointConsYieldedLater targetStage origin
      (StageFromAdvance nameEq keyEq actor tag equation later fiber found
        remaining accumulator view lifecycle step rest suffix)
      (\state => trans Refl (sym (yieldedExact origin state)))
      (\state => trans (outcomeExact state) Refl)

||| Candidate-(2) producer. It does not inspect a generator or stage directly:
||| all stage payload and runtime equations arrive through the already-closed
||| LocatedConsTargetGenerator/LocatedConsTargetStageRuntime producer chain.
0 consumeJointConsTargetGenerator :
  LocatedConsTargetGenerator name key world error value targetHead targetTail
    actor target ->
  JointLocatedConsTargetGenerator name key world error value targetHead targetTail
    actor target
consumeJointConsTargetGenerator
  (LocatedConsActualGenerator before afterState nameEq keyEq action tag equation
    occurs occurrenceView owned) =
      case occurrenceView of
        ConsStageOccursHere =>
          let targetWhole : TraceEffectGenerator name key world error value actor
                (MoreTransitions targetHead targetTail)
              targetWhole = ActualForwardGenerator before afterState nameEq keyEq
                action tag equation occurs owned
              targetSingleton : TraceEffectGenerator name key world error value
                actor (MoreTransitions targetHead NoTransitions)
              targetSingleton = ActualForwardGenerator before afterState nameEq
                keyEq action tag equation OccursHere owned
          in JointConsActualHere targetWhole targetSingleton (\state => Refl)
        ConsStageOccursLater targetHead later =>
          let targetWhole : TraceEffectGenerator name key world error value actor
                (MoreTransitions targetHead targetTail)
              targetWhole = ActualForwardGenerator before afterState nameEq keyEq
                action tag equation occurs owned
              targetLocal : TraceEffectGenerator name key world error value actor
                targetTail
              targetLocal = ActualForwardGenerator before afterState nameEq keyEq
                action tag equation later owned
          in JointConsActualLater targetWhole targetLocal (\state => Refl)
consumeJointConsTargetGenerator (LocatedConsForwardGenerator stage located) =
  case located of
    MkLocatedConsTargetStageRuntime actor targetStage before afterState nameEq
      keyEq tag equation occurs occurrenceView fiber found remaining accumulator
      view lifecycle step rest suffix forwardExact yieldedExact outcomeExact =>
        jointForwardTargetRegion targetStage before afterState nameEq keyEq tag
          equation occurs occurrenceView fiber found remaining accumulator view
          lifecycle step rest suffix forwardExact outcomeExact
consumeJointConsTargetGenerator
  (LocatedConsYieldedGenerator stage origin located) =
    case located of
      MkLocatedConsTargetStageRuntime actor targetStage before afterState nameEq
        keyEq tag equation occurs occurrenceView fiber found remaining accumulator
        view lifecycle step rest suffix forwardExact yieldedExact outcomeExact =>
          jointYieldedTargetRegion targetStage origin before afterState nameEq
            keyEq tag equation occurs occurrenceView fiber found remaining
            accumulator view lifecycle step rest suffix yieldedExact outcomeExact

0 locateJointConsTargetGenerator :
  {targetHead : Transition targetFirst targetMiddle} ->
  {targetTail : Transitions targetMiddle targetFinal} ->
  {actor : name} ->
  (target : TraceEffectGenerator name key world error value actor
...
locateJointConsTargetGenerator target =
  consumeJointConsTargetGenerator (locateConsTargetGenerator target)
...
||| Empty-program L-Advance reconstruction under pointwise controls. Target
||| matching is transported through the independently ordered registries before
||| selecting the finish or landing-divert evaluator branch.
0 replayPointwiseAdvanceEmptyOperational :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
...
replayPointwiseAdvanceEmptyOperational nameEq keyEq actor sourceWorld
  ...
  tag sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint =
    let sourceState : SystemState name key value world error
        ...
        replayedState = MkSystemState replayedWorld replayedRegistry
        sourceOwner : Fiber name key value world error
        sourceOwner = MkFiber component sourceParent retiredFlag sourceTable
          (Reloading [] sourceAccumulator view)
        replayedOwner : Fiber name key value world error
        replayedOwner = MkFiber component replayedParent retiredFlag replayedTable
          (Reloading [] replayedAccumulator view)
        0 targetsSame : targetFiber @{nameEq} @{keyEq} sourceOwner sourceRegistry =
          targetFiber @{nameEq} @{keyEq} replayedOwner replayedRegistry
        targetsSame = pointwiseConcreteTargetFiberSame nameEq keyEq component
          sourceParent replayedParent retiredFlag retiredFlag sourceTable
          replayedTable (Reloading [] sourceAccumulator view)
          (Reloading [] replayedAccumulator view) Refl sourceState replayedState
          (replayedControls beforeEndpoint) (replayedEffects beforeEndpoint)
          (replayedWellFormed beforeEndpoint)
        matches : Bool
        matches = targetMatches @{nameEq}
          (targetFiber @{nameEq} @{keyEq} sourceOwner sourceRegistry) view
        0 sourceMatches : targetMatches @{nameEq}
          (targetFiber @{nameEq} @{keyEq} sourceOwner sourceRegistry) view =
          matches
        sourceMatches = Refl
        0 replayedMatches : targetMatches @{nameEq}
          (targetFiber @{nameEq} @{keyEq} replayedOwner replayedRegistry) view =
          matches
        replayedMatches = sym (cong (\target => targetMatches @{nameEq} target view)
          targetsSame)
    in emptyByMatch matches sourceMatches replayedMatches
  where
  0 emptyByMatch :
    (matches : Bool) ->
    targetMatches @{nameEq}
      (targetFiber @{nameEq} @{keyEq}
        (MkFiber component sourceParent retiredFlag sourceTable
          (Reloading [] sourceAccumulator view)) sourceRegistry) view = matches ->
    targetMatches @{nameEq}
      (targetFiber @{nameEq} @{keyEq}
        (MkFiber component replayedParent retiredFlag replayedTable
          (Reloading [] replayedAccumulator view)) replayedRegistry) view = matches ->
    PointwiseAdvanceOperationalReplay name key world error value nameEq keyEq actor
      (MkSystemState sourceWorld sourceRegistry) sourceAfter tag sourceChecked
      (MkSystemState replayedWorld replayedRegistry)
  emptyByMatch True sourceMatches replayedMatches =
      let sourceState : SystemState name key value world error
          sourceState = MkSystemState sourceWorld sourceRegistry
          replayedState : SystemState name key value world error
          replayedState = MkSystemState replayedWorld replayedRegistry
          ...
          replayedOwner = MkFiber component replayedParent retiredFlag replayedTable
            (Reloading [] replayedAccumulator view)
          sourceNext : Fiber name key value world error
          sourceNext = MkFiber component sourceParent retiredFlag sourceTable
            (Active sourceAccumulator view)
          replayedNext : Fiber name key value world error
          replayedNext = MkFiber component replayedParent retiredFlag replayedTable
            (Active replayedAccumulator view)
          0 sourceConcrete : applyAction @{nameEq} @{keyEq} (LAdvance actor)
            sourceState = Just (LFinishTag, MkSystemState sourceWorld
              (replaceBinding @{nameEq} actor sourceNext sourceRegistry))
          sourceConcrete = rewrite sourceFound in rewrite sourceMatches in Refl
          0 pairSame : (LFinishTag, MkSystemState sourceWorld
              (replaceBinding @{nameEq} actor sourceNext sourceRegistry)) =
            (tag, sourceAfter)
          pairSame = justInjective (trans (sym sourceConcrete) sourceRaw)
          0 targetRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
            replayedState = Just (LFinishTag, MkSystemState replayedWorld
              (replaceBinding @{nameEq} actor replayedNext replayedRegistry))
          targetRaw = rewrite replayedFound in rewrite replayedMatches in Refl
          0 nextOwnerRelated : FiberControlRelated sourceNext replayedNext
          nextOwnerRelated = FibersControlRelated sourceParent replayedParent
            retiredFlag retiredFlag sourceTable replayedTable
            (Active sourceAccumulator view) (Active replayedAccumulator view)
            parentsSame Refl (ActiveControls {error = error} accumulatorsSame Refl)
      in case pairSame of
        Refl =>
          let 0 nextControls : ControlEquivalent name key world error value
                nameEq sourceAfter
                (MkSystemState replayedWorld
                  (replaceBinding @{nameEq} actor replayedNext replayedRegistry))
              nextControls = pointwiseControlAfterReplace nameEq actor sourceWorld
                replayedWorld sourceRegistry replayedRegistry sourceOwner
                replayedOwner sourceNext replayedNext sourceFound replayedFound
                nextOwnerRelated (replayedControls beforeEndpoint)
          in packagePointwiseAdvanceOperationalReplay nameEq keyEq actor
            sourceState sourceAfter replayedState
            (MkSystemState replayedWorld
              (replaceBinding @{nameEq} actor replayedNext replayedRegistry))
            LFinishTag sourceChecked (replayedWellFormed beforeEndpoint)
            targetRaw nextControls mapsRelated (replayedEffects beforeEndpoint)
  emptyByMatch False sourceMatches replayedMatches =
      let sourceState : SystemState name key value world error
          sourceState = MkSystemState sourceWorld sourceRegistry
          replayedState : SystemState name key value world error
          replayedState = MkSystemState replayedWorld replayedRegistry
          ...
          sourceNext : Fiber name key value world error
          sourceNext = MkFiber component sourceParent retiredFlag sourceTable
            (Unloading sourceAccumulator view Nothing)
          replayedNext : Fiber name key value world error
          replayedNext = MkFiber component replayedParent retiredFlag replayedTable
            (Unloading replayedAccumulator view Nothing)
          0 sourceConcrete : applyAction @{nameEq} @{keyEq} (LAdvance actor)
            sourceState = Just (LDivertTag, MkSystemState sourceWorld
              (replaceBinding @{nameEq} actor sourceNext sourceRegistry))
          sourceConcrete = rewrite sourceFound in rewrite sourceMatches in Refl
          0 pairSame : (LDivertTag, MkSystemState sourceWorld
              (replaceBinding @{nameEq} actor sourceNext sourceRegistry)) =
            (tag, sourceAfter)
          pairSame = justInjective (trans (sym sourceConcrete) sourceRaw)
          0 targetRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
            replayedState = Just (LDivertTag, MkSystemState replayedWorld
              (replaceBinding @{nameEq} actor replayedNext replayedRegistry))
          targetRaw = rewrite replayedFound in rewrite replayedMatches in Refl
          0 nextOwnerRelated : FiberControlRelated sourceNext replayedNext
          nextOwnerRelated = FibersControlRelated sourceParent replayedParent
            retiredFlag retiredFlag sourceTable replayedTable
            (Unloading sourceAccumulator view Nothing)
            (Unloading replayedAccumulator view Nothing) parentsSame Refl
      ...
      in case pairSame of
        Refl =>
          ...
          in packagePointwiseAdvanceOperationalReplay nameEq keyEq actor
            ...
            (MkSystemState replayedWorld
              (replaceBinding @{nameEq} actor replayedNext replayedRegistry))
            LDivertTag sourceChecked (replayedWellFormed beforeEndpoint)
            targetRaw nextControls mapsRelated (replayedEffects beforeEndpoint)

||| Defined failure L-Advance branch. Exact observable failure equality is the
||| only branch-specific outcome capital; all registry framing stays pointwise.
0 replayPointwiseAdvanceRaisedOperational :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  (replayedFound : lookupFiber @{nameEq} actor replayedRegistry = Just
    (MkFiber component replayedParent retiredFlag replayedTable
      (Reloading (step :: rest) replayedAccumulator view))) ->
  (sourceCapability, replayedCapability : DepValues key value
    (dependencies (componentDependencies component))) ->
  ...
  resolveCommittedValues @{nameEq} @{keyEq} {name = name} {key = key}
    {value = value} {world = world} {error = error}
    (dependencies (componentDependencies component)) view replayedRegistry =
    Just replayedCapability ->
  (sourceError, replayedError : error) -> sourceError = replayedError ->
  runStepEffect step sourceCapability
    (MkLocalState sourceWorld
      (restrictOwnedPreservingOrder @{keyEq} (componentProvisions component)
        (ownedValues sourceTable))) = Left sourceError ->
  runStepEffect step replayedCapability
    (MkLocalState replayedWorld
      (restrictOwnedPreservingOrder @{keyEq} (componentProvisions component)
        (ownedValues replayedTable))) = Left replayedError ->
  (tag : RuleTag) -> (sourceAfter : SystemState name key value world error) ->
  (sourceRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
    (MkSystemState sourceWorld sourceRegistry) = Just (tag, sourceAfter)) ->
  (sourceChecked : checkedApplyAction @{nameEq} @{keyEq} (LAdvance actor)
  ...
  PointwiseAdvanceOperationalReplay name key world error value nameEq keyEq actor
    (MkSystemState sourceWorld sourceRegistry) sourceAfter tag sourceChecked
    (MkSystemState replayedWorld replayedRegistry)
replayPointwiseAdvanceRaisedOperational nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  ...
  sourceFound replayedFound sourceCapability replayedCapability sourceResolved
  replayedResolved sourceError replayedError errorsSame sourceRan replayedRan tag
  sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint =
    let sourceState : SystemState name key value world error
        sourceState = MkSystemState sourceWorld sourceRegistry
        replayedState : SystemState name key value world error
        replayedState = MkSystemState replayedWorld replayedRegistry
        ...
        sourceNext : Fiber name key value world error
        sourceNext = MkFiber component sourceParent retiredFlag sourceTable
          (Unloading sourceAccumulator view (Just sourceError))
        replayedNext : Fiber name key value world error
        replayedNext = MkFiber component replayedParent retiredFlag replayedTable
          (Unloading replayedAccumulator view (Just replayedError))
        0 sourceConcrete : applyAction @{nameEq} @{keyEq} (LAdvance actor)
          sourceState = Just (LRaiseTag, MkSystemState sourceWorld
            (replaceBinding @{nameEq} actor sourceNext sourceRegistry))
        sourceConcrete = rewrite sourceFound in rewrite sourceResolved in
          rewrite sourceRan in Refl
        0 pairSame : (LRaiseTag, MkSystemState sourceWorld
            (replaceBinding @{nameEq} actor sourceNext sourceRegistry)) =
          (tag, sourceAfter)
        pairSame = justInjective (trans (sym sourceConcrete) sourceRaw)
        0 targetRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
          replayedState = Just (LRaiseTag, MkSystemState replayedWorld
            (replaceBinding @{nameEq} actor replayedNext replayedRegistry))
        targetRaw = rewrite replayedFound in rewrite replayedResolved in
          rewrite replayedRan in Refl
        0 nextOwnerRelated : FiberControlRelated sourceNext replayedNext
        nextOwnerRelated = FibersControlRelated sourceParent replayedParent
          retiredFlag retiredFlag sourceTable replayedTable
          (Unloading sourceAccumulator view (Just sourceError))
          (Unloading replayedAccumulator view (Just replayedError)) parentsSame
          Refl (UnloadingControls accumulatorsSame Refl (cong Just errorsSame))
    in case pairSame of
      Refl =>
        let targetState : SystemState name key value world error
            ...
            nextControls = pointwiseControlAfterReplace nameEq actor sourceWorld
              ...
              nextOwnerRelated (replayedControls beforeEndpoint)
        in packagePointwiseAdvanceOperationalReplay nameEq keyEq actor
          sourceState sourceAfter replayedState targetState LRaiseTag sourceChecked
          (replayedWellFormed beforeEndpoint) targetRaw nextControls mapsRelated
          (replayedEffects beforeEndpoint)

||| Successful yielded landing-divert reconstructed with concrete, fully typed
||| source and replay endpoints before any common operational packaging.
0 replayPointwiseAdvanceYieldedDivertOperational :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  resolveCommittedValues @{nameEq} @{keyEq} {name = name} {key = key}
    ...
    (dependencies (componentDependencies component)) view replayedRegistry =
    Just replayedCapability ->
  (sourceLocal, replayedLocal : LocalState key value world
    (componentProvisions component)) ->
  (sourceUndo, replayedUndo : LocalState key value world
    (componentProvisions component) -> LocalState key value world
    (componentProvisions component)) ->
  runStepEffect step sourceCapability
    (MkLocalState sourceWorld
      (restrictOwnedPreservingOrder @{keyEq} (componentProvisions component)
        (ownedValues sourceTable))) = Right (sourceLocal, sourceUndo) ->
  runStepEffect step replayedCapability
    (MkLocalState replayedWorld
      (restrictOwnedPreservingOrder @{keyEq} (componentProvisions component)
        (ownedValues replayedTable))) = Right (replayedLocal, replayedUndo) ->
  PartialMapsEquivalent (EffectStateEquivalence keyEq)
    ...
    (yieldedInverseEffectMap nameEq keyEq actor
      (componentProvisions component) replayedUndo) ->
  targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq}
      (MkFiber component sourceParent retiredFlag sourceTable
        (Reloading (step :: rest) sourceAccumulator view)) sourceRegistry) view =
    False ->
  targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq}
      (MkFiber component replayedParent retiredFlag replayedTable
        (Reloading (step :: rest) replayedAccumulator view)) replayedRegistry)
    view = False ->
  (tag : RuleTag) -> (sourceAfter : SystemState name key value world error) ->
  (sourceRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
    (MkSystemState sourceWorld sourceRegistry) = Just (tag, sourceAfter)) ->
  ...
  PointwiseAdvanceOperationalReplay name key world error value nameEq keyEq actor
    (MkSystemState sourceWorld sourceRegistry) sourceAfter tag sourceChecked
    (MkSystemState replayedWorld replayedRegistry)
replayPointwiseAdvanceYieldedDivertOperational nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step rest sourceAccumulator
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  sourceCapability replayedCapability sourceResolved replayedResolved sourceLocal
  replayedLocal sourceUndo replayedUndo sourceRan replayedRan undoMaps sourceMatches
  replayedMatches tag sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint =
    let sourceState : SystemState name key value world error
        sourceState = MkSystemState sourceWorld sourceRegistry
        replayedState : SystemState name key value world error
        replayedState = MkSystemState replayedWorld replayedRegistry
        ...
        replayedOwner = MkFiber component replayedParent retiredFlag replayedTable
          (Reloading (step :: rest) replayedAccumulator view)
        sourcePushed : LocalState key value world (componentProvisions component) ->
          LocalState key value world (componentProvisions component)
        sourcePushed = pushLocalUndo @{keyEq} (componentProvisions component)
          sourceAccumulator sourceUndo
        replayedPushed : LocalState key value world (componentProvisions component) ->
          LocalState key value world (componentProvisions component)
        replayedPushed = pushLocalUndo @{keyEq} (componentProvisions component)
          replayedAccumulator replayedUndo
        0 undosRelated : (input : LocalState key value world
              (componentProvisions component)) -> LocalStateRuntimeRelated
          (sourceUndo (normalizeLocal @{keyEq} (componentProvisions component)
            input))
          (replayedUndo (normalizeLocal @{keyEq} (componentProvisions component)
            input))
        undosRelated = yieldedMapsGiveLocalUndoRuntimeRelated nameEq keyEq actor
          (componentProvisions component) sourceUndo replayedUndo undoMaps
        0 pushedRelated : AccumulatorRelated sourcePushed replayedPushed
        pushedRelated = pushLocalUndoRuntimeRelated keyEq
          (componentProvisions component) sourceAccumulator replayedAccumulator
          sourceUndo replayedUndo accumulatorsSame undosRelated
        sourceNext : Fiber name key value world error
        sourceNext = MkFiber component sourceParent retiredFlag
          (localTable sourceLocal) (Unloading sourcePushed view Nothing)
        replayedNext : Fiber name key value world error
        replayedNext = MkFiber component replayedParent retiredFlag
          (localTable replayedLocal) (Unloading replayedPushed view Nothing)
        sourceEndpoint : SystemState name key value world error
        sourceEndpoint = MkSystemState (localWorld sourceLocal)
          (replaceBinding @{nameEq} actor sourceNext sourceRegistry)
        replayedEndpoint : SystemState name key value world error
        replayedEndpoint = MkSystemState (localWorld replayedLocal)
          (replaceBinding @{nameEq} actor replayedNext replayedRegistry)
        0 sourceConcrete : (applyAction @{nameEq} @{keyEq} (LAdvance actor)
          sourceState = Just (LDivertTag, sourceEndpoint))
        sourceConcrete = rewrite sourceFound in rewrite sourceResolved in
          rewrite sourceRan in rewrite sourceMatches in Refl
        0 pairSame : (LDivertTag, sourceEndpoint) = (tag, sourceAfter)
        pairSame = justInjective (trans (sym sourceConcrete) sourceRaw)
        0 targetRaw : (applyAction @{nameEq} @{keyEq} (LAdvance actor)
          replayedState = Just (LDivertTag, replayedEndpoint))
        targetRaw = rewrite replayedFound in rewrite replayedResolved in
          rewrite replayedRan in rewrite replayedMatches in Refl
        0 nextOwnerRelated : FiberControlRelated sourceNext replayedNext
        nextOwnerRelated = FibersControlRelated sourceParent replayedParent
          retiredFlag retiredFlag (localTable sourceLocal)
          (localTable replayedLocal) (Unloading sourcePushed view Nothing)
          (Unloading replayedPushed view Nothing) parentsSame Refl
          (UnloadingControls pushedRelated Refl Refl)
    in case pairSame of
      Refl =>
        let 0 rebasedControls : ControlEquivalent name key world error value
              nameEq (MkSystemState (localWorld sourceLocal) sourceRegistry)
              (MkSystemState (localWorld replayedLocal) replayedRegistry)
            rebasedControls = MkControlEquivalent
              (controlPointwise (replayedControls beforeEndpoint))
            0 nextControls : ControlEquivalent name key world error value nameEq
              sourceAfter replayedEndpoint
            nextControls = pointwiseControlAfterReplace nameEq actor
              (localWorld sourceLocal) (localWorld replayedLocal) sourceRegistry
              replayedRegistry sourceOwner replayedOwner sourceNext replayedNext
              sourceFound replayedFound nextOwnerRelated rebasedControls
        in packagePointwiseAdvanceOperationalReplay nameEq keyEq actor sourceState
          sourceAfter replayedState replayedEndpoint LDivertTag sourceChecked
          (replayedWellFormed beforeEndpoint) targetRaw nextControls mapsRelated
          (replayedEffects beforeEndpoint)

||| Successful yielded finish reconstructed at the concrete singleton-program
||| endpoints before entering the common operational package.
0 replayPointwiseAdvanceYieldedFinishOperational :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  (sourceFound : lookupFiber @{nameEq} actor sourceRegistry = Just
    (MkFiber component sourceParent retiredFlag sourceTable
      (Reloading [step] sourceAccumulator view))) ->
  (replayedFound : lookupFiber @{nameEq} actor replayedRegistry = Just
    (MkFiber component replayedParent retiredFlag replayedTable
      (Reloading [step] replayedAccumulator view))) ->
  (sourceCapability, replayedCapability : DepValues key value
    (dependencies (componentDependencies component))) ->
  resolveCommittedValues @{nameEq} @{keyEq} {name = name} {key = key}
    {value = value} {world = world} {error = error}
  ...
  targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq}
      (MkFiber component sourceParent retiredFlag sourceTable
        (Reloading [step] sourceAccumulator view)) sourceRegistry) view = True ->
  targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq}
      (MkFiber component replayedParent retiredFlag replayedTable
        (Reloading [step] replayedAccumulator view)) replayedRegistry) view =
    True ->
  (tag : RuleTag) -> (sourceAfter : SystemState name key value world error) ->
  (sourceRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
    (MkSystemState sourceWorld sourceRegistry) = Just (tag, sourceAfter)) ->
  ...
  PointwiseAdvanceOperationalReplay name key world error value nameEq keyEq actor
    (MkSystemState sourceWorld sourceRegistry) sourceAfter tag sourceChecked
    (MkSystemState replayedWorld replayedRegistry)
replayPointwiseAdvanceYieldedFinishOperational nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step sourceAccumulator
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  sourceCapability replayedCapability sourceResolved replayedResolved sourceLocal
  replayedLocal sourceUndo replayedUndo sourceRan replayedRan undoMaps sourceMatches
  replayedMatches tag sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint =
    let sourceState : SystemState name key value world error
        ...
        sourceNext : Fiber name key value world error
        sourceNext = MkFiber component sourceParent retiredFlag
          (localTable sourceLocal) (Active sourcePushed view)
        replayedNext : Fiber name key value world error
        replayedNext = MkFiber component replayedParent retiredFlag
          (localTable replayedLocal) (Active replayedPushed view)
        sourceEndpoint : SystemState name key value world error
        sourceEndpoint = MkSystemState (localWorld sourceLocal)
          (replaceBinding @{nameEq} actor sourceNext sourceRegistry)
        replayedEndpoint : SystemState name key value world error
        replayedEndpoint = MkSystemState (localWorld replayedLocal)
          (replaceBinding @{nameEq} actor replayedNext replayedRegistry)
        0 sourceConcrete : (applyAction @{nameEq} @{keyEq} (LAdvance actor)
          sourceState = Just (LFinishTag, sourceEndpoint))
        sourceConcrete = rewrite sourceFound in rewrite sourceResolved in
          rewrite sourceRan in rewrite sourceMatches in Refl
        0 pairSame : (LFinishTag, sourceEndpoint) = (tag, sourceAfter)
        pairSame = justInjective (trans (sym sourceConcrete) sourceRaw)
        0 targetRaw : (applyAction @{nameEq} @{keyEq} (LAdvance actor)
          replayedState = Just (LFinishTag, replayedEndpoint))
        targetRaw = rewrite replayedFound in rewrite replayedResolved in
          rewrite replayedRan in rewrite replayedMatches in Refl
        0 nextOwnerRelated : FiberControlRelated sourceNext replayedNext
        nextOwnerRelated = FibersControlRelated sourceParent replayedParent
          retiredFlag retiredFlag (localTable sourceLocal)
          (localTable replayedLocal) (Active sourcePushed view)
          (Active replayedPushed view) parentsSame Refl
      ...
      Refl =>
        let 0 rebasedControls : ControlEquivalent name key world error value
            ...
            nextControls = pointwiseControlAfterReplace nameEq actor
              ...
              sourceFound replayedFound nextOwnerRelated rebasedControls
        in packagePointwiseAdvanceOperationalReplay nameEq keyEq actor sourceState
          sourceAfter replayedState replayedEndpoint LFinishTag sourceChecked
          (replayedWellFormed beforeEndpoint) targetRaw nextControls mapsRelated
          (replayedEffects beforeEndpoint)

||| Successful yielded iteration reconstructed at concrete nonempty-rest
||| endpoints before entering the common operational package.
0 replayPointwiseAdvanceYieldedIterOperational :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  (next : StepEffect key value world error
    (dependencies (componentDependencies component))
    (componentProvisions component)) ->
  (later : List (StepEffect key value world error
    (dependencies (componentDependencies component))
    (componentProvisions component))) ->
  (sourceAccumulator, replayedAccumulator : LocalState key value world
    (componentProvisions component) -> LocalState key value world
  ...
  (sourceFound : lookupFiber @{nameEq} actor sourceRegistry = Just
    (MkFiber component sourceParent retiredFlag sourceTable
      (Reloading (step :: next :: later) sourceAccumulator view))) ->
  (replayedFound : lookupFiber @{nameEq} actor replayedRegistry = Just
    (MkFiber component replayedParent retiredFlag replayedTable
      (Reloading (step :: next :: later) replayedAccumulator view))) ->
  (sourceCapability, replayedCapability : DepValues key value
    (dependencies (componentDependencies component))) ->
  resolveCommittedValues @{nameEq} @{keyEq} {name = name} {key = key}
    {value = value} {world = world} {error = error}
  ...
  targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq}
      (MkFiber component sourceParent retiredFlag sourceTable
        (Reloading (step :: next :: later) sourceAccumulator view)) sourceRegistry) view = True ->
  targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq}
      (MkFiber component replayedParent retiredFlag replayedTable
        (Reloading (step :: next :: later) replayedAccumulator view)) replayedRegistry) view =
    True ->
  (tag : RuleTag) -> (sourceAfter : SystemState name key value world error) ->
  (sourceRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
    (MkSystemState sourceWorld sourceRegistry) = Just (tag, sourceAfter)) ->
  ...
  PointwiseAdvanceOperationalReplay name key world error value nameEq keyEq actor
    (MkSystemState sourceWorld sourceRegistry) sourceAfter tag sourceChecked
    (MkSystemState replayedWorld replayedRegistry)
replayPointwiseAdvanceYieldedIterOperational nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step next later sourceAccumulator
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  sourceCapability replayedCapability sourceResolved replayedResolved sourceLocal
  replayedLocal sourceUndo replayedUndo sourceRan replayedRan undoMaps sourceMatches
  replayedMatches tag sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint =
    let sourceState : SystemState name key value world error
        ...
        sourceNext : Fiber name key value world error
        sourceNext = MkFiber component sourceParent retiredFlag
          (localTable sourceLocal) (Reloading (next :: later) sourcePushed view)
        replayedNext : Fiber name key value world error
        replayedNext = MkFiber component replayedParent retiredFlag
          (localTable replayedLocal) (Reloading (next :: later) replayedPushed view)
        sourceEndpoint : SystemState name key value world error
        sourceEndpoint = MkSystemState (localWorld sourceLocal)
          (replaceBinding @{nameEq} actor sourceNext sourceRegistry)
        replayedEndpoint : SystemState name key value world error
        replayedEndpoint = MkSystemState (localWorld replayedLocal)
          (replaceBinding @{nameEq} actor replayedNext replayedRegistry)
        0 sourceConcrete : (applyAction @{nameEq} @{keyEq} (LAdvance actor)
          sourceState = Just (LIterTag, sourceEndpoint))
        sourceConcrete = rewrite sourceFound in rewrite sourceResolved in
          rewrite sourceRan in rewrite sourceMatches in Refl
        0 pairSame : (LIterTag, sourceEndpoint) = (tag, sourceAfter)
        pairSame = justInjective (trans (sym sourceConcrete) sourceRaw)
        0 targetRaw : (applyAction @{nameEq} @{keyEq} (LAdvance actor)
          replayedState = Just (LIterTag, replayedEndpoint))
        targetRaw = rewrite replayedFound in rewrite replayedResolved in
          rewrite replayedRan in rewrite replayedMatches in Refl
        0 nextOwnerRelated : FiberControlRelated sourceNext replayedNext
        nextOwnerRelated = FibersControlRelated sourceParent replayedParent
          retiredFlag retiredFlag (localTable sourceLocal)
          (localTable replayedLocal) (Reloading (next :: later) sourcePushed view)
          (Reloading (next :: later) replayedPushed view) parentsSame Refl
      ...
      Refl =>
        let 0 rebasedControls : ControlEquivalent name key world error value
            ...
            nextControls = pointwiseControlAfterReplace nameEq actor
              ...
              sourceFound replayedFound nextOwnerRelated rebasedControls
        in packagePointwiseAdvanceOperationalReplay nameEq keyEq actor sourceState
          sourceAfter replayedState replayedEndpoint LIterTag sourceChecked
          (replayedWellFormed beforeEndpoint) targetRaw nextControls mapsRelated
          (replayedEffects beforeEndpoint)
...
||| Producer-owned defined-failure L-Advance branch. Exact runtime outcome
||| equations force both capability resolutions and both evaluator runs; no
||| resolution, run, or error equality is supplied by the outer caller.
0 eliminateSealedPointwiseAdvanceFailureBranch :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  (replayedFound : lookupFiber @{nameEq} actor replayedRegistry = Just
    (MkFiber component replayedParent retiredFlag replayedTable
      (Reloading (step :: rest) replayedAccumulator view))) ->
  (sourceError, replayedError : error) ->
  (sourceOutcome : runtimeAdvanceOutcome nameEq keyEq actor component step rest
    view sourceWorld sourceTable sourceRegistry =
    Just (IteratorRaised sourceError)) ->
  (replayedOutcome : runtimeAdvanceOutcome nameEq keyEq actor component step rest
    view replayedWorld replayedTable replayedRegistry =
    Just (IteratorRaised replayedError)) ->
  sourceError = replayedError ->
  (tag : RuleTag) -> (sourceAfter : SystemState name key value world error) ->
  (sourceRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
    (MkSystemState sourceWorld sourceRegistry) = Just (tag, sourceAfter)) ->
  (sourceChecked : checkedApplyAction @{nameEq} @{keyEq} (LAdvance actor)
  ...
  PointwiseRelationalHeadReplay name key world error value nameEq keyEq
    (Fired {before = MkSystemState sourceWorld sourceRegistry}
      {afterState = sourceAfter} nameEq keyEq (LAdvance actor) tag sourceChecked)
    (MkSystemState replayedWorld replayedRegistry)
eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step rest sourceAccumulator
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  sourceError replayedError sourceOutcome replayedOutcome errorsSame tag
  sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint
  with (resolveCommittedValues @{nameEq} @{keyEq}
    (dependencies (componentDependencies component)) view sourceRegistry)
  proof sourceResolved
  eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor sourceWorld
    replayedWorld sourceRegistry replayedRegistry component sourceParent
    replayedParent retiredFlag sourceTable replayedTable step rest
    sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
    sourceFound replayedFound sourceError replayedError sourceOutcome
    replayedOutcome errorsSame tag sourceAfter sourceRaw sourceChecked mapsRelated
    beforeEndpoint | Nothing = case sourceOutcome of Refl impossible
  eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor sourceWorld
    replayedWorld sourceRegistry replayedRegistry component sourceParent
    replayedParent retiredFlag sourceTable replayedTable step rest
    sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
    sourceFound replayedFound sourceError replayedError sourceOutcome
    replayedOutcome errorsSame tag sourceAfter sourceRaw sourceChecked mapsRelated
    beforeEndpoint | Just sourceCapability
    with (runStepEffect step sourceCapability
      (MkLocalState sourceWorld
        (restrictOwnedPreservingOrder @{keyEq} (componentProvisions component)
          (ownedValues sourceTable)))) proof sourceRan
    eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor sourceWorld
      replayedWorld sourceRegistry replayedRegistry component sourceParent
      replayedParent retiredFlag sourceTable replayedTable step rest
      sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
      sourceFound replayedFound sourceError replayedError sourceOutcome
      replayedOutcome errorsSame tag sourceAfter sourceRaw sourceChecked
      mapsRelated beforeEndpoint | Just sourceCapability |
      Right (sourceLocal, sourceUndo) =
        case justInjective sourceOutcome of Refl impossible
    eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor sourceWorld
      replayedWorld sourceRegistry replayedRegistry component sourceParent
      replayedParent retiredFlag sourceTable replayedTable step rest
      sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
      sourceFound replayedFound sourceError replayedError sourceOutcome
      replayedOutcome errorsSame tag sourceAfter sourceRaw sourceChecked
      mapsRelated beforeEndpoint | Just sourceCapability | Left observedSourceError
      with (resolveCommittedValues @{nameEq} @{keyEq}
        (dependencies (componentDependencies component)) view replayedRegistry)
      proof replayedResolved
      eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor sourceWorld
        replayedWorld sourceRegistry replayedRegistry component sourceParent
        replayedParent retiredFlag sourceTable replayedTable step rest
        sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
        ...
        replayedOutcome errorsSame tag sourceAfter sourceRaw sourceChecked
        mapsRelated beforeEndpoint | Just sourceCapability |
        Left observedSourceError | Nothing =
          case replayedOutcome of Refl impossible
      eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor sourceWorld
        replayedWorld sourceRegistry replayedRegistry component sourceParent
        replayedParent retiredFlag sourceTable replayedTable step rest
        sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
        ...
        replayedOutcome errorsSame tag sourceAfter sourceRaw sourceChecked
        mapsRelated beforeEndpoint | Just sourceCapability |
        Left observedSourceError | Just replayedCapability
        with (runStepEffect step replayedCapability
          (MkLocalState replayedWorld
            (restrictOwnedPreservingOrder @{keyEq}
              (componentProvisions component) (ownedValues replayedTable))))
        proof replayedRan
        eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor
          sourceWorld replayedWorld sourceRegistry replayedRegistry component
          sourceParent replayedParent retiredFlag sourceTable replayedTable step
          rest sourceAccumulator replayedAccumulator view parentsSame
          accumulatorsSame sourceFound replayedFound sourceError replayedError
          sourceOutcome replayedOutcome errorsSame tag sourceAfter sourceRaw
          sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
          Left observedSourceError | Just replayedCapability |
          Right (replayedLocal, replayedUndo) =
            case justInjective replayedOutcome of Refl impossible
        eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq actor
          sourceWorld replayedWorld sourceRegistry replayedRegistry component
          sourceParent replayedParent retiredFlag sourceTable replayedTable step
          rest sourceAccumulator replayedAccumulator view parentsSame
          ...
          sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
          Left observedSourceError | Just replayedCapability |
          Left observedReplayedError =
            let 0 sourceErrorSame : (observedSourceError = sourceError)
                sourceErrorSame = case justInjective sourceOutcome of Refl => Refl
                0 replayedErrorSame : (observedReplayedError = replayedError)
                replayedErrorSame =
                  case justInjective replayedOutcome of Refl => Refl
                0 observedErrorsSame : (observedSourceError =
                  observedReplayedError)
                observedErrorsSame = trans sourceErrorSame
                  (trans errorsSame (sym replayedErrorSame))
            in packagePointwiseAdvanceHead nameEq keyEq actor sourceWorld
              replayedWorld sourceRegistry replayedRegistry sourceAfter tag
              sourceChecked component sourceParent replayedParent retiredFlag
              sourceTable replayedTable (step :: rest) sourceAccumulator
              replayedAccumulator view sourceFound replayedFound
              (replayPointwiseAdvanceRaisedOperational nameEq keyEq actor
                sourceWorld replayedWorld sourceRegistry replayedRegistry
                component sourceParent replayedParent retiredFlag sourceTable
                replayedTable step rest sourceAccumulator replayedAccumulator view
                parentsSame accumulatorsSame sourceFound replayedFound
                sourceCapability replayedCapability sourceResolved
                replayedResolved observedSourceError observedReplayedError
                observedErrorsSame sourceRan replayedRan tag sourceAfter sourceRaw
                sourceChecked mapsRelated beforeEndpoint)

||| Erased certificate that one already-computed indexed runtime agreement is
||| specifically the yielded case. The outer dispatcher constructs this seal by
||| eliminating the agreement once; branch code never recomputes the outcome.
data SealedPointwiseAdvanceYieldedAgreement :
  RuntimeIteratorOutcomeAgreement name key value world error keyEq first second ->
  Type where
  MkSealedPointwiseAdvanceYieldedAgreement :
    SealedPointwiseAdvanceYieldedAgreement
      (RuntimeYieldsAgree afterRelated undoMaps)

||| Agreement and yielded certificate transported as one dependent package.
||| Keeping both fields under the same outcome indices prevents their proof
||| terms from becoming detached while reindexing concrete runtime results.
record LocatedSealedPointwiseAdvanceYieldedAgreement
  (name, key, world, error : Type) (value : key -> Type) (keyEq : DecEq key)
  (first, second : Maybe (IteratorStageOutcome name key value world error)) where
  constructor MkLocatedSealedPointwiseAdvanceYieldedAgreement
  0 locatedYieldedAgreement : RuntimeIteratorOutcomeAgreement name key value
    world error keyEq first second
  0 locatedYieldedSeal :
    SealedPointwiseAdvanceYieldedAgreement locatedYieldedAgreement

||| Final yielded target-match eliminator. Making the Boolean and both exact
||| equations explicit lets each clause refine them without unfolding the
||| computed target query inside the producer.
0 packageSealedPointwiseAdvanceYieldedMatch :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  PointwiseRelationalHeadReplay name key world error value nameEq keyEq
    (Fired {before = MkSystemState sourceWorld sourceRegistry}
      {afterState = sourceAfter} nameEq keyEq (LAdvance actor) tag sourceChecked)
    (MkSystemState replayedWorld replayedRegistry)
packageSealedPointwiseAdvanceYieldedMatch nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step rest sourceAccumulator
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  sourceCapability replayedCapability sourceResolved replayedResolved sourceLocal
  replayedLocal sourceUndo replayedUndo sourceRan replayedRan undoMaps False
  sourceMatches replayedMatches tag sourceAfter sourceRaw sourceChecked mapsRelated
  beforeEndpoint =
    packagePointwiseAdvanceHead nameEq keyEq actor sourceWorld replayedWorld
      sourceRegistry replayedRegistry sourceAfter tag sourceChecked component
      sourceParent replayedParent retiredFlag sourceTable replayedTable
      (step :: rest) sourceAccumulator replayedAccumulator view sourceFound
      replayedFound
      (replayPointwiseAdvanceYieldedDivertOperational nameEq keyEq actor
        sourceWorld replayedWorld sourceRegistry replayedRegistry component
        sourceParent replayedParent retiredFlag sourceTable replayedTable step rest
        sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
        sourceFound replayedFound sourceCapability replayedCapability
        sourceResolved replayedResolved sourceLocal replayedLocal sourceUndo
        replayedUndo sourceRan replayedRan undoMaps sourceMatches replayedMatches
        tag sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint)
packageSealedPointwiseAdvanceYieldedMatch nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step [] sourceAccumulator
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  sourceCapability replayedCapability sourceResolved replayedResolved sourceLocal
  replayedLocal sourceUndo replayedUndo sourceRan replayedRan undoMaps True
  sourceMatches replayedMatches tag sourceAfter sourceRaw sourceChecked mapsRelated
  beforeEndpoint =
    packagePointwiseAdvanceHead nameEq keyEq actor sourceWorld replayedWorld
      sourceRegistry replayedRegistry sourceAfter tag sourceChecked component
      sourceParent replayedParent retiredFlag sourceTable replayedTable [step]
      sourceAccumulator replayedAccumulator view sourceFound replayedFound
      (replayPointwiseAdvanceYieldedFinishOperational nameEq keyEq actor
        sourceWorld replayedWorld sourceRegistry replayedRegistry component
        sourceParent replayedParent retiredFlag sourceTable replayedTable step
        sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
        sourceFound replayedFound sourceCapability replayedCapability
...
packageSealedPointwiseAdvanceYieldedMatch nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step (next :: later)
  sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
  sourceFound replayedFound sourceCapability replayedCapability sourceResolved
  replayedResolved sourceLocal replayedLocal sourceUndo replayedUndo sourceRan
  replayedRan undoMaps True sourceMatches replayedMatches tag sourceAfter
  sourceRaw sourceChecked mapsRelated beforeEndpoint =
    packagePointwiseAdvanceHead nameEq keyEq actor sourceWorld replayedWorld
      sourceRegistry replayedRegistry sourceAfter tag sourceChecked component
      sourceParent replayedParent retiredFlag sourceTable replayedTable
      (step :: next :: later) sourceAccumulator replayedAccumulator view
      sourceFound replayedFound
      (replayPointwiseAdvanceYieldedIterOperational nameEq keyEq actor sourceWorld
        replayedWorld sourceRegistry replayedRegistry component sourceParent
        replayedParent retiredFlag sourceTable replayedTable step next later
        sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
...
||| Producer-owned yielded L-Advance branch. The yielded seal forces both
||| capability resolutions and evaluator runs. This eliminator owns inverse-map
||| orientation and the divert/finish/iter target-match dispatch.
0 eliminateSealedPointwiseAdvanceYieldedBranch :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  (replayedFound : lookupFiber @{nameEq} actor replayedRegistry = Just
    (MkFiber component replayedParent retiredFlag replayedTable
      (Reloading (step :: rest) replayedAccumulator view))) ->
  (agreement : RuntimeIteratorOutcomeAgreement name key value world error keyEq
  ...
  SealedPointwiseAdvanceYieldedAgreement agreement ->
  (tag : RuleTag) -> (sourceAfter : SystemState name key value world error) ->
  (sourceRaw : applyAction @{nameEq} @{keyEq} (LAdvance actor)
    (MkSystemState sourceWorld sourceRegistry) = Just (tag, sourceAfter)) ->
  (sourceChecked : checkedApplyAction @{nameEq} @{keyEq} (LAdvance actor)
  ...
  PointwiseRelationalHeadReplay name key world error value nameEq keyEq
    (Fired {before = MkSystemState sourceWorld sourceRegistry}
      {afterState = sourceAfter} nameEq keyEq (LAdvance actor) tag sourceChecked)
    (MkSystemState replayedWorld replayedRegistry)
eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor sourceWorld
  replayedWorld sourceRegistry replayedRegistry component sourceParent
  replayedParent retiredFlag sourceTable replayedTable step rest sourceAccumulator
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  agreement yieldedSeal tag sourceAfter sourceRaw sourceChecked mapsRelated
  beforeEndpoint
  with (resolveCommittedValues @{nameEq} @{keyEq}
    (dependencies (componentDependencies component)) view sourceRegistry)
  proof sourceResolved
  eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor sourceWorld
    ...
    replayedParent retiredFlag sourceTable replayedTable step rest
    sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
    sourceFound replayedFound agreement yieldedSeal tag sourceAfter sourceRaw
    sourceChecked mapsRelated beforeEndpoint | Nothing impossible
  eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor sourceWorld
    replayedWorld sourceRegistry replayedRegistry component sourceParent
    replayedParent retiredFlag sourceTable replayedTable step rest
    sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
    ...
    eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor sourceWorld
      ...
      eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor sourceWorld
        ...
        sourceFound replayedFound agreement yieldedSeal tag sourceAfter sourceRaw
        sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
        Right (sourceLocal, sourceUndo) | Nothing impossible
      eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor sourceWorld
        replayedWorld sourceRegistry replayedRegistry component sourceParent
        replayedParent retiredFlag sourceTable replayedTable step rest
        sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
        sourceFound replayedFound agreement yieldedSeal tag sourceAfter sourceRaw
        sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
        Right (sourceLocal, sourceUndo) | Just replayedCapability
        with (runStepEffect step replayedCapability
          (MkLocalState replayedWorld
            (restrictOwnedPreservingOrder @{keyEq}
              (componentProvisions component) (ownedValues replayedTable))))
        proof replayedRan
        eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor
          sourceWorld replayedWorld sourceRegistry replayedRegistry component
          sourceParent replayedParent retiredFlag sourceTable replayedTable step
          rest sourceAccumulator replayedAccumulator view parentsSame
          accumulatorsSame sourceFound replayedFound agreement yieldedSeal tag
          sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint |
          Just sourceCapability | Right (sourceLocal, sourceUndo) |
          Just replayedCapability | Left replayedError impossible
        eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq actor
          sourceWorld replayedWorld sourceRegistry replayedRegistry component
          sourceParent replayedParent retiredFlag sourceTable replayedTable step
          rest sourceAccumulator replayedAccumulator view parentsSame
          accumulatorsSame sourceFound replayedFound
          (RuntimeYieldsAgree primitiveEffects targetSourceUndoMaps)
          MkSealedPointwiseAdvanceYieldedAgreement tag sourceAfter sourceRaw
          sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
          Right (sourceLocal, sourceUndo) | Just replayedCapability |
          Right (replayedLocal, replayedUndo) =
            let sourceOwner : Fiber name key value world error
                sourceOwner = MkFiber component sourceParent retiredFlag
                  sourceTable (Reloading (step :: rest) sourceAccumulator view)
                replayedOwner : Fiber name key value world error
                replayedOwner = MkFiber component replayedParent retiredFlag
                  replayedTable
                  (Reloading (step :: rest) replayedAccumulator view)
                0 targetsSame : targetFiber @{nameEq} @{keyEq} sourceOwner
                    sourceRegistry = targetFiber @{nameEq} @{keyEq}
                    replayedOwner replayedRegistry
                targetsSame = pointwiseConcreteTargetFiberSame nameEq keyEq
                  component sourceParent replayedParent retiredFlag retiredFlag
                  sourceTable replayedTable
                  (Reloading (step :: rest) sourceAccumulator view)
                  (Reloading (step :: rest) replayedAccumulator view) Refl
                  (MkSystemState sourceWorld sourceRegistry)
                  (MkSystemState replayedWorld replayedRegistry)
                  (replayedControls beforeEndpoint)
                  (replayedEffects beforeEndpoint)
                ...
                matches = targetMatches @{nameEq}
                  (targetFiber @{nameEq} @{keyEq} sourceOwner sourceRegistry) view
                0 sourceMatches : targetMatches @{nameEq}
                  (targetFiber @{nameEq} @{keyEq} sourceOwner sourceRegistry)
                  view = matches
                sourceMatches = Refl
                0 replayedMatches : targetMatches @{nameEq}
                  (targetFiber @{nameEq} @{keyEq} replayedOwner replayedRegistry)
                  view = matches
                replayedMatches = sym (cong
                  (\target => targetMatches @{nameEq} target view) targetsSame)
                0 undoMaps : PartialMapsEquivalent
                  (EffectStateEquivalence keyEq)
                  (yieldedInverseEffectMap nameEq keyEq actor
                    (componentProvisions component) sourceUndo)
                  (yieldedInverseEffectMap nameEq keyEq actor
                    (componentProvisions component) replayedUndo)
                undoMaps = replayRelationalEquivalentMapsSymmetric
                  targetSourceUndoMaps
            in packageSealedPointwiseAdvanceYieldedMatch nameEq keyEq actor
              sourceWorld replayedWorld sourceRegistry replayedRegistry component
              sourceParent replayedParent retiredFlag sourceTable replayedTable
              step rest sourceAccumulator replayedAccumulator view parentsSame
              accumulatorsSame sourceFound replayedFound sourceCapability
              replayedCapability sourceResolved replayedResolved sourceLocal
              replayedLocal sourceUndo replayedUndo sourceRan replayedRan
              undoMaps matches sourceMatches replayedMatches tag sourceAfter
              sourceRaw sourceChecked mapsRelated beforeEndpoint

||| An authenticated nonempty source action cannot carry an undefined runtime
||| stage. The source resolution and run are opened here solely to eliminate the
||| impossible `RuntimeOutcomesUndefined` branch.
0 pointwiseAdvanceUndefinedContradiction :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld : world) ->
  (sourceRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  (sourceParent : Parent name) -> (retiredFlag : Bool) ->
  ...
  (sourceFound : lookupFiber @{nameEq} actor sourceRegistry = Just
    (MkFiber component sourceParent retiredFlag sourceTable
      (Reloading (step :: rest) sourceAccumulator view))) ->
  runtimeAdvanceOutcome nameEq keyEq actor component step rest view sourceWorld
    sourceTable sourceRegistry = Nothing ->
  {tag : RuleTag} ->
  {sourceAfter : SystemState name key value world error} ->
  (applyAction @{nameEq} @{keyEq} (LAdvance actor)
    (MkSystemState sourceWorld sourceRegistry) = Just (tag, sourceAfter)) ->
  Void
pointwiseAdvanceUndefinedContradiction nameEq keyEq actor sourceWorld
  sourceRegistry component sourceParent retiredFlag sourceTable step rest
  sourceAccumulator view sourceFound sourceUndefined sourceRaw
  with (resolveCommittedValues @{nameEq} @{keyEq}
    (dependencies (componentDependencies component)) view sourceRegistry)
  proof sourceResolved
  pointwiseAdvanceUndefinedContradiction nameEq keyEq actor sourceWorld
    sourceRegistry component sourceParent retiredFlag sourceTable step rest
    sourceAccumulator view sourceFound sourceUndefined sourceRaw | Nothing =
      nothingNotJustAdvanceDispatch
        (trans (sym (advanceMissingCapabilityIsNothing nameEq keyEq actor
          sourceWorld sourceRegistry component sourceParent retiredFlag
          sourceTable step rest sourceAccumulator view sourceFound
          sourceResolved)) sourceRaw)
  pointwiseAdvanceUndefinedContradiction nameEq keyEq actor sourceWorld
    sourceRegistry component sourceParent retiredFlag sourceTable step rest
    sourceAccumulator view sourceFound sourceUndefined sourceRaw |
    Just sourceCapability
    ...
    pointwiseAdvanceUndefinedContradiction nameEq keyEq actor sourceWorld
      sourceRegistry component sourceParent retiredFlag sourceTable step rest
      sourceAccumulator view sourceFound sourceUndefined sourceRaw |
      Just sourceCapability | Left sourceError =
        case sourceUndefined of Refl impossible
    pointwiseAdvanceUndefinedContradiction nameEq keyEq actor sourceWorld
      sourceRegistry component sourceParent retiredFlag sourceTable step rest
      sourceAccumulator view sourceFound sourceUndefined sourceRaw |
      Just sourceCapability | Right (sourceLocal, sourceUndo) =
...
||| Join one once-computed indexed runtime agreement to the failure or yielded
||| producer-owned seal. No resolution, run, match, map, or endpoint capital is
||| accepted here beyond the sealed branch interfaces.
0 replayPointwiseAdvanceNonemptyHead :
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (actor : name) ->
  (sourceWorld, replayedWorld : world) ->
  (sourceRegistry, replayedRegistry : Registry name key value world error) ->
  (component : Component key value world error) ->
  ...
  PointwiseRelationalHeadReplay name key world error value nameEq keyEq
    (Fired {before = MkSystemState sourceWorld sourceRegistry}
      {afterState = sourceAfter} nameEq keyEq (LAdvance actor) tag sourceChecked)
    (MkSystemState replayedWorld replayedRegistry)
replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld replayedWorld
  ...
  replayedAccumulator view parentsSame accumulatorsSame sourceFound replayedFound
  agreement tag sourceAfter sourceRaw sourceChecked mapsRelated beforeEndpoint
  with (resolveCommittedValues @{nameEq} @{keyEq}
    (dependencies (componentDependencies component)) view sourceRegistry)
  proof sourceResolved
  replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld replayedWorld
    ...
    retiredFlag sourceTable replayedTable step rest sourceAccumulator
    replayedAccumulator view parentsSame accumulatorsSame sourceFound
    replayedFound agreement tag sourceAfter sourceRaw sourceChecked mapsRelated
    beforeEndpoint | Nothing = void (nothingNotJustAdvanceDispatch
      (trans (sym (advanceMissingCapabilityIsNothing nameEq keyEq actor
        sourceWorld sourceRegistry component sourceParent retiredFlag sourceTable
        step rest sourceAccumulator view sourceFound sourceResolved)) sourceRaw))
  replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld replayedWorld
    sourceRegistry replayedRegistry component sourceParent replayedParent
    retiredFlag sourceTable replayedTable step rest sourceAccumulator
    replayedAccumulator view parentsSame accumulatorsSame sourceFound
    ...
    with (resolveCommittedValues @{nameEq} @{keyEq}
      (dependencies (componentDependencies component)) view replayedRegistry)
    proof replayedResolved
    replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
      replayedWorld sourceRegistry replayedRegistry component sourceParent
      replayedParent retiredFlag sourceTable replayedTable step rest
      sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
      sourceFound replayedFound agreement tag sourceAfter sourceRaw sourceChecked
      mapsRelated beforeEndpoint | Just sourceCapability | Nothing
      with (runStepEffect step sourceCapability
        (MkLocalState sourceWorld
          (restrictOwnedPreservingOrder @{keyEq} (componentProvisions component)
            (ownedValues sourceTable)))) proof sourceRan
      replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
        ...
        replayedParent retiredFlag sourceTable replayedTable step rest
        sourceAccumulator replayedAccumulator view parentsSame accumulatorsSame
        sourceFound replayedFound agreement tag sourceAfter sourceRaw
        sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
        Nothing | Left sourceError = void (undefinedDefinedOutcomeImpossible
      ...
      replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
        ...
        sourceFound replayedFound agreement tag sourceAfter sourceRaw
        sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
        Nothing | Right (sourceLocal, sourceUndo) = void
          (undefinedDefinedOutcomeImpossible
            (runtimeAgreementForControls agreement))
    replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
      replayedWorld sourceRegistry replayedRegistry component sourceParent
      replayedParent retiredFlag sourceTable replayedTable step rest
      ...
      replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
        ...
        sourceFound replayedFound agreement tag sourceAfter sourceRaw
        sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
        Just replayedCapability | Left sourceError
        with (runStepEffect step replayedCapability
          (MkLocalState replayedWorld
            (restrictOwnedPreservingOrder @{keyEq}
              (componentProvisions component) (ownedValues replayedTable))))
        ...
        replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
          ...
          sourceFound replayedFound agreement tag sourceAfter sourceRaw
          sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
          Just replayedCapability | Left sourceError | Left replayedError =
            case agreement of
              RuntimeFailuresAgree errorsSame =>
                let 0 sourceOutcome :
                      (runtimeAdvanceOutcome nameEq keyEq actor component step
                        rest view sourceWorld sourceTable sourceRegistry =
                        Just (IteratorRaised sourceError))
                    sourceOutcome = rewrite sourceResolved in
                      rewrite sourceRan in Refl
                    0 replayedOutcome :
                      (runtimeAdvanceOutcome nameEq keyEq actor component step
                        rest view replayedWorld replayedTable replayedRegistry =
                        Just (IteratorRaised replayedError))
                    replayedOutcome = rewrite replayedResolved in
                      rewrite replayedRan in Refl
                in eliminateSealedPointwiseAdvanceFailureBranch nameEq keyEq
                  actor sourceWorld replayedWorld sourceRegistry
                  replayedRegistry component sourceParent replayedParent
                  retiredFlag sourceTable replayedTable step rest
                  sourceAccumulator replayedAccumulator view parentsSame
                  accumulatorsSame sourceFound replayedFound sourceError
                  replayedError sourceOutcome replayedOutcome (sym errorsSame)
                  tag sourceAfter sourceRaw sourceChecked mapsRelated
                  beforeEndpoint
        replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
          replayedWorld sourceRegistry replayedRegistry component sourceParent
          replayedParent retiredFlag sourceTable replayedTable step rest
          ...
          sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
          Just replayedCapability | Left sourceError |
          Right (replayedLocal, replayedUndo) = void
            (successFailureOutcomeImpossible
              (runtimeAgreementForControls agreement))
      replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
        replayedWorld sourceRegistry replayedRegistry component sourceParent
        replayedParent retiredFlag sourceTable replayedTable step rest
        ...
        sourceFound replayedFound agreement tag sourceAfter sourceRaw
        sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
        Just replayedCapability | Right (sourceLocal, sourceUndo)
        with (runStepEffect step replayedCapability
          (MkLocalState replayedWorld
            (restrictOwnedPreservingOrder @{keyEq}
              (componentProvisions component) (ownedValues replayedTable))))
        ...
        replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
          ...
          sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
          Just replayedCapability | Right (sourceLocal, sourceUndo) |
          Left replayedError = void (failureSuccessOutcomeImpossible
            (runtimeAgreementForControls agreement))
        replayPointwiseAdvanceNonemptyHead nameEq keyEq actor sourceWorld
          replayedWorld sourceRegistry replayedRegistry component sourceParent
          replayedParent retiredFlag sourceTable replayedTable step rest
          ...
          sourceChecked mapsRelated beforeEndpoint | Just sourceCapability |
          Just replayedCapability | Right (sourceLocal, sourceUndo) |
          Right (replayedLocal, replayedUndo) = case agreement of
            RuntimeYieldsAgree primitiveEffects targetSourceUndoMaps =>
              let sourceYielded : IteratorStageOutcome name key value world error
                  sourceYielded = IteratorYielded
                    (setEffectTable @{nameEq} actor
                      (ownedValues (localTable sourceLocal))
                      (setEffectAmbient (localWorld sourceLocal)
                        (projectEffectState @{nameEq}
                          (MkSystemState sourceWorld sourceRegistry))))
                    (yieldedInverseEffectMap nameEq keyEq actor
                      (componentProvisions component) sourceUndo)
                    (MkIteratorContinuation rest)
                  replayedYielded : IteratorStageOutcome name key value world
                    error
                  replayedYielded = IteratorYielded
                    (setEffectTable @{nameEq} actor
                      (ownedValues (localTable replayedLocal))
                      (setEffectAmbient (localWorld replayedLocal)
                        (projectEffectState @{nameEq}
                          (MkSystemState replayedWorld replayedRegistry))))
                    (yieldedInverseEffectMap nameEq keyEq actor
                      (componentProvisions component) replayedUndo)
                    (MkIteratorContinuation rest)
                  0 sourceOutcome :
                    (runtimeAdvanceOutcome nameEq keyEq actor component step rest
                      view sourceWorld sourceTable sourceRegistry =
                      Just sourceYielded)
                  sourceOutcome = rewrite sourceResolved in
                    rewrite sourceRan in Refl
                  0 replayedOutcome :
                    (runtimeAdvanceOutcome nameEq keyEq actor component step rest
                      view replayedWorld replayedTable replayedRegistry =
                      Just replayedYielded)
                  replayedOutcome = rewrite replayedResolved in
                    rewrite replayedRan in Refl
                  0 concreteLocated :
                    LocatedSealedPointwiseAdvanceYieldedAgreement name key world
                      error value keyEq (Just replayedYielded)
                      (Just sourceYielded)
                  concreteLocated =
                    MkLocatedSealedPointwiseAdvanceYieldedAgreement
                      (RuntimeYieldsAgree primitiveEffects targetSourceUndoMaps)
                      MkSealedPointwiseAdvanceYieldedAgreement
                  0 replayedLocated :
                    LocatedSealedPointwiseAdvanceYieldedAgreement name key world
                      ...
                      (runtimeAdvanceOutcome nameEq keyEq actor component step
                        rest view replayedWorld replayedTable replayedRegistry)
                      (Just sourceYielded)
                  replayedLocated = replace
                    {p = \targetOutcome =>
                      LocatedSealedPointwiseAdvanceYieldedAgreement name key
                        world error value keyEq targetOutcome
                        (Just sourceYielded)}
                    (sym replayedOutcome) concreteLocated
                  0 runtimeLocated :
                    LocatedSealedPointwiseAdvanceYieldedAgreement name key world
                      error value keyEq
                      (runtimeAdvanceOutcome nameEq keyEq actor component step
                      ...
                      (runtimeAdvanceOutcome nameEq keyEq actor component step
                        rest view sourceWorld sourceTable sourceRegistry)
                  runtimeLocated = replace
                    {p = \sourceOutcome =>
                      LocatedSealedPointwiseAdvanceYieldedAgreement name key
                        world error value keyEq
                        (runtimeAdvanceOutcome nameEq keyEq actor component step
                          rest view replayedWorld replayedTable replayedRegistry)
                        sourceOutcome}
                    (sym sourceOutcome) replayedLocated
              in case runtimeLocated of
                MkLocatedSealedPointwiseAdvanceYieldedAgreement
                  locatedAgreement locatedSeal =>
                    eliminateSealedPointwiseAdvanceYieldedBranch nameEq keyEq
                      actor sourceWorld replayedWorld sourceRegistry
                      replayedRegistry component sourceParent replayedParent
                      retiredFlag sourceTable replayedTable step rest
                      sourceAccumulator replayedAccumulator view parentsSame
                      accumulatorsSame sourceFound replayedFound locatedAgreement
