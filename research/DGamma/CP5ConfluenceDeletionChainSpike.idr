...
0 scopedNonBeginClosingExclusion :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  {initial, finalState, before, afterState :
    SystemState name key value world error} ->
  ...
  (ownerDeclares : Elem wanted (dependencies
    (componentDependencies (fiberComponent leftOwner)))) ->
  providerCandidate @{keyEq} wanted leftSelected = False
scopedNonBeginClosingExclusion name key world error value nameEq keyEq global
  ...
  lifecycle notBegin actorDistinct tag checked insidePrefix rest
  insideDecomposition closingResult boundary leftSelected leftOwner selectedFound
  ownerFound wanted ownerDeclares =
    scopedLocalizedCrossingExclusion name key world error value nameEq keyEq
      global aligned initialWellFormed initialEmpty selected (actionOwner action)
      actorDistinct selectedStartOrdinal selectedStartLive located
      selectedOrdinalExact noDependent registered ordinal live
      (Fired {before = before} {afterState = afterState}
        nameEq keyEq action tag checked)
      (appendTransitions (traceBeforeOpening located)
        (MoreTransitions
      ...
      (scopedSelectedAfterCloseUninstalled name key world error value nameEq
        keyEq selected (locatedEpisode located))
      (scopedNonBeginClosedPrefixAnchor name key world error value nameEq keyEq
        selected located action lifecycle notBegin tag checked insidePrefix rest
        insideDecomposition)
      (scopedNonBeginClosingLocalization name key world error value nameEq keyEq
        global aligned initialEmpty selected located action lifecycle notBegin tag
        checked insidePrefix rest insideDecomposition closingResult)
      insidePrefix
      (MoreTransitions
        (Fired {before = before} {afterState = afterState}
          nameEq keyEq action tag checked)
        rest)
      insideDecomposition Refl before
        ...
        (scopedGlobalAnchorActivation name key world error value nameEq keyEq
          ...
          (appendTransitions (traceBeforeOpening located)
            (MoreTransitions
              ...
              (closedTransitions (locatedEpisode located))))
          global
          (scopedNonBeginClosedPrefixAnchor name key world error value nameEq
            keyEq selected located action lifecycle notBegin tag checked
            insidePrefix rest insideDecomposition)
          (scopedNonBeginClosingLocalization name key world error value nameEq
            keyEq global aligned initialEmpty selected located action lifecycle
            notBegin tag checked insidePrefix rest insideDecomposition
            closingResult)))
...
0 scopedBeginWithAfterOwnerExclusion :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  {initial, finalState, before, afterState :
    SystemState name key value world error} ->
  ...
  (insideDecomposition : appendTransitions insidePrefix
    (MoreTransitions
      (Fired {before = before} {afterState = afterState}
        nameEq keyEq (LBegin actor) tag checked)
      rest) = closedInside (locatedEpisode selectedEpisode)) ->
  (leftSelected, leftOwner, afterOwner : Fiber name key value world error) ->
  (selectedAfterFound : lookupFiber @{nameEq} selected (registry afterState) =
    Just leftSelected) ->
  (afterOwnerFound : lookupFiber @{nameEq} actor (registry afterState) =
    Just afterOwner) ->
  (ownerComponent : fiberComponent afterOwner = fiberComponent leftOwner) ->
  (afterWellFormed : registryWellFormed @{nameEq} @{keyEq} afterState = True) ->
  (targetInstalled : installedAt @{nameEq} actor afterState = True) ->
  (wanted : key) ->
  (ownerDeclares : Elem wanted (dependencies
    (componentDependencies (fiberComponent leftOwner)))) ->
  providerCandidate @{keyEq} wanted leftSelected = False
scopedBeginWithAfterOwnerExclusion name key world error value nameEq keyEq global
  aligned initialWellFormed initialEmpty selected actor actorDistinct
  selectedStartOrdinal selectedStartLive selectedEpisode selectedOrdinalExact
  noDependent tag checked rest selectedRest insidePrefix insideDecomposition
  leftSelected leftOwner afterOwner selectedAfterFound afterOwnerFound
  ownerComponent afterWellFormed targetInstalled wanted ownerDeclares =
    case classifyInstalledContinuation nameEq keyEq actor rest
      (scopedAlignedFromInstalledTrace name key world error value nameEq keyEq
        selected rest selectedRest)
      targetInstalled of
      ContinuationStaysInstalled actorInstalled =>
        scopedDirectAtCurrentExclusion name key world error value nameEq keyEq
          selected actor actorDistinct (locatedEpisode selectedEpisode) rest
          actorInstalled leftSelected afterOwner selectedAfterFound
          afterOwnerFound afterWellFormed wanted
          (replace
            {p = \component => Elem wanted
              (dependencies (componentDependencies component))}
            (sym ownerComponent) ownerDeclares)
      ContinuationCloses closingResult =>
        scopedBeginClosingExclusionAtAfter name key world error value nameEq keyEq
          global aligned initialWellFormed initialEmpty selected actor
          actorDistinct selectedStartOrdinal selectedStartLive selectedEpisode
          selectedOrdinalExact noDependent tag checked insidePrefix rest
          insideDecomposition closingResult leftSelected afterOwner
          selectedAfterFound afterOwnerFound afterWellFormed wanted
          (replace
            {p = \component => Elem wanted
              (dependencies (componentDependencies component))}
            (sym ownerComponent) ownerDeclares)

0 scopedBeginLifecycleExclusion :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  {initial, finalState, before, afterState :
    SystemState name key value world error} ->
  ...
  (selectedFound : lookupFiber @{nameEq} selected (registry before) =
    Just leftSelected) ->
  (ownerFound : lookupFiber @{nameEq} actor (registry before) = Just leftOwner) ->
  (wanted : key) ->
  (ownerDeclares : Elem wanted (dependencies
    (componentDependencies (fiberComponent leftOwner)))) ->
  providerCandidate @{keyEq} wanted leftSelected = False
scopedBeginLifecycleExclusion name key world error value nameEq keyEq global
  aligned initialWellFormed initialEmpty selected actor actorDistinct
  selectedStartOrdinal selectedStartLive selectedEpisode selectedOrdinalExact
  noDependent registered ordinal live tag checked rest selectedRest insidePrefix
  insideDecomposition boundary leftSelected leftOwner selectedFound ownerFound
  wanted ownerDeclares =
    case installedFiberScoped nameEq actor afterState
      (snd
        (snd
          (lBeginBoundary nameEq keyEq actor before afterState tag checked))) of
      (afterOwner ** afterOwnerFound) =>
        scopedBeginWithAfterOwnerExclusion name key world error value nameEq keyEq
          global aligned initialWellFormed initialEmpty selected actor
          actorDistinct selectedStartOrdinal selectedStartLive selectedEpisode
          selectedOrdinalExact noDependent tag checked rest selectedRest
          insidePrefix insideDecomposition leftSelected leftOwner afterOwner
          (trans
            (systemLocalUpdateForeign nameEq selected actor
              (\same => actorDistinct (sym same)) before afterState
              (applyActionLocalUpdate nameEq keyEq (LBegin actor) before
                afterState tag
                (checkedActionProjects nameEq keyEq (LBegin actor) before
                  afterState tag checked)))
            selectedFound)
          afterOwnerFound
          (checkedStepPreservesPresentComponent nameEq keyEq actor
            (LBegin actor) tag before afterState checked leftOwner afterOwner
            ownerFound afterOwnerFound)
          (preservationTheoremProof nameEq keyEq (LBegin actor) before afterState
            tag (selectedOriginalWellFormed boundary)
            (checkedActionProjects nameEq keyEq (LBegin actor) before afterState
              tag checked))
          (snd
            (snd
              (lBeginBoundary nameEq keyEq actor before afterState tag checked)))
...
0 scopedSelectedWholeGlobalEmbedding :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (selected : name) ->
  {initial, finalState : SystemState name key value world error} ->
  {global : Transitions initial finalState} ->
  ...
  OccurrenceEmbedding
    (appendTransitions (closedTransitions (locatedEpisode located))
      (traceAfterClosing located))
    global
scopedSelectedWholeGlobalEmbedding name key world error value nameEq keyEq
  selected located =
    scopedTransportEmbeddingTarget name key world error value
      (locatedDecomposition located)
      (\transition, occurrence =>
        scopedAppendRightEmbedding name key world error value
...
||| An O-Insert of the invariant name cannot fire while the invariant holds:
||| the executable insertion guard requires the name to be absent.  The
||| observed guard and fresh-slot values are explicit arguments so the
||| reduction of `applyAction` reaches inside the insertion branch.
0 scopedInsertAbsentAt :
  (name : Type) -> (key : Type) -> (world : Type) -> (error : Type) ->
  (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  ...
  (0 raw : applyAction @{nameEq} @{keyEq} {name = name} {key = key}
    ...
    (MkSystemState ambient fibers) = Just (tag, afterState)) ->
  lookupFiber @{nameEq} {name = name} {key = key} {value = value}
    {world = world} {error = error} child fibers = Nothing
scopedInsertAbsentAt name key world error value nameEq keyEq child
...
scopedInsertAbsentAt name key world error value nameEq keyEq child
  ...
  freshEq tag afterState =
    \equation =>
      setFreshAbsent {name = name} {key = key} {value = value} {world = world}
        {error = error} nameEq child (freshFiber insertedComponent insertedParent)
        fibers applied freshEq

0 scopedInsertPresentAbsurd :
  (name : Type) -> (key : Type) -> (world : Type) -> (error : Type) ->
  (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  (child : name) -> (insertedParent : Parent name) ->
...
scopedInsertPresentAbsurd name key world error value nameEq keyEq child
  insertedParent insertedComponent (MkSystemState ambient fibers) afterState tag
  checked (MkScopedUnretiredFiberAt fiber found flag) =
    nothingIsNotJust
      (trans
        (sym
          (scopedInsertAbsentAt name key world error value nameEq keyEq child
...
||| L-Begin fires only on an inactive fiber with no stored failure; every
||| other lifecycle makes the source step impossible, so the invariant is
||| either preserved by the worker above or the step is vacuous.
0 scopedBeginPreservesUnretired :
  (name : Type) -> (key : Type) -> (world : Type) -> (error : Type) ->
  (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  (child : name) ->
...
scopedBeginPreservesUnretired name key world error value nameEq keyEq child
  ...
  (MkScopedUnretiredFiberAt
    (MkFiber component parent retiredFlag table (Inactive Nothing)) found
    flag) =
      \equation =>
        scopedBeginPreservesUnretiredAt name key world error value nameEq keyEq
          child before component parent retiredFlag table
          (targetFiber @{nameEq} @{keyEq} {value = value} {world = world}
            {error = error}
            (MkFiber component parent retiredFlag table (Inactive Nothing))
            (registry before))
...
||| Concrete diverted advance outcome, with all evaluator observations explicit.
0 scopedAdvanceDivertedRaw :
  (name : Type) -> (key : Type) -> (world : Type) -> (error : Type) ->
  (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (child : name) ->
  (ambient : world) -> (source : Registry name key value world error) ->
  ...
  (0 matches : (targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq} {name = name} {key = key} {value = value}
      {world = world} {error = error}
      (MkFiber component parent retiredFlag table
        (Reloading (step :: rest) accumulator view)) source) view = False)) ->
  (applyAction @{nameEq} @{keyEq} {name = name} {key = key} {value = value}
    {world = world} {error = error} (LAdvance child) (MkSystemState ambient source) =
    Just (LDivertTag, MkSystemState (localWorld localAfter)
      (replaceBinding @{nameEq} child
        (MkFiber component parent retiredFlag (localTable localAfter)
          (Unloading (pushLocalUndo (componentProvisions component) accumulator undo) view Nothing)) source)))
scopedAdvanceDivertedRaw name key world error value nameEq keyEq child ambient
  source component parent retiredFlag table step rest accumulator view capability
  localAfter undo found resolved ran matches =
    rewrite found in rewrite resolved in rewrite ran in rewrite matches in Refl

||| Concrete finished advance outcome, with all evaluator observations explicit.
0 scopedAdvanceFinishedRaw :
  (name : Type) -> (key : Type) -> (world : Type) -> (error : Type) ->
  (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (child : name) ->
  (ambient : world) -> (source : Registry name key value world error) ->
  ...
  (0 matches : (targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq} {name = name} {key = key} {value = value}
      {world = world} {error = error}
      (MkFiber component parent retiredFlag table
        (Reloading [step] accumulator view)) source) view = True)) ->
  (applyAction @{nameEq} @{keyEq} {name = name} {key = key} {value = value}
    {world = world} {error = error} (LAdvance child) (MkSystemState ambient source) =
    Just (LFinishTag, MkSystemState (localWorld localAfter)
      (replaceBinding @{nameEq} child
        (MkFiber component parent retiredFlag (localTable localAfter)
          (Active (pushLocalUndo (componentProvisions component) accumulator undo) view)) source)))
scopedAdvanceFinishedRaw name key world error value nameEq keyEq child ambient
  source component parent retiredFlag table step accumulator view capability
  localAfter undo found resolved ran matches =
    rewrite found in rewrite resolved in rewrite ran in rewrite matches in Refl
...
||| A matching successful stage retains the flag, whether it finishes or iterates.
0 scopedAdvanceMatchedUnretired :
  (name : Type) -> (key : Type) -> (world : Type) -> (error : Type) ->
  (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (child : name) ->
  (ambient : world) -> (source : Registry name key value world error) ->
  ...
  (0 matches : (targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq} {name = name} {key = key} {value = value}
      {world = world} {error = error}
      (MkFiber component parent retiredFlag table
        (Reloading (step :: rest) accumulator view)) source) view = True)) ->
  ...
  (0 raw : (applyAction @{nameEq} @{keyEq} {name = name} {key = key}
    ...
    (MkSystemState ambient source) = Just (tag, afterState))) ->
  ScopedUnretiredFiberAt name key value world error nameEq child afterState
scopedAdvanceMatchedUnretired name key world error value nameEq keyEq child
  ambient source component parent retiredFlag table step [] accumulator view
  capability localAfter undo found resolved ran matches afterState tag
  notRetired raw =
    scopedUnretiredAfterRuntimeReplace name key world error value nameEq child
      (MkSystemState ambient source) afterState tag LFinishTag component parent
      retiredFlag table (localTable localAfter)
      (Reloading (step :: []) accumulator view)
      (Active (pushLocalUndo (componentProvisions component) accumulator undo) view)
      (localWorld localAfter) found notRetired
      (trans (sym (scopedAdvanceFinishedRaw name key world error value nameEq keyEq
        child ambient source component parent retiredFlag table step
        accumulator view capability localAfter undo found resolved ran matches))
        raw)
scopedAdvanceMatchedUnretired name key world error value nameEq keyEq child
  ambient source component parent retiredFlag table step (next :: more) accumulator view
  capability localAfter undo found resolved ran matches afterState tag
  notRetired raw =
    scopedUnretiredAfterRuntimeReplace name key world error value nameEq child
      (MkSystemState ambient source) afterState tag LIterTag component parent
      retiredFlag table (localTable localAfter)
      (Reloading (step :: (next :: more)) accumulator view)
      (Reloading (next :: more) (pushLocalUndo (componentProvisions component) accumulator undo) view)
      (localWorld localAfter) found notRetired
      (trans (sym (scopedAdvanceIteratedRaw name key world error value nameEq keyEq
        child ambient source component parent retiredFlag table step next more
        accumulator view capability localAfter undo found resolved ran matches))
        raw)

||| Observed target matching chooses one concrete yielded outcome.
0 scopedAdvanceYieldUnretiredAt :
  (name : Type) -> (key : Type) -> (world : Type) -> (error : Type) ->
  (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (child : name) ->
  (ambient : world) -> (source : Registry name key value world error) ->
  ...
  (0 matches : (targetMatches @{nameEq}
    (targetFiber @{nameEq} @{keyEq} {name = name} {key = key} {value = value}
      {world = world} {error = error}
      (MkFiber component parent retiredFlag table
        (Reloading (step :: rest) accumulator view)) source) view = observed)) ->
  (afterState : SystemState name key value world error) -> (tag : RuleTag) ->
  (0 notRetired : (retiredFlag = False)) ->
  (0 raw : (applyAction @{nameEq} @{keyEq} {name = name} {key = key}
    {value = value} {world = world} {error = error} (LAdvance child)
...
scopedAdvanceYieldUnretiredAt name key world error value nameEq keyEq child
  ...
  capability localAfter undo found resolved ran True matches afterState tag
  notRetired raw =
    scopedAdvanceMatchedUnretired name key world error value nameEq keyEq child
      ambient source component parent retiredFlag table step rest accumulator view
      capability localAfter undo found resolved ran matches afterState tag
...
scopedAdvanceYieldUnretiredAt name key world error value nameEq keyEq child
  ambient source component parent retiredFlag table step rest accumulator view
  capability localAfter undo found resolved ran False matches afterState tag
  notRetired raw =
    scopedUnretiredAfterRuntimeReplace name key world error value nameEq child
      (MkSystemState ambient source) afterState tag LDivertTag component parent
      retiredFlag table (localTable localAfter)
      (Reloading (step :: rest) accumulator view)
      (Unloading (pushLocalUndo (componentProvisions component) accumulator undo)
        view Nothing)
      (localWorld localAfter) found notRetired
      (trans (sym (scopedAdvanceDivertedRaw name key world error value nameEq keyEq
