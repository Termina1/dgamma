...
||| The exact fourth R147 bridge clause on the originally SUPPORTED child
||| subdomain, for any actual occurrence replay. No operational permutation,
||| ambient/table/control equivalence or opposite-birth oracle is assumed.
export
0 supportedReplayedBirthBridge :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  (protocol : RegistrationProtocol key value world error) ->
