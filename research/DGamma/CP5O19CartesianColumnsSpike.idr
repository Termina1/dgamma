...
||| Consume an EXPLICIT actual word cut. Nonempty block words force actual
||| left and right heads. E7 derives all current source classifications from
||| ORIGINAL static labels; D15 builds this row. The only continuation is the
||| internal strictly smaller-right-word induction hypothesis from the loop.
export
0 o19ColumnAtWordCut :
  {name, key, world, error : Type} -> {value : key -> Type} ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (protocol : RegistrationProtocol key value world error) ->
  ...
  ((action : Action name key value world error) -> Elem action (rightHead :: remainingRight) -> Elem action fullRightWord) ->
  O19WordCut name key world error value (leftHead :: leftTail) ((rightHead :: remainingRight) ++ suffixWord) residual ->
  (0 smallerColumns : (next : O19ReachedCursor name key world error value protocol nameEq keyEq original) ->
    {nextBefore : SystemState name key value world error} ->
    (nextEarlier : Transitions initial nextBefore) -> (nextRest : Transitions nextBefore (cursorFinal next)) ->
    (appendTransitions nextEarlier nextRest = cursorTrace next) ->
  ...
  O19ColumnRun name key world error value protocol nameEq keyEq (cursorTrace current) earlier (leftHead :: leftTail) (rightHead :: remainingRight) suffixWord
o19ColumnAtWordCut nameEq keyEq protocol swap original blocks premises safety unique leftHead leftTail fullRightWord suffixWord rightHead remainingRight originalClasses current earlier residual decomposition rightMembers
  (MkO19WordCut _ NoTransitions tail cutExact leftExact rightExact count) smallerColumns =
    void (uninhabited (cong length leftExact))
o19ColumnAtWordCut nameEq keyEq protocol swap original blocks premises safety unique leftHead leftTail fullRightWord suffixWord rightHead remainingRight originalClasses current earlier residual decomposition rightMembers
  (MkO19WordCut _ (MoreTransitions left rest) NoTransitions cutExact leftExact rightExact count) smallerColumns =
    void (uninhabited (cong length rightExact))
o19ColumnAtWordCut nameEq keyEq protocol swap original blocks premises safety unique leftHead leftTail fullRightWord suffixWord rightHead remainingRight originalClasses current earlier residual decomposition rightMembers
  (MkO19WordCut _ (MoreTransitions left rest) (MoreTransitions right later) cutExact leftExact rightExact count) smallerColumns =
    o19ColumnAfterRow nameEq keyEq protocol original current earlier (MoreTransitions left rest) right later (leftHead :: leftTail) rightHead remainingRight suffixWord
...
0 o19CartesianColumnsSites :
  ...
  (earlier : Transitions initial before) -> (residual : Transitions before (cursorFinal current)) ->
  (decomposition : appendTransitions earlier residual = cursorTrace current) ->
  (exact : o19ActionWord residual = (leftHead :: leftTail) ++ (remainingRight ++ suffixWord)) ->
  (rightMembers : (action : Action name key value world error) -> Elem action remainingRight -> Elem action fullRightWord) ->
  (o19CrossingSites (cursorDerivation (columnCursor (o19CartesianColumns nameEq keyEq protocol swap original blocks premises safety unique leftHead leftTail fullRightWord suffixWord remainingRight originalClasses current earlier residual decomposition exact rightMembers))) =
...
o19CartesianColumnsSites nameEq keyEq protocol swap original blocks premises safety unique leftHead leftTail fullRightWord suffixWord
  (rightHead :: remainingRight) originalClasses current earlier residual decomposition exact rightMembers =
    o19ColumnAtWordCutSites nameEq keyEq protocol swap original blocks premises safety unique leftHead leftTail fullRightWord suffixWord
      rightHead remainingRight originalClasses current earlier residual decomposition rightMembers
      (o19CutByWord (leftHead :: leftTail) ((rightHead :: remainingRight) ++ suffixWord) residual exact)
      (\next, nextEarlier, nextRest, nextDecomposition, nextExact =>
        o19CartesianColumns nameEq keyEq protocol swap original blocks premises safety unique leftHead leftTail fullRightWord suffixWord
...
||| E14 ACTUAL source-spine site adapter: both factors are actual trace
||| counts, from D16 and structural word lengths. Static classes stay internal.
export
0 o19CartesianSourceSpinesSites :
  {name, key, world, error : Type} -> {value : key -> Type} ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (protocol : RegistrationProtocol key value world error) ->
  {sourceOrder, targetOrder : List name} -> (swap : AdjacentActorOrderSwap name sourceOrder targetOrder) ->
  {initial, sourceFinal, before, leftAfter, rightBefore, rightAfter : SystemState name key value world error} ->
  ...
  (earlier : Transitions initial before) -> (firstLeft : Transition before leftAfter) ->
  (leftRest : Transitions leftAfter rightBefore) -> (rightSpine : Transitions rightBefore rightAfter) -> (later : Transitions rightAfter sourceFinal) ->
  (decomposition : appendTransitions earlier (appendTransitions (MoreTransitions firstLeft leftRest) (appendTransitions rightSpine later)) = source) ->
  (0 originalClasses : {leftAction, rightAction : Action name key value world error} ->
    (leftOccurrence : LocatedActionOccurrence leftAction source) ->
    (rightOccurrence : LocatedActionOccurrence rightAction source) ->
    Elem leftAction (o19ActionWord (MoreTransitions firstLeft leftRest)) -> Elem rightAction (o19ActionWord rightSpine) ->
    O19SourcePairObservation name key world error value (actorLeft swap) (actorRight swap)
      (locatedTransition leftOccurrence) (locatedTransition rightOccurrence)) ->
  (o19CrossingSites (cursorDerivation (columnCursor (o19CartesianSourceSpines nameEq keyEq protocol swap source blocks premises safety unique earlier firstLeft leftRest rightSpine later decomposition originalClasses))) =
    o19ColumnSites (transitionCount earlier) (transitionCount (MoreTransitions firstLeft leftRest)) (transitionCount rightSpine))
o19CartesianSourceSpinesSites nameEq keyEq protocol swap source blocks premises safety unique earlier firstLeft leftRest rightSpine later decomposition originalClasses =
  trans (o19CartesianColumnsSites nameEq keyEq protocol swap source blocks premises safety unique (transitionAction firstLeft) (o19ActionWord leftRest)
    (o19ActionWord rightSpine) (o19ActionWord later) (o19ActionWord rightSpine) originalClasses
    (o19InitialCursor nameEq keyEq protocol source premises unique)
    earlier (appendTransitions (MoreTransitions firstLeft leftRest) (appendTransitions rightSpine later)) decomposition
    (trans (o19ActionWordAppend (MoreTransitions firstLeft leftRest) (appendTransitions rightSpine later))
      (cong ((o19ActionWord (MoreTransitions firstLeft leftRest)) ++) (o19ActionWordAppend rightSpine later)))
    (\action, member => member))
    (cong2 (o19ColumnSites (transitionCount earlier)) (o19ActionWordLength (MoreTransitions firstLeft leftRest)) (o19ActionWordLength rightSpine))
