...
0 registrationIndexLiveAdvance :
  (name, key, world, error : Type) -> (value : key -> Type) -> (nameEq : DecEq name) ->
...
registrationIndexLiveAdvance name key world error value nameEq ordinal (OInsert child Root component)
  (MkRegistrationIndexState live activations counts deleted) = Refl
registrationIndexLiveAdvance name key world error value nameEq ordinal (OInsert child (ChildOf parent) component)
  (MkRegistrationIndexState live activations counts deleted) = Refl
registrationIndexLiveAdvance name key world error value nameEq ordinal (ORetire actor)
  (MkRegistrationIndexState live activations counts deleted) = Refl
registrationIndexLiveAdvance name key world error value nameEq ordinal (ORemove actor)
  (MkRegistrationIndexState live activations counts deleted) = Refl
registrationIndexLiveAdvance name key world error value nameEq ordinal (LBegin actor)
  (MkRegistrationIndexState live activations counts deleted) =
    registrationBeginLiveObserved name key world error value nameEq ordinal actor live activations counts deleted
      (lookupCurrentGeneration @{nameEq} actor live) Refl
registrationIndexLiveAdvance name key world error value nameEq ordinal (LAdvance actor)
  (MkRegistrationIndexState live activations counts deleted) = Refl
registrationIndexLiveAdvance name key world error value nameEq ordinal (LDivert actor)
  (MkRegistrationIndexState live activations counts deleted) = Refl
registrationIndexLiveAdvance name key world error value nameEq ordinal (LLeave actor)
  (MkRegistrationIndexState live activations counts deleted) = Refl
...
export
0 acceptedRightEndpointCurrent :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  {leftFirst, leftFinal, rightFirst, rightFinal : SystemState name key value world error} ->
  (left : Transitions leftFirst leftFinal) -> (right : Transitions rightFirst rightFinal) ->
  (renaming : RegistrationGenerationBijection name) ->
  (registrations : RegistrationCorrespondenceByGeneration nameEq renaming left right) ->
  AlignedTransitions name key world error value nameEq keyEq right -> (bindings (registry rightFirst) = []) ->
  (selected : name) -> (observed : Fiber name key value world error) ->
  (lookupFiber {name = name} {key = key} {value = value} {world = world} {error = error}
    @{nameEq} selected (registry rightFinal) = Just observed) ->
  (generation : RegistrationGeneration name ** lookupCurrentGeneration @{nameEq} selected (rightFinalGenerations registrations) = Just generation)
acceptedRightEndpointCurrent name key world error value nameEq keyEq left right renaming registrations aligned empty selected observed found =
  case registrationSideGenerationScan name key world error value nameEq Z emptyRegistrationIndex
    right (rightFinalIndex registrations) (rightRegistrationSideScan (generationTraceCorrespondence registrations)) of
...
0 acceptedRightCurrentFiber :
  ...
  (fiber : Fiber name key value world error **
    lookupFiber {name = name} {key = key} {value = value} {world = world} {error = error}
      @{nameEq} selected (registry rightFinal) = Just fiber)
acceptedRightCurrentFiber name key world error value nameEq keyEq left right renaming registrations aligned selected generation current =
  case registrationSideGenerationScan name key world error value nameEq Z emptyRegistrationIndex
    right (rightFinalIndex registrations) (rightRegistrationSideScan (generationTraceCorrespondence registrations)) of
    (finalOrdinal ** scan) => currentFiberFromEmptyScan name key world error value nameEq keyEq right finalOrdinal
      (rightFinalGenerations registrations) scan aligned selected generation current

||| Full forward DOMAIN bridge from endpoint support: both exact accepted
||| current lookups, the same generation bijection, and the actual right fiber.
||| This deliberately does not assert destination support or component equality.
export
0 acceptedSupportedForwardDomain :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) ->
  {initial, leftFinal, rightFinal : SystemState name key value world error} ->
  ...
  (leftGeneration : RegistrationGeneration name ** rightGeneration : RegistrationGeneration name **
    rightFiber : Fiber name key value world error **
    (lookupCurrentGeneration @{nameEq} selected (leftFinalGenerations (generatedRegistrationTree sameInputs)) = Just leftGeneration,
     lookupCurrentGeneration @{nameEq} (renameForward (currentNameBijection (endpointRenaming sameInputs)) selected)
       (rightFinalGenerations (generatedRegistrationTree sameInputs)) = Just rightGeneration,
...
acceptedSupportedForwardDomain name key world error value nameEq keyEq {leftFinal} left right sameInputs leftAligned rightAligned empty selected supported =
  case computedSupportPresent name key world error value nameEq keyEq leftFinal selected supported of
    (leftFiber ** leftFound) => case acceptedLeftEndpointCurrent name key world error value nameEq keyEq left right
      (generatedGenerationBijection sameInputs) (generatedRegistrationTree sameInputs) leftAligned empty selected leftFiber leftFound of
      (leftGeneration ** leftCurrent) => case leftCurrentGenerationMapped (endpointRenaming sameInputs) selected leftGeneration leftCurrent of
        Left vestigial => case trans (sym supported) (vestigialUnsupported vestigial) of Refl impossible
        Right (rightGeneration ** (mapped, rightCurrent)) =>
          case acceptedRightCurrentFiber name key world error value nameEq keyEq left right
...
0 projectionRightBirthCoverage :
  (name, key, world, error : Type) -> (value : key -> Type) -> (nameEq : DecEq name) ->
  {leftFirst, leftFinal, rightFirst, rightFinal : SystemState name key value world error} ->
  (left : Transitions leftFirst leftFinal) -> (right : Transitions rightFirst rightFinal) ->
  (renaming : RegistrationGenerationBijection name) ->
  ...
  ClassifiedGeneratedBirth name key world error value Z right
    (rightScannedEvents (authenticatedMatchingFromProjection name key world error value nameEq left right renaming projection)) selected
...
||| ALL supported generated left endpoint names belong to the exact accepted
||| retained-event list. The authentic original birth is derived from lookup.
export
0 acceptedSupportedGeneratedLeftCoverage :
  (name, key, world, error : Type) -> (value : key -> Type) ->
  ...
  (renaming : RegistrationGenerationBijection name) ->
  (registrations : RegistrationCorrespondenceByGeneration nameEq renaming left right) ->
  AlignedTransitions name key world error value nameEq keyEq left -> RegistrationDiscipline protocol nameEq left ->
  (bindings (registry leftFirst) = []) -> UniqueRawNameInsertions name key world error value nameEq keyEq left ->
  (selected : name) -> (fiber : Fiber name key value world error) ->
  (lookupFiber {name = name} {key = key} {value = value} {world = world} {error = error} @{nameEq} selected (registry leftFinal) = Just fiber) ->
  (parent : name) -> (fiberParent fiber = ChildOf parent) ->
  (isSupported {name = name} {key = key} {value = value} {world = world} {error = error} @{nameEq} @{keyEq} selected leftFinal = True) ->
  (event : RegistrationEvent name key world error value **
    (Elem event (leftScannedEvents (acceptedAuthenticatedRegistrationMatching name key world error value nameEq left right renaming registrations)),
     eventChild event = selected))
acceptedSupportedGeneratedLeftCoverage name key world error value nameEq keyEq protocol left right renaming registrations
  aligned discipline empty unique selected fiber found parent parentExact supported =
    supportedClassifiedBirthRetained name key world error value nameEq keyEq protocol left aligned discipline empty unique
      (leftScannedEvents (acceptedAuthenticatedRegistrationMatching name key world error value nameEq left right renaming registrations))
      selected fiber found supported
      (acceptedLeftBirthCoverage name key world error value nameEq left right renaming registrations selected parent (fiberComponent fiber)
