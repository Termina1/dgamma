...
||| External order follows from the actual finite chain INSIDE expandedRun.
||| This does not assert that a chain exists for two nonempty root bundles;
||| constructing expandedRun is precisely the AT-RISK obligation.
export
0 o19ExpandedTargetSameExternalInputs :
  {name, key, world, error : Type} -> {value : key -> Type} ->
  (nameEq : DecEq name) -> (keyEq : DecEq key) -> (protocol : RegistrationProtocol key value world error) ->
  {sourceOrder, targetOrder : List name} -> (swap : AdjacentActorOrderSwap name sourceOrder targetOrder) ->
  {initial, sourceFinal : SystemState name key value world error} -> (source : Transitions initial sourceFinal) ->
